---
title: "Get started with OpenrindDesktop Cloud"
description: "Connect the desktop app to your OpenrindDesktop Cloud org and unlock shared workspaces, skill hubs, and shared providers"
---

OpenrindDesktop Cloud requires an active subscription at [app.openrind-desktoplabs.com](https://app.openrind-desktoplabs.com/).
It is intended for teams what want to share `SKILLs.md`, `MCPs` and `configs`. 


First, you should connect your desktop app with OpenrindDesktop Cloud.

<Frame>
  ![OpenrindDesktop Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.openrind-desktoplabs.com](https://app.openrind-desktoplabs.com/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if OpenrindDesktop Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.openrind-desktoplabs.com/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to OpenrindDesktop automatically, click `Paste sign-in code`, paste the `openrind-desktop://den-auth?...` link or one-time code from OpenrindDesktop Cloud, then click `Finish sign-in`.
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted OpenrindDesktop runtimes that teammates can open from the desktop app.
