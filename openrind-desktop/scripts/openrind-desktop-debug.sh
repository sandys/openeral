#!/usr/bin/env bash
#
# openrind-desktop-debug.sh — one-stop observability + lifecycle control for the
# OpenrindDesktop dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan openrind-desktop/opencode processes (ppid == launchd)
...
#   wait-healthy    block until openrind-desktop-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (OPENRIND_DESKTOP_DEV_VARIANT):
#   electron   (default) pnpm --filter @openrind/desktop dev:electron
...
#   3. Tauri webview       (target/debug/OpenrindDesktop-Dev)  <-- never /Applications/
#   4. Electron main+helpers (node_modules/electron/...Electron.app)
#   5. Vite                (node node_modules/.../vite)
#   6. orchestrator + openrind-desktop-server + opencode + opencode-router
...
#   - ~/Library/Application Support/com.differentai.openrind-desktop.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/OpenrindDesktop.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${OPENRIND_DESKTOP_DEV_LOG_FILE:-$HOME/.openrind-desktop/debug/openrind-desktop-dev.log}"
PNPM_DEV_LOG="${OPENRIND_DESKTOP_PNPM_DEV_LOG:-/tmp/openrind-desktop-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${OPENRIND_DESKTOP_PNPM_DEV_PID:-/tmp/openrind-desktop-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${OPENRIND_DESKTOP_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${OPENRIND_DESKTOP_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${OPENRIND_DESKTOP_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[openrind-desktop-debug] unknown OPENRIND_DESKTOP_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
log() { printf '[openrind-desktop-debug] %s\n' "$*"; }
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod OpenrindDesktop.app is safe).
  local pattern="$1"
  local pids
...
discover_openrind_desktop_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/openrind-desktop-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.differentai\.openrind-desktop/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.OPENRIND_DESKTOP_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  ps -Ao pid,ppid,command | awk '/target\/debug\/OpenrindDesktop-Dev|node_modules\/electron\/dist\/Electron\.app\/Contents\/MacOS\/Electron|apps\/desktop\/scripts\/electron-dev\.mjs|target\/debug\/openrind-desktop-server|target\/debug\/openrind-desktop-orchestrator|target\/debug\/opencode( |\/)|target\/debug\/opencode-router|apps\/desktop\/src-tauri\/sidecars\/openrind-desktop-server|apps\/desktop\/src-tauri\/sidecars\/openrind-desktop-orchestrator|apps\/desktop\/src-tauri\/sidecars\/opencode( |\/)|apps\/desktop\/src-tauri\/sidecars\/opencode-router|vite|pnpm .*dev/ && !/awk/ && !/grep/' | sed -E 's#/Users/[^ ]*/#…/#g' | head -20

  echo
  echo "=== openrind-desktop-server ==="
  local port
  port=$(discover_openrind_desktop_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev openrind-desktop-server running)"
  else
  ...
  oc_port=$(ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/openrind-desktop-orchestrator" \
    | grep -v grep \
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /openrind-desktop-server|openrind-desktop-orchestrator|opencode( |\/)|opencode-router/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with OPENRIND_DESKTOP_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /openrind-desktop-server|openrind-desktop-orchestrator|opencode( |\/)|opencode-router/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  echo "=== openrind-desktop hang diagnosis ==="
  echo "time=$now"
  echo "repo=$REPO_ROOT"
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(OPENRIND_DESKTOP_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_openrind_desktop_server_port)
  if [[ -n "$port" ]]; then
    echo "  openrind-desktop-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no openrind-desktop-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/openrind-desktop-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set OPENRIND_DESKTOP_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "target/debug/OpenrindDesktop-Dev"
  ...
  # 5. openrind-desktop-server / orchestrator / opencode / opencode-router for the
  ...
  kill_by_pattern "target/debug/openrind-desktop-server"
  kill_by_pattern "target/debug/openrind-desktop-orchestrator"
  kill_by_pattern "target/debug/opencode"
  kill_by_pattern "target/debug/opencode-router"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/openrind-desktop-server"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/openrind-desktop-orchestrator"
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env OPENRIND_DESKTOP_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @openrind/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "openrind-desktop-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "openrind-desktop-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the openrind-desktop-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.differentai.openrind-desktop.dev"
