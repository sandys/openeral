...
const composeProject = "openrind-desktop-den-local"
...
const databaseUrl = process.env.DATABASE_URL?.trim() || "mysql://root:password@127.0.0.1:3306/openrind_desktop_den"
...
async function main() {
  ...
  await run("bash", ["-lc", "pnpm --filter @openrind-ee/den-db build && pnpm --filter @openrind-ee/den-db exec node --import tsx ./node_modules/drizzle-kit/bin.cjs push --config drizzle.config.ts --force"], {
    env: {
      ...process.env,
  ...
  turboChild = spawn(
    ...
    [
      ...
      "dev:local",
      "--output-logs=full",
        "--filter=@openrind-ee/den-api",
        "--filter=@openrind-ee/den-worker-proxy",
        "--filter=@openrind-ee/den-web",
    ],
    {
      cwd: rootDir,
      stdio: "inherit",
      ...
      env: {
        ...process.env,
        OPENRIND_DESKTOP_DEV_MODE: process.env.OPENRIND_DESKTOP_DEV_MODE?.trim() || "1",
        DATABASE_URL: databaseUrl,
