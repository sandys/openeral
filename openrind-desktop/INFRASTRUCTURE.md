# Openrind Desktop Infrastructure Principles

Openrind Desktop is an experience layer. `opencode` is the engine. This document defines how infrastructure is built so every component is usable on its own, composable as a sidecar, and easy to automate.
...
* The Openrind Desktop UI may wrap these, but never replace or lock them out.
...
* OpenCode remains the engine; Openrind Desktop adds a thin config + UX layer.
...
* Openrind Desktop never replaces the CLI; it only connects to the engine.

### Openrind Desktop Server

* Runs standalone via `openrind-desktop-server` CLI.
* Provides filesystem-backed config surfaces (skills, plugins, MCP, commands).
* Sidecar lifecycle is described in `packages/app/pr/openrind-desktop-server.md`.
* Can also be consumed as a hosted Openrind Desktop Cloud control surface for remote worker lifecycle.

### Openrind Desktop Cloud Control Plane

* Hosted deployment of Openrind Desktop server capabilities for worker provisioning and remote connect.
...
* Must be able to use Openrind Desktop server for config and approvals.
...
* `PRINCIPLES.md`
* `ARCHITECTURE.md`
* `packages/app/pr/openrind-desktop-server.md`
