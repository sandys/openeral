...
function resolveOpenrindDesktopDataDir(): string {
  const override = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim();
  if (override) return expandHome(override);
  return join(homedir(), ".openrind-desktop", "openrind-desktop-server");
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveOpenrindDesktopDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "openrind-desktop", "audit.jsonl");
}
