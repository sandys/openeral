...
describe("env-file", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "openrind-desktop-env-"));
  ...
  test("isReservedEnvKey blocks OPENRIND_DESKTOP_ / OPENCODE_ prefixes", () => {
    expect(isReservedEnvKey("OPENRIND_DESKTOP_TOKEN")).toBe(true);
  ...
  test("upsertMany rejects reserved keys", async () => {
    const svc = new EnvService({ path });
    const promise = svc.upsertMany([{ key: "OPENRIND_DESKTOP_TOKEN", value: "x" }]);
    await expect(promise).rejects.toBeInstanceOf(InvalidEnvKeyError);
  ...
  test("readForInjection strips reserved keys even if present on disk", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        updatedAt: Date.now(),
        variables: [
          { key: "OPENRIND_DESKTOP_TOKEN", value: "stolen" },
