...
export function normalizeBlueprintSessionTemplates(openrindDesktop: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(openrindDesktop?.blueprint);
...
export function readMaterializedBlueprintSessions(openrindDesktop: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(openrindDesktop?.blueprint);
...
export function sanitizeOpenrindDesktopTemplateConfig(openrindDesktop: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(openrindDesktop ?? {});
...
export function applyMaterializedBlueprintSessions(
  openrindDesktop: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeOpenrindDesktopTemplateConfig(openrindDesktop);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
