...
if (!config.opencodeBaseUrl && process.env.OPENRIND_DESKTOP_MANAGE_OPENCODE === "1") {
  const workspace = config.workspaces[0];
  if (workspace?.path) {
    const managedOpencodeCwd = process.env.OPENRIND_DESKTOP_MANAGED_OPENCODE_CWD?.trim() || workspace.path;
    await mkdir(managedOpencodeCwd, { recursive: true });
    managedOpencode = await createManagedOpencodeServer({
      bin: process.env.OPENRIND_DESKTOP_OPENCODE_BIN,
      cwd: managedOpencodeCwd,
      env: {
        ...(process.env.OPENRIND_DESKTOP_DEV_MODE ? { OPENRIND_DESKTOP_DEV_MODE: process.env.OPENRIND_DESKTOP_DEV_MODE } : {}),
      },
    });
...
logger.log("info", `Openrind Desktop server listening on ${url}`);
