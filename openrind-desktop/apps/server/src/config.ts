...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.OPENRIND_DESKTOP_SERVER_CONFIG;
  const configPath = cli.configPath ?? envConfigPath ?? resolve(homedir(), ".config", "openrind-desktop", "server.json");
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.OPENRIND_DESKTOP_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.OPENRIND_DESKTOP_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.OPENRIND_DESKTOP_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.OPENRIND_DESKTOP_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.OPENRIND_DESKTOP_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.OPENRIND_DESKTOP_TOKEN;
  const hostTokenFromEnv = process.env.OPENRIND_DESKTOP_HOST_TOKEN;
  ...
  const approvalTimeoutMs =
    ...
    (process.env.OPENRIND_DESKTOP_APPROVAL_TIMEOUT_MS ? Number(process.env.OPENRIND_DESKTOP_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.OPENRIND_DESKTOP_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.OPENRIND_DESKTOP_READONLY;
  ...
  const envLogFormat = process.env.OPENRIND_DESKTOP_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.OPENRIND_DESKTOP_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.OPENRIND_DESKTOP_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.OPENRIND_DESKTOP_PORT ? Number(process.env.OPENRIND_DESKTOP_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
