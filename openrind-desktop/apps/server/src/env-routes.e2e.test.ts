...
const dirs: string[] = [];
const priorEnvStore = process.env.OPENRIND_DESKTOP_ENV_STORE;
const priorTokenStore = process.env.OPENRIND_DESKTOP_TOKEN_STORE;
...
function hostAuth() {
  return { "x-openrinddesktop-host-token": HOST_TOKEN, "content-type": "application/json" };
}

beforeEach(() => {
  const dir = mkdtempSync(join(tmpdir(), "openrind-desktop-env-routes-"));
  dirs.push(dir);
  // Redirect the shared env.json path into a throwaway dir so the test never
  // touches the developer's real ~/.config/openrind-desktop/env.json.
  process.env.OPENRIND_DESKTOP_ENV_STORE = join(dir, "env.json");
  process.env.OPENRIND_DESKTOP_TOKEN_STORE = join(dir, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorEnvStore === undefined) {
    delete process.env.OPENRIND_DESKTOP_ENV_STORE;
  } else {
    process.env.OPENRIND_DESKTOP_ENV_STORE = priorEnvStore;
  }
  if (priorTokenStore === undefined) {
    delete process.env.OPENRIND_DESKTOP_TOKEN_STORE;
  } else {
    process.env.OPENRIND_DESKTOP_TOKEN_STORE = priorTokenStore;
  }
});
...
describe("env routes", () => {
  ...
  test("invalid env store returns 409 instead of overwriting on PUT", async () => {
    writeFileSync(process.env.OPENRIND_DESKTOP_ENV_STORE!, "{ this is not json");
    const { base } = boot();
  ...
  test("PUT rejects reserved keys with 400", async () => {
    ...
    const put = await fetch(`${base}/env`, {
      method: "PUT",
      headers: hostAuth(),
      body: JSON.stringify({ key: "OPENRIND_DESKTOP_TOKEN", value: "x" }),
    });
    expect(put.status).toBe(400);
    const body = (await put.json()) as { code: string; message: string };
    expect(body.code).toBe("reserved_env_key");
    expect(body.message).toBe("Environment variable name is reserved for Openrind Desktop internals");
    expect(body.message).not.toContain("OPENRIND_DESKTOP_TOKEN");
  });
