...
const DEFAULT_AGENT = "openrind-desktop";
const DEFAULT_PROVIDER = "openai";
const DEFAULT_MODEL = "gpt-5.4";
const OPENRIND_DESKTOP_DEV_DATA_DIRS = ["openrind-desktop-dev-data", "opencode-dev"];
...
function opencodeOrchestratorDataDirs(): string[] {
  const root = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim();
  if (!root) return [];
  ...
  for (const name of OPENRIND_DESKTOP_DEV_DATA_DIRS) {
