...
const DEFAULT_PUBLISHER_BASE_URL = String(process.env.OPENRIND_DESKTOP_PUBLISHER_BASE_URL ?? "").trim() || "https://share.openrind-desktoplabs.com";
const DEFAULT_PUBLISHER_ORIGIN = String(process.env.OPENRIND_DESKTOP_PUBLISHER_REQUEST_ORIGIN ?? "").trim() || "https://app.openrind-desktop.software";
...
export function resolveTrustedSharedBundleFetchUrl(bundleUrl: unknown): URL {
  ...
  if (inputUrl.origin !== trustedBaseUrl.origin) {
    throw new ApiError(
      ...
      `Shared bundle URLs must use the configured Openrind Desktop publisher (${trustedBaseUrl.origin}). Import only bundles from trusted sources.`,
    );
  }
  ...
  try {
    const response = await fetch(`${baseUrl}/v1/bundles`, {
      ...
      headers: {
        ...
        ...(input.name?.trim() ? { "X-OpenrindDesktop-Name": input.name.trim() } : {}),
      },
