...
const VALID_SKILL_SHARE_URL = `https://share.openrind-desktoplabs.com/b/${VALID_SKILL_BUNDLE_ID}`;
  ...
  test("rejects bundle URLs that use another origin", () => {
    expect(() => resolveTrustedSharedBundleFetchUrl(`https://evil.example/b/${VALID_SKILL_BUNDLE_ID}`)).toThrow(
      new ApiError(
        ...
        "Shared bundle URLs must use the configured Openrind Desktop publisher (https://share.openrind-desktoplabs.com). Import only bundles from trusted sources.",
      ),
    );
  });

  test("rejects bundle URLs without a bundle id path", () => {
    expect(() => resolveTrustedSharedBundleFetchUrl("https://share.openrind-desktoplabs.com/not-a-bundle")).toThrow(
