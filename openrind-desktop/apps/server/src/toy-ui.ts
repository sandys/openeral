...
<html lang="en">
  <head>
    ...
    <title>Openrind Desktop Toy UI</title>
    <link rel="icon" type="image/svg+xml" href="/ui/assets/openrind-desktop-mark.svg" />
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>Openrind Desktop Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by openrind-desktop-server)</div>
        </div>
        <div class="row">
        ...
        <div class="card">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/openrind-desktop/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/openrind-desktop/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "openrind-desktop.toy.token";
const STORAGE_SESSION_PREFIX = "openrind-desktop.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "Openrind Desktop Toy UI" }),
  });
...
function renderConnectArtifact(workspaceId, token, scope) {
  ...
  const payload = {
    kind: "openrind-desktop.connect.v1",
    hostUrl: hostUrl,
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's Openrind Desktop server config?")) return;
    try {
