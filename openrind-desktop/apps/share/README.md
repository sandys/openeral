# Openrind Desktop Share Service (Publisher)

This is a Next.js publisher app for Openrind Desktop "share link" bundles.
...
- `GET /`
  - Human-friendly packaging page for Openrind Desktop worker files.
  - Supports drag/drop of skills, agents, commands, `opencode.json[c]`, and `openrind-desktop.json`.
  - Previews the inferred bundle and publishes a share link.

- `POST /v1/bundles`
  - Accepts JSON bundle payloads.
  - Stores bytes in Vercel Blob.
  - Returns `{ "url": "https://share.openrind-desktoplabs.com/b/<id>" }`.

- `POST /v1/package`
  - Accepts `{ files: [{ path, name?, content }], preview?: boolean }`.
  - Parses supported Openrind Desktop files into the smallest useful bundle shape.
...
- `GET /b/:id`
  ...
  - Includes an **Open in app** action that opens `openrind-desktop://import-bundle` with:
    - `ow_bundle=<share-url>`
    - `ow_intent=new_worker` (desktop Openrind Desktop converts single-skill bundles into a destination picker before import)
...
- `PUBLIC_BASE_URL`
  - Default: `https://share.openrind-desktoplabs.com`
...
- `OPENRIND_DESKTOP_PUBLISHER_ALLOWED_ORIGINS`
  - Optional comma-separated browser origins allowed to publish bundles.
  - Defaults include the share origin, the hosted Openrind Desktop app origin, and common local dev origins.
...
# Machine-readable payload (Openrind Desktop parser path)
