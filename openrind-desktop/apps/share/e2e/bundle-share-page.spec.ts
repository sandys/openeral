...
test("shows a read-only shared skill page with Openrind Desktop import actions", async ({ page }) => {
  ...
  const openInAppHref = await page.getByRole("link", { name: /^open in openrind-desktop$/i }).getAttribute("href");
  expect(openInAppHref).toBeTruthy();
  expect(openInAppHref ?? "").toContain("openrind-desktop://import-bundle?");

  const openInAppLink = page.getByRole("link", { name: /^open in openrind-desktop$/i });
  ...
  const deepLinkQuery = new URL((openInAppHref ?? "").replace("openrind-desktop://import-bundle?", "https://example.test/?"));
...
test("publishes a share page with a valid OG preview card for link unfurls", async ({ page }) => {
  ...
  expect(svg).toContain("share.openrind-desktoplabs.com");
