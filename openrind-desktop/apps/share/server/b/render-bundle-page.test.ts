...
function makeReq({ accept = "", query = {}, host = "share.openrind-desktoplabs.com" }: { accept?: string; query?: Record<string, string>; host?: string } = {}): RequestLike {
  return {
    query,
    headers: {
      accept,
...
test("renderBundlePage includes machine-readable metadata and escaped json script", () => {
  ...
  const html = renderBundlePage({
    id: "01TEST",
    rawJson,
    req: makeReq({ accept: "text/html", host: "share.openrind-desktoplabs.com" }),
  });

  assert.match(html, /data-openrind-desktop-share="true"/);
  assert.match(html, /data-openrind-desktop-bundle-type="skill"/);
  assert.match(html, /meta name="openrind-desktop:bundle-id" content="01TEST"/);
  assert.match(html, /\/b\/01TEST\/data/);
  assert.match(html, /openrind-desktop:\/\/import-bundle\?/);
  assert.match(html, /ow_bundle=https%3A%2F%2Fshare\.openrind-desktoplabs\.com%2Fb%2F01TEST/);
  assert.match(html, /ow_intent=new_worker/);
  assert.match(html, /ow_source=share_service/);
  assert.match(html, /id="openrind-desktop-bundle-json" type="application\/json"/);
  ...
  assert.match(html, /Open in Openrind Desktop app/);
  assert.match(html, /Open in an Openrind Desktop den/);
