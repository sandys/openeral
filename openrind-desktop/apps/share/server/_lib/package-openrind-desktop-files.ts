...
function isOpenrindDesktopConfigFile(file: NormalizedFile): boolean {
  return /(^|\/)(?:\.opencode\/)?openrindDesktop\.jsonc?$/i.test(file.path);
}
...
function looksLikeOpenrindDesktopShape(parsed: unknown): boolean {
  if (!isRecord(parsed)) return false;

  const schema = maybeString(parsed.$schema).toLowerCase();
  return schema.includes("openrind-desktop") && schema.includes("config");
}
...
interface ConfigSection {
  ...
  openrindDesktop: Record<string, unknown> | null;
  config: Record<string, unknown>;
  preview: PreviewItem[];
  configCount: number;
}
...
function readConfigSection(file: NormalizedFile, warnings: string[]): ConfigSection | null {
  ...
  let openrindDesktop: Record<string, unknown> | null = null;
  const config: Record<string, unknown> = {};
  const preview: PreviewItem[] = [];
  let configCount = 0;

  if (isOpenrindDesktopConfigFile(file)) {
    openrindDesktop = cloneRecord(parsed);
    preview.push(buildConfigPreview(file, "Openrind Desktop config"));
    configCount += 1;
  }
  ...
  if (!opencode && !openrindDesktop) {
    if (looksLikeNamedMcpFile(file)) {
      ...
      preview.push(...collectOpencodePreviewItems(parsed));
      configCount += 1;
    } else if (looksLikeOpenrindDesktopShape(parsed)) {
      openrindDesktop = cloneRecord(parsed);
      preview.push(buildConfigPreview(file, "Openrind Desktop config"));
      configCount += 1;
  ...
  const secretHits = [
    ...
    ...redactSecrets(openrindDesktop ?? {}, ["openrind-desktop"]),
  ...
  if (!opencode && !openrindDesktop && !Object.keys(config).length) {
    warnings.push(`Ignored unsupported config file: ${file.path}`);
    return null;
  }

  return { opencode, openrindDesktop, config, preview, configCount };
}
...
export function packageOpenrindDesktopFiles(input: PackageInput): PackageResult {
  ...
  if (!summary.skills && !summary.agents && !summary.mcpServers && !summary.commands && !summary.configs && !openrindDesktop) {
    throw new Error("No shareable files found. Drop markdown or JSON/JSONC config files to package them.");
  }

  const hasWorkspaceConfig =
    Boolean(openrindDesktop) ||
