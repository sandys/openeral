...
function buildRootOgInput(): OgImageModel {
  return {
    title: "Share Openrind Desktop skills beautifully",
    fileName: "agent-creator.md",
    fileType: "SKILL.md",
    description: "Clean metadata-first social cards for shared Openrind Desktop skills and bundles.",
    category: "share",
    tag: "openrind-desktop preview",
...
function buildBundleOgInput({ rawJson }: { id: string; rawJson: string }): OgImageModel {
  ...
  const bundleName = maybeString(data.name).trim() || bundle.name || titleFromFileName(preview.filename) || "Openrind Desktop bundle";
  ...
  const title =
    bundle.type === "skills-set" && bundle.skills.length > 1
      ...
      : humanizeTitle(bundleName) || "Openrind Desktop bundle";
...
export function computeOgImageLayout(model: OgImageModel): OgImageLayout {
  const displayTitle = truncateAtWordBoundary(humanizeTitle(model.title) || "Openrind Desktop bundle", MAX_DISPLAY_CHARS);
...
function renderOpenrindDesktopLogo({ x, y, width, height }: { x: number; y: number; width: number; height: number }): string {
  return `
...
<svg width="${variantConfig.width}" height="${variantConfig.height}" viewBox="0 0 ${BASE_OG_IMAGE_WIDTH} ${BASE_OG_IMAGE_HEIGHT}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  ${renderOpenrindDesktopLogo({ x: cardX + 66, y: cardY + 34, width: 40, height: 34 })}
