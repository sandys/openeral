...
export const OPENRIND_DESKTOP_SITE_URL = "https://openrind-desktoplabs.com";
export const OPENRIND_DESKTOP_DOWNLOAD_URL = "https://openrind-desktoplabs.com/download";
export const DEFAULT_PUBLIC_BASE_URL = "https://share.openrind-desktoplabs.com";
...
export function setCors(
  ...
  res.setHeader(
    ...
    options.headers ??
      "Content-Type,Accept,X-OpenrindDesktop-Bundle-Type,X-OpenrindDesktop-Schema-Version,X-OpenrindDesktop-Name",
  );
}
...
export function buildOpenInAppUrls(shareUrl: string, options: { label?: string } = {}): OpenInAppUrls {
  ...
  const appScheme = (getEnv("PUBLIC_OPENRIND_DESKTOP_APP_SCHEME", "openrind-desktop") || "openrind-desktop").trim().toLowerCase();
  return {
...
export function buildBundlePreview(bundle: NormalizedBundle): {
  ...
  if (bundle.type === "skill") {
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(bundle.content, `# ${bundle.name || "Openrind Desktop skill"}`),
      tone: "skill",
...
export function buildBundlePreviewSelections(bundle: NormalizedBundle): {
  ...
  return [
    {
      id: "preview-0",
      name: bundle.name || "Openrind Desktop bundle",
...
export function buildBundleNarrative(bundle: NormalizedBundle): string {
  ...
  return parts.length
    ? `${parts.join(", ")} bundled into a worker package that imports through Openrind Desktop with one step.`
    : "Worker configuration bundle prepared for Openrind Desktop import.";
}
