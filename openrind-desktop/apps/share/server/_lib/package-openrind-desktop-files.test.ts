...
import { packageOpenrindDesktopFiles } from "./package-openrind-desktop-files.ts";

test("packageOpenrindDesktopFiles creates a single skill bundle from skill markdown", () => {
  const result = packageOpenrindDesktopFiles({
    files: [
      {
...
test("packageOpenrindDesktopFiles accepts any single text file when frontmatter has name and description", () => {
  const result = packageOpenrindDesktopFiles({
    files: [
      {
        path: "AGENTS.md",
...
test("packageOpenrindDesktopFiles rejects content without name and description frontmatter", () => {
  assert.throws(
    () =>
      packageOpenrindDesktopFiles({
        files: [
          {
            path: "whatever.txt",
...
test("packageOpenrindDesktopFiles rejects multiple uploaded files", () => {
  assert.throws(
    () =>
      packageOpenrindDesktopFiles({
        files: [
...
test("packageOpenrindDesktopFiles rejects config json uploads", () => {
  assert.throws(
    () =>
      packageOpenrindDesktopFiles({
        files: [
...
test("packageOpenrindDesktopFiles rejects agent and config combinations", () => {
  assert.throws(
    () =>
      packageOpenrindDesktopFiles({
        files: [
