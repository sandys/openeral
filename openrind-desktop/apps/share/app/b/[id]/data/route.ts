import {
  fetchBundleJsonById,
  isValidBundleId,
} from "../../../../server/_lib/blob-store.ts";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isValidBundleId(id)) {
    return Response.json({ message: "invalid bundle id" }, { status: 400 });
  }
  ...
  try {
    ...
    if (wantsDownload(requestLike)) {
      responseHeaders.set("Content-Disposition", `attachment; filename="openrind-desktop-bundle-${id}.json"`);
    }
