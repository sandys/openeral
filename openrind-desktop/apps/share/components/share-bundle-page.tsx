...
const SPECIAL_TOKENS: Record<string, string> = {
  ...
  mdx: "MDX",
  opencode: "OpenCode",
  openrindDesktop: "Openrind Desktop",
  yaml: "YAML",
  yml: "YAML",
};
...
function humanizeLabel(value: string | null | undefined): string {
  ...
  if (!normalized) return "Openrind Desktop Bundle";
...
function formatSurfaceEyebrow(selection: BundleSelection | undefined, parsedName: string, fallbackTitle: string | undefined): string {
  ...
  if (/^openrind-desktop\.json$/i.test(filename)) return "Openrind Desktop Config";
...
export default function ShareBundlePage(props: BundlePageProps & { stars?: string }) {
  ...
  const openInOpenrindDesktop = (event: MouseEvent<HTMLAnchorElement>) => {
  ...
  const prepareOpenInOpenrindDesktop = (event: PointerEvent<HTMLAnchorElement> | MouseEvent<HTMLAnchorElement>) => {
    refreshOpenInAppHref(event.currentTarget);
  };

  const prepareOpenInOpenrindDesktopFromKeyboard = (event: KeyboardEvent<HTMLAnchorElement>) => {
  ...
  const actionButtons = (
    <>
      ...
      <a
        ...
        href={openInAppUrl}
        onPointerDown={prepareOpenInOpenrindDesktop}
        onMouseDown={prepareOpenInOpenrindDesktop}
        onKeyDown={prepareOpenInOpenrindDesktopFromKeyboard}
        onClick={openInOpenrindDesktop}
      >
        Open in Openrind Desktop
      </a>
    </>
  );
    ...
    <main className="shell">
      ...
      {props.missing ? (
        <section className="status-card">
          <span className="eyebrow">Openrind Desktop Share</span>
