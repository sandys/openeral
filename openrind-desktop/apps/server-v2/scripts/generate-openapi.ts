...
async function main() {
  const workingDirectory = await mkdtemp(join(os.tmpdir(), "openrind-desktop-server-v2-openapi-"));
  ...
  try {
    ...
    process.stdout.write(`[openrind-desktop-server-v2] ${changed ? "wrote" : "verified"} ${outputPath}\n`);
  } finally {
