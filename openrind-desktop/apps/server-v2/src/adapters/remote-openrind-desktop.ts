...
export function buildRemoteOpenrindDesktopHeaders(server: ServerRecord) {
  const auth = server.auth && typeof server.auth === "object" ? server.auth as Record<string, unknown> : null;
  const headers: Record<string, string> = {
    Accept: "application/json",
  };
  const bearer = pickString(auth, ["openrindDesktopClientToken", "openrindDesktopToken", "authToken", "token", "bearerToken"]);
  const hostToken = pickString(auth, ["openrindDesktopHostToken", "hostToken"]);
...
function unwrapEnvelope<T>(payload: unknown): T {
  if (payload && typeof payload === "object" && "ok" in (payload as Record<string, unknown>)) {
    ...
    if (record.ok === false) {
      ...
      const message = typeof error.message === "string" ? error.message : "Remote Openrind Desktop request failed.";
...
export async function requestRemoteOpenrindDesktop<T>(input: {
  body?: unknown;
  method?: string;
  path: string;
  server: ServerRecord;
  ...
  const response = await fetch(`${normalizeBaseUrl(baseUrl)}${input.path}`, {
    ...
    headers: {
      ...buildRemoteOpenrindDesktopHeaders(input.server),
  ...
  if (response.status === 401) {
    throw new RouteError(502, "bad_gateway", "Remote Openrind Desktop server rejected the stored credentials.");
  }
  if (response.status === 403) {
    throw new RouteError(502, "bad_gateway", "Remote Openrind Desktop server rejected the stored permissions.");
  }
  if (!response.ok) {
    const message = typeof (payload as any)?.error?.message === "string"
      ...
      : typeof (payload as any)?.message === "string"
        ...
        : `Remote Openrind Desktop request failed with status ${response.status}.`;
...
export async function requestRemoteOpenrindDesktopRaw(input: {
  body?: BodyInit | null;
  ...
  if (!response.ok) {
    ...
    throw new RouteError(502, "bad_gateway", text.trim() || `Remote Openrind Desktop request failed with status ${response.status}.`);
  }
