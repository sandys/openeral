...
export const authSummarySchema = z.object({
  ...
  headers: z.object({
    ...
    hostToken: z.literal("X-OpenrindDesktop-Host-Token"),
  }),
  required: z.boolean(),
  scopes: z.object({
...
}).meta({ ref: "OpenrindDesktopServerV2AuthSummary" });
...
}).meta({ ref: "OpenrindDesktopServerV2ServerInventoryItem" });
...
}).meta({ ref: "OpenrindDesktopServerV2RegistrySummary" });
...
}).meta({ ref: "OpenrindDesktopServerV2CapabilitiesData" });

const workspaceBackendSchema = z.object({
  kind: z.enum(["local_opencode", "remote_openrind-desktop"]),
  local: z.object({
  ...
  remote: z.object({
    ...
    remoteType: z.enum(["openrind-desktop", "opencode"]),
    remoteWorkspaceId: z.string().nullable(),
    workspaceName: z.string().nullable(),
  }).nullable(),
  serverId: identifierSchema,
}).meta({ ref: "OpenrindDesktopServerV2WorkspaceBackend" });

const workspaceRuntimeSummarySchema = z.object({
  backendKind: z.enum(["local_opencode", "remote_openrind-desktop"]),
...
}).meta({ ref: "OpenrindDesktopServerV2WorkspaceRuntimeSummary" });
...
}).meta({ ref: "OpenrindDesktopServerV2RemoteServerConnectRequest" });

export const remoteServerSyncRequestSchema = z.object({
  directory: z.string().nullable().optional(),
  workspaceId: z.string().nullable().optional(),
}).meta({ ref: "OpenrindDesktopServerV2RemoteServerSyncRequest" });
...
}).meta({ ref: "OpenrindDesktopServerV2RemoteServerConnectData" });
...
export const systemStatusDataSchema = z.object({
  ...
  version: z.string(),
}).meta({ ref: "OpenrindDesktopServerV2SystemStatusData" });

export const capabilitiesResponseSchema = successResponseSchema("OpenrindDesktopServerV2CapabilitiesResponse", capabilitiesDataSchema);
export const serverInventoryListResponseSchema = successResponseSchema(
  "OpenrindDesktopServerV2ServerInventoryListResponse",
  serverInventoryListDataSchema,
);
export const remoteServerConnectResponseSchema = successResponseSchema(
  "OpenrindDesktopServerV2RemoteServerConnectResponse",
  remoteServerConnectDataSchema,
);
export const systemStatusResponseSchema = successResponseSchema("OpenrindDesktopServerV2SystemStatusResponse", systemStatusDataSchema);
export const workspaceDetailResponseSchema = successResponseSchema("OpenrindDesktopServerV2WorkspaceDetailResponse", workspaceDetailDataSchema);
export const workspaceListResponseSchema = successResponseSchema("OpenrindDesktopServerV2WorkspaceListResponse", workspaceListDataSchema);
