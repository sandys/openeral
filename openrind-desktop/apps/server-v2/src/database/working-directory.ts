...
function resolvePlatformDataRoot() {
  ...
  const devMode = isTruthy(process.env.OPENRIND_DESKTOP_DEV_MODE);
  const folderName = devMode ? "com.differentai.openrind-desktop.dev" : "com.differentai.openrind-desktop";
...
function resolveRootDir(options: ResolveServerWorkingDirectoryOptions) {
  ...
  const override = process.env.OPENRIND_DESKTOP_SERVER_V2_WORKDIR?.trim();
  if (override) {
    return path.resolve(override);
  }

  const sharedDataDir = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim();
  if (sharedDataDir) {
    return path.join(path.resolve(sharedDataDir), "server-v2");
  }

  if (options.environment === "test") {
    return path.join(process.cwd(), ".openrind-desktop-server-v2-test");
  }
...
export function resolveServerWorkingDirectory(options: ResolveServerWorkingDirectoryOptions): ServerWorkingDirectory {
  ...
  return {
    databaseDir,
    databasePath: path.join(databaseDir, "openrind-desktop-server-v2.sqlite"),
