...
const legacyWorkspaceSchema = z.object({
  ...
  openrindDesktopClientToken: z.string().optional().nullable(),
  openrindDesktopHostToken: z.string().optional().nullable(),
  openrindDesktopHostUrl: z.string().optional().nullable(),
  openrindDesktopToken: z.string().optional().nullable(),
  openrindDesktopWorkspaceId: z.string().optional().nullable(),
  openrindDesktopWorkspaceName: z.string().optional().nullable(),
  path: z.string().default(""),
  preset: z.string().optional().nullable(),
  remoteType: z.enum(["openrind-desktop", "opencode"]).optional().nullable(),
...
function detectRemoteHostingKind(value: string) {
  ...
  if (
    hostname === "app.openrind-desktoplabs.com" ||
    hostname === "app.openrind-desktop.software" ||
    hostname.endsWith(".openrind-desktoplabs.com") ||
    hostname.endsWith(".openrind-desktop.software")
  ) {
...
function legacyDesktopDataDirCandidates(explicitDir?: string) {
  ...
  const names = ["com.differentai.openrind-desktop.dev", "com.differentai.openrind-desktop", "Openrind Desktop Dev", "Openrind Desktop"];
...
function legacyOrchestratorDirCandidates(explicitDir?: string) {
  ...
  const candidates: string[] = [];
  const fromEnv = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim();
  if (fromEnv) {
    candidates.push(path.resolve(fromEnv));
  }

  const home = os.homedir();
  for (const name of ["openrind-desktop-orchestrator-dev-react", "openrind-desktop-orchestrator-dev", "openrind-desktop-orchestrator"]) {
    candidates.push(path.join(home, ".openrind-desktop", name));
  }
...
export function createServerPersistence(options: CreateServerPersistenceOptions): ServerPersistence {
  const inMemory = options.inMemory ?? (isTruthy(process.env.OPENRIND_DESKTOP_SERVER_V2_IN_MEMORY) || options.environment === "test");
  ...
  const desktopWorkspaceFile = resolveExistingFile(
    ...
    "openrind-desktop-workspaces.json",
  );
  ...
  if (shouldImportLegacyWorkspaceState && desktopWorkspaceFile) {
    try {
      ...
      for (const workspace of parsed.workspaces) {
          ...
          const record = registry.importLocalWorkspace({
            ...
            legacyNotes: {
              legacyDesktop: {
                ...
                source: "openrind-desktop-workspaces.json",
              },
            },
            status: "imported",
          });
        ...
        const remoteType = workspace.remoteType === "openrind-desktop" ? "openrind-desktop" : "opencode";
        const openrindDesktopServerBaseUrl = stripWorkspaceMount(workspace.openrindDesktopHostUrl ?? workspace.baseUrl ?? "");
        const remoteServerBaseUrl = openrindDesktopServerBaseUrl ?? normalizeUrl(workspace.baseUrl) ?? "";
        ...
        if (workspace.openrindDesktopToken?.trim()) auth.openrindDesktopToken = workspace.openrindDesktopToken.trim();
        if (workspace.openrindDesktopClientToken?.trim()) auth.openrindDesktopClientToken = workspace.openrindDesktopClientToken.trim();
        if (workspace.openrindDesktopHostToken?.trim()) auth.openrindDesktopHostToken = workspace.openrindDesktopHostToken.trim();
        ...
        const record = registry.importRemoteWorkspace({
          ...
          legacyNotes: {
            legacyDesktop: {
              ...
              displayName: workspace.displayName ?? null,
              legacyId: workspace.id,
              openrindDesktopHostUrl: workspace.openrindDesktopHostUrl ?? null,
          ...
          remoteWorkspaceId:
            ...
            parseWorkspaceIdFromUrl(workspace.openrindDesktopHostUrl ?? null) ||
  ...
  const orchestratorStateFile = resolveExistingFile(
    ...
    "openrind-desktop-orchestrator-state.json",
  );
  ...
  const orchestratorAuthFile = resolveExistingFile(
    legacyOrchestratorDirCandidates(options.legacy?.orchestratorDataDir),
    "openrind-desktop-orchestrator-auth.json",
  );
  ...
  const cloudSigninFile =
    ...
    resolveExistingFile(legacyDesktopDataDirCandidates(options.legacy?.desktopDataDir), "openrind-desktop-cloud-signin.json");
  const cloudSigninReport = options.legacy?.cloudSigninJson?.trim() || cloudSigninFile
    ? createEmptyReport("imported", cloudSigninFile ?? "env:OPENRIND_DESKTOP_SERVER_V2_CLOUD_SIGNIN_JSON")
    : createEmptyReport(
        "unavailable",
        null,
        {
