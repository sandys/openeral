...
function createPersistence(overrides: Partial<Parameters<typeof createServerPersistence>[0]> = {}) {
  return createServerPersistence({
    ...
    localServer: {
      ...
      label: "Local Openrind Desktop Server",
    },
    version: "0.0.0-test",
    ...overrides,
  });
...
test("fresh bootstrap seeds the local server and hidden workspaces", () => {
  const workingDirectory = makeTempDir("openrind-desktop-server-v2-phase2-fresh");
...
test("migration runner upgrades an existing database from the first migration", () => {
  const rootDir = makeTempDir("openrind-desktop-server-v2-phase2-upgrade");
...
test("legacy workspace import only runs once across repeated boots", () => {
  const rootDir = makeTempDir("openrind-desktop-server-v2-phase2-idempotent");
  const desktopDataDir = makeTempDir("openrind-desktop-server-v2-phase2-desktop");
  const orchestratorDataDir = makeTempDir("openrind-desktop-server-v2-phase2-orchestrator");
  const localWorkspaceDir = makeTempDir("openrind-desktop-server-v2-phase2-local-workspace");
  const orchestratorOnlyWorkspaceDir = makeTempDir("openrind-desktop-server-v2-phase2-orch-only-workspace");

  fs.writeFileSync(
    path.join(desktopDataDir, "openrind-desktop-workspaces.json"),
    JSON.stringify(
      {
        ...
        workspaces: [
          ...
          },
          {
            ...
            openrindDesktopHostUrl: "https://remote.example.com",
            openrindDesktopWorkspaceId: "remote-one",
            openrindDesktopToken: "client-token",
          },
        ],
      },
      null,
  ...
  fs.writeFileSync(
    path.join(orchestratorDataDir, "openrind-desktop-orchestrator-state.json"),
    JSON.stringify(
      {
        activeId: "orch-1",
        cliVersion: "0.11.206",
  ...
  const first = createPersistence({
    legacy: {
      cloudSigninJson: JSON.stringify({
        authToken: "den-token",
        baseUrl: "https://app.openrind-desktoplabs.com",
        userId: "user-1",
      }),
      desktopDataDir,
      orchestratorDataDir,
  ...
  expect(first.repositories.cloudSignin.getPrimary()?.cloudBaseUrl).toBe("https://app.openrind-desktoplabs.com");
  first.close();
...
test("deleted legacy-imported workspace stays deleted after restart", () => {
  const rootDir = makeTempDir("openrind-desktop-server-v2-phase2-delete-persist");
  const desktopDataDir = makeTempDir("openrind-desktop-server-v2-phase2-delete-desktop");
  const localWorkspaceDir = makeTempDir("openrind-desktop-server-v2-phase2-delete-workspace");
...
test("corrupt legacy workspace state is surfaced without blocking bootstrap", () => {
  const rootDir = makeTempDir("openrind-desktop-server-v2-phase2-corrupt");
  const desktopDataDir = makeTempDir("openrind-desktop-server-v2-phase2-corrupt-desktop");
  const orchestratorDataDir = makeTempDir("openrind-desktop-server-v2-phase2-corrupt-orchestrator");
  fs.writeFileSync(path.join(desktopDataDir, "openrind-desktop-workspaces.json"), "{not-json");
