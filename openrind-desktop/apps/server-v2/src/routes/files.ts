...
function readActorKey(c: Context<AppBindings>) {
  ...
  const hostToken = c.req.raw.headers.get("x-openrinddesktop-host-token")?.trim() ?? "";
  return {
...
export function registerFileRoutes(app: Hono<AppBindings>) {
  ...
  app.patch(
    ...
    describeRoute({
      ...
      description: "Updates stored workspace Openrind Desktop/OpenCode config, absorbs recognized managed sections into the database, and rematerializes the effective config files.",
      responses: withCommonErrorResponses({
    ...
    async (c) => {
      ...
      if (body.openrindDesktop) {
        requestContext.services.files.emitReloadEvent(workspaceId, "config", {
          action: "updated",
          name: "openrind-desktop.json",
          path: snapshot.materialized.configOpenrindDesktopPath ?? undefined,
          type: "config",
        });
      }
      await requestContext.services.files.recordWorkspaceAudit(
        workspaceId,
        "config.patch",
        snapshot.materialized.configOpencodePath ?? snapshot.materialized.configOpenrindDesktopPath ?? workspaceId,
