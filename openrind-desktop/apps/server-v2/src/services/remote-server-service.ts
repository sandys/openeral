...
import { requestRemoteOpenrindDesktop } from "../adapters/remote-openrind-desktop.js";
...
export function createRemoteServerService(input: {
  ...
  function buildServerRecord(payload: {
    ...
    const auth: JsonObject = {
      ...(existing?.auth ?? {}),
      ...(payload.token?.trim() ? { openrindDesktopToken: payload.token.trim(), openrindDesktopClientToken: payload.token.trim() } : {}),
      ...(payload.hostToken?.trim() ? { openrindDesktopHostToken: payload.hostToken.trim() } : {}),
    };
  ...
  async function fetchRemoteWorkspaces(server: ServerRecord) {
    const response = await requestRemoteOpenrindDesktop<unknown>({
      path: "/workspaces",
      server,
      timeoutMs: 10_000,
    });
  ...
  function updateWorkspaceRuntime(workspace: WorkspaceRecord, details: Record<string, unknown>) {
    ...
    input.repositories.workspaceRuntimeState.upsert({
      backendKind: "remote_openrind-desktop",
      health: {
        ...(current?.health ?? {}),
        ...details,
      },
  ...
  return {
    async connect(inputValue: {
      ...
      if (discovered.length === 0) {
        throw new HTTPException(404, { message: "Remote Openrind Desktop server did not return any visible workspaces." });
      }
