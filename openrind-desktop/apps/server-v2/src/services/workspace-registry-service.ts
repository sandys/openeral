...
export type WorkspaceBackend = {
  ...
  remote: null | {
    ...
    hostUrl: string | null;
    remoteType: "openrind-desktop" | "opencode";
...
function readRemoteType(workspace: WorkspaceRecord): "openrind-desktop" | "opencode" {
  const explicit = workspace.notes?.remoteType;
  return explicit === "opencode" ? "opencode" : "openrind-desktop";
}

function readRemoteWorkspaceName(workspace: WorkspaceRecord) {
  const legacyDesktop = asJsonObject(workspace.notes?.legacyDesktop);
  const value = legacyDesktop?.openrindDesktopWorkspaceName;
  return typeof value === "string" && value.trim() ? value.trim() : null;
}
...
export function createWorkspaceRegistryService(input: {
  ...
  function resolveBackend(workspace: WorkspaceRecord): WorkspaceBackend {
    ...
    const backendKind = runtimeState?.backendKind ?? (workspace.kind === "remote" ? "remote_openrind-desktop" : "local_opencode");

    if (backendKind === "remote_openrind-desktop") {
