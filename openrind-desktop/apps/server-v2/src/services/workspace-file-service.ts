...
import { requestRemoteOpenrindDesktop, requestRemoteOpenrindDesktopRaw, resolveRemoteWorkspaceTarget } from "../adapters/remote-openrind-desktop.js";
...
function recordAuditEntry(workspaceId: string, workspaceRoot: string, action: string, target: string, summary: string) {
  const auditRoot = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim()
    ? path.join(process.env.OPENRIND_DESKTOP_DATA_DIR.trim(), "audit")
    : path.join(workspaceRoot, ".opencode", "openrind-desktop");
  fs.mkdirSync(auditRoot, { recursive: true });
  const filePath = process.env.OPENRIND_DESKTOP_DATA_DIR?.trim()
...
function initializeWorkspaceFiles(workspaceRoot: string) {
  ...
  fs.mkdirSync(path.join(workspaceRoot, ".opencode", "openrind-desktop"), { recursive: true });
}
  ...
  return {
    ...
    async disposeWorkspaceInstance(workspaceId: string) {
      ...
      if (workspace.kind === "remote") {
        ...
        await requestRemoteOpenrindDesktop<{ reloadedAt: number }>({
          method: "POST",
          path: remote.path,
    ...
    createWorkspaceFileSession(workspaceId: string, inputValue: { actorKey?: string; actorKind: "client" | "host"; ttlSeconds?: number; write?: boolean }) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{
          canWrite: boolean;
          createdAt: number;
          expiresAt: number;
          id: string;
    ...
    async downloadArtifact(workspaceId: string, artifactId: string) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        const response = await requestRemoteOpenrindDesktopRaw({
          path: remote.path,
          server: remote.server,
          timeoutMs: 30_000,
        });
    ...
    getReloadEvents(workspaceId: string, since?: number) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ cursor: number; items: ReloadEvent[] }>({
          path: remote.path,
          server: remote.server,
          timeoutMs: 10_000,
        });
    ...
    async listArtifacts(workspaceId: string) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ items: Array<Record<string, unknown>> }>({
          path: remote.path,
          server: remote.server,
          timeoutMs: 15_000,
        });
    ...
    listFileSessionEvents(workspaceId: string, sessionId: string, actorKey: string, actorKind: "client" | "host", since?: string | null) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ cursor: number; items: FileSessionEvent[] }>({
          path: remote.path,
          server: remote.server,
          timeoutMs: 10_000,
        });
    ...
    async readSimpleContent(workspaceId: string, relativePathInput: string) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ bytes: number; content: string; path: string; updatedAt: number; revision?: string }>({
          path: remote.path,
          server: remote.server,
          timeoutMs: 15_000,
        });
    ...
    async reloadWorkspaceEngine(workspaceId: string) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ reloadedAt: number }>({
          method: "POST",
          path: remote.path,
          server: remote.server,
          timeoutMs: 20_000,
    ...
    closeWorkspaceFileSession(workspaceId: string, sessionId: string, actorKey: string, actorKind: "client" | "host") {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ closed?: boolean }>({
          method: "DELETE",
          path: remote.path,
          server: remote.server,
          timeoutMs: 10_000,
    ...
    async writeSimpleContent(workspaceId: string, inputValue: { baseUpdatedAt?: number | null; content: string; force?: boolean; path: string }) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ bytes: number; path: string; revision?: string; updatedAt: number }>({
          body: inputValue,
          method: "POST",
          path: remote.path,
          server: remote.server,
    ...
    async writeWorkspaceFiles(workspaceId: string, sessionId: string, actorKey: string, actorKind: "client" | "host", writes: unknown) {
      ...
      if (workspaceRecord.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ cursor: number; items: Array<Record<string, unknown>> }>({
          body: { writes },
          method: "POST",
          path: remote.path,
          server: remote.server,
