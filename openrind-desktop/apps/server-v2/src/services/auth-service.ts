...
export type AuthSummary = {
  ...
  headers: {
    ...
    hostToken: "X-OpenrindDesktop-Host-Token";
  };
  required: boolean;
  scopes: {
...
export function createAuthService() {
  const clientToken = trimToken(
    process.env.OPENRIND_DESKTOP_SERVER_V2_CLIENT_TOKEN
      ?? process.env.OPENRIND_DESKTOP_CLIENT_TOKEN
      ?? process.env.OPENRIND_DESKTOP_TOKEN,
  );
  const hostToken = trimToken(
    process.env.OPENRIND_DESKTOP_SERVER_V2_HOST_TOKEN
  ...
  function resolveActor(headers: Headers): RequestActor {
    const hostHeader = headers.get("x-openrinddesktop-host-token")?.trim() ?? "";
