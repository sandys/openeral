...
import { requestRemoteOpenrindDesktop, resolveRemoteWorkspaceTarget } from "../adapters/remote-openrind-desktop.js";

const MANAGED_SKILL_DOMAIN = "openrind-desktop-managed";
const OPENRIND_DESKTOP_CONFIG_VERSION = 1;

type WorkspaceConfigSnapshot = {
  effective: {
    opencode: JsonObject;
    openrindDesktop: JsonObject;
  };
  materialized: {
    compatibilityOpencodePath: string | null;
    compatibilityOpenrindDesktopPath: string | null;
    configDir: string | null;
    configOpencodePath: string | null;
    configOpenrindDesktopPath: string | null;
  };
  stored: {
    opencode: JsonObject;
    openrindDesktop: JsonObject;
...
function canonicalizeWorkspaceConfigState(workspace: WorkspaceRecord, config: { openrindDesktop: JsonObject; opencode: JsonObject }) {
  const nextOpenrindDesktop = asObject(config.openrindDesktop);
  nextOpenrindDesktop.authorizedRoots = withoutWorkspaceRoot(normalizeStringArray(nextOpenrindDesktop.authorizedRoots), workspace);
  ...
  return {
    openrindDesktop: nextOpenrindDesktop,
...
export function createConfigMaterializationService(input: {
  ...
  function workspaceOpenrindDesktopConfigPath(workspace: WorkspaceRecord) {
    const configDir = workspace.configDir?.trim();
    if (!configDir) {
      throw new RouteError(500, "internal_error", `Workspace ${workspace.id} is missing its config directory.`);
    }
    return path.join(configDir, ".opencode", "openrind-desktop.json");
  }
  ...
  function compatibilityOpenrindDesktopConfigPath(workspace: WorkspaceRecord) {
    const dataDir = workspace.dataDir?.trim();
    return dataDir ? path.join(dataDir, ".opencode", "openrind-desktop.json") : null;
  }
  ...
  function buildDefaultOpenrindDesktop(workspace: WorkspaceRecord) {
    return {
      authorizedRoots: [],
      blueprint: null,
      reload: null,
      version: OPENRIND_DESKTOP_CONFIG_VERSION,
      workspace: {
  ...
  function readLegacyWorkspaceState(workspace: WorkspaceRecord) {
    const openrindDesktop =
      readJsonFile(workspaceOpenrindDesktopConfigPath(workspace))
      ?? (compatibilityOpenrindDesktopConfigPath(workspace) ? readJsonFile(compatibilityOpenrindDesktopConfigPath(workspace)!) : null)
      ?? buildDefaultOpenrindDesktop(workspace);
    const opencode =
    ...
    return {
      openrindDesktop: asObject(openrindDesktop),
  ...
  function ensureWorkspaceConfigState(workspace: WorkspaceRecord) {
    ...
    return input.repositories.workspaceConfigState.upsert({
      openrindDesktop: canonical.openrindDesktop,
  ...
  function absorbWorkspaceConfigState(workspace: WorkspaceRecord) {
    ...
    const canonical = canonicalizeWorkspaceConfigState(workspace, {
      openrindDesktop: mergeObjects(buildDefaultOpenrindDesktop(workspace), legacy.openrindDesktop),
  ...
  function computeSnapshot(workspace: WorkspaceRecord): WorkspaceConfigSnapshot {
    ...
    const canonicalState = canonicalizeWorkspaceConfigState(workspace, {
      openrindDesktop: workspaceState.openrindDesktop,
      opencode: workspaceState.opencode,
    });
    const storedOpenrindDesktop = mergeObjects(buildDefaultOpenrindDesktop(workspace), canonicalState.openrindDesktop);
    const storedOpencode = mergeObjects(buildDefaultOpencode(), canonicalState.opencode);
    const effectiveOpenrindDesktop = mergeObjects(buildDefaultOpenrindDesktop(workspace), storedOpenrindDesktop);
    ...
    const authorizedRoots = withoutWorkspaceRoot(normalizeStringArray([
      ...normalizeStringArray(effectiveOpenrindDesktop.authorizedRoots),
    ...
    effectiveOpenrindDesktop.authorizedRoots = authorizedRoots;

    return {
      effective: {
        opencode: effectiveOpencode,
        openrindDesktop: effectiveOpenrindDesktop,
      },
      materialized: {
        compatibilityOpencodePath: compatibilityOpencodeConfigPath(workspace),
        compatibilityOpenrindDesktopPath: compatibilityOpenrindDesktopConfigPath(workspace),
        configDir: workspace.configDir,
        configOpencodePath: workspaceOpencodeConfigPath(workspace),
        configOpenrindDesktopPath: workspaceOpenrindDesktopConfigPath(workspace),
      },
      stored: {
        opencode: storedOpencode,
        openrindDesktop: storedOpenrindDesktop,
      },
  ...
  function materializeWorkspaceSnapshot(workspaceId: string) {
    ...
    writeJsonFile(snapshot.materialized.configOpenrindDesktopPath!, snapshot.effective.openrindDesktop);
    ...
    if (snapshot.materialized.compatibilityOpenrindDesktopPath) {
      writeJsonFile(snapshot.materialized.compatibilityOpenrindDesktopPath, snapshot.effective.openrindDesktop);
    }
  ...
  return {
    ...
    async getWorkspaceConfigSnapshot(workspaceId: string) {
      ...
      if (workspace.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<WorkspaceConfigSnapshot>({
    ...
    async patchWorkspaceConfig(workspaceId: string, patch: { openrindDesktop?: JsonObject; opencode?: JsonObject }) {
      const workspace = getWorkspaceOrThrow(workspaceId);
      if (workspace.kind === "remote") {
        const server = getRemoteServerOrThrow(workspace);
        const target = resolveRemoteWorkspaceTarget(server, workspace);
      ...
      const nextOpenrindDesktop = patch.openrindDesktop ? mergeObjects(current.openrindDesktop, asObject(patch.openrindDesktop)) : current.openrindDesktop;
    ...
    async readRawOpencodeConfig(workspaceId: string, scope: "global" | "project") {
      ...
      if (workspace.kind === "remote") {
        ...
        return requestRemoteOpenrindDesktop<{ content: string; exists: boolean; path: string | null; updatedAt: string }>({
    ...
    async writeWorkspaceRawOpencodeConfig(workspaceId: string, content: string) {
      ...
      upsertManagedRecords(workspace.id, "providerConfigs", recognized.providers);
      const canonical = canonicalizeWorkspaceConfigState(workspace, {
        openrindDesktop: ensureWorkspaceConfigState(workspace).openrindDesktop,
        opencode: recognized.base,
      });
      input.repositories.workspaceConfigState.upsert({
        openrindDesktop: canonical.openrindDesktop,
