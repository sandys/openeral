...
export function startServer(options: StartServerOptions = {}): StartedServer {
  const host = options.host ?? process.env.OPENRIND_DESKTOP_SERVER_V2_HOST ?? "127.0.0.1";
  const port = resolvePort(options.port ?? Number.parseInt(process.env.OPENRIND_DESKTOP_SERVER_V2_PORT ?? "3100", 10));
  ...
  const dependencies = options.dependencies ?? createAppDependencies({
    localServer: {
      ...
      hostingKind: process.env.OPENRIND_DESKTOP_SERVER_V2_HOSTING_KIND === "desktop" ? "desktop" : "self_hosted",
  ...
  if (!options.silent) {
    console.info(
      JSON.stringify({
        ...
        scope: "openrind-desktop-server-v2.start",
        url,
      }),
    );
  }
