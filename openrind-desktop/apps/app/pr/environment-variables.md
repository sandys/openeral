...
Openrind Desktop from a terminal, which:
...
in once, and every child Openrind Desktop spawns — OpenCode, the Openrind Desktop server,
...
| OS | Path |
| --- | --- |
| Linux / macOS | `~/.config/openrind-desktop/env.json` |
| Windows | `%APPDATA%\openrind-desktop\env.json` |

Override via `OPENRIND_DESKTOP_ENV_STORE` (mirrors `OPENRIND_DESKTOP_TOKEN_STORE`). The file
...
pattern. Four desktop-host-token routes on the Openrind Desktop server, so remote
...
Reserved-keys policy: **any key starting with `OPENRIND_DESKTOP_` or `OPENCODE_`** is
...
an **Apply changes** action. Applying restarts the local Openrind Desktop runtime with
...
```
pnpm --filter openrind-desktop-server test                                    # 107 pass, 0 fail
bun test ./src/env-file.test.ts ./src/env-routes.e2e.test.ts          # 24 pass, 0 fail
pnpm --filter openrind-desktop-server typecheck                               # clean
pnpm --filter openrind-desktop-server build:bin                               # ok
pnpm --filter openrind-desktop-orchestrator typecheck                         # clean
pnpm --filter @openrind/app typecheck                                 # clean
