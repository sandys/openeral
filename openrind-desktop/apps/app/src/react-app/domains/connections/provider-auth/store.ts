...
export function createProviderAuthStore(options: CreateProviderAuthStoreOptions) {
  ...
  const readWorkspaceOpenrindDesktopConfigRecord = async (): Promise<
  ...
  > => {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopCapabilities?.config?.read;
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      return (await workspaceOpenrindDesktopRead({
        workspacePath: root,
  ...
  const writeWorkspaceOpenrindDesktopConfigRecord = async (
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopClient &&
      openrindDesktopWorkspaceId &&
      openrindDesktopCapabilities?.config?.write;
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      ...
      if (!result.ok) {
        throw new Error(
          result.stderr || result.stdout || "Failed to write .opencode/openrind-desktop.json",
        );
      }
      return true;
    }
  ...
  const persistImportedCloudProviders = async (
    ...
    if (!persisted) {
      throw new Error(
        "Openrind Desktop server unavailable. Connect to manage imported cloud providers.",
      );
    }
  ...
  const readProjectConfigFile = async () => {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopWorkspaceId &&
      openrindDesktopCapabilities?.config?.read &&
      typeof openrindDesktopClient.readOpencodeConfigFile === "function";

    if (canUseOpenrindDesktopServer) {
      return await openrindDesktopClient.readOpencodeConfigFile(openrindDesktopWorkspaceId, "project");
    }
  ...
  const writeProjectConfigFile = async (content: string) => {
    ...
    const canUseOpenrindDesktopServer =
      ...
      openrindDesktopWorkspaceId &&
      openrindDesktopCapabilities?.config?.write &&
      typeof openrindDesktopClient.writeOpencodeConfigFile === "function";

    if (canUseOpenrindDesktopServer) {
      const result = await openrindDesktopClient.writeOpencodeConfigFile(
        openrindDesktopWorkspaceId,
        "project",
        content,
      );
  ...
  const cloudProviderComment = (provider: Pick<DenOrgLlmProvider, "id" | "name">) =>
    `// Openrind Desktop Cloud import: ${provider.name
      .replace(/\s+/g, " ")
  ...
  async function connectCloudProviderInternal(
    ...
    const orgId = settings.activeOrgId?.trim() ?? "";
    if (!token || !orgId) {
      throw new Error("Sign in to Openrind Desktop Cloud and choose an organization first.");
    }
