...
import type {
  ...
  OpenrindDesktopSessionSnapshot,
...
function transcriptToText(messages: UIMessage[]) {
  return messages
    .map((message) => {
      const header = message.role === "user" ? "You" : message.role === "assistant" ? "Openrind Desktop" : message.role;
...
function statusLabel(snapshot: OpenrindDesktopSessionSnapshot | undefined, busy: boolean) {
...
export function SessionSurface(props: SessionSurfaceProps) {
  ...
  const opencodeClient = useMemo(
    () => createClient(props.opencodeBaseUrl, undefined, { token: props.openrindDesktopToken, mode: "openrind-desktop" }),
    [props.opencodeBaseUrl, props.openrindDesktopToken],
  );
  ...
  const snapshotQuery = useQuery<OpenrindDesktopSessionSnapshot>({
  ...
  const listImportedPlugins = async (): Promise<CloudImportedPlugin[]> => {
    ...
    const plugins = Object.values(readWorkspaceCloudImports(response.openrindDesktop).plugins)
