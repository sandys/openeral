...
export type SessionPageSurfaceProps = Omit<
  SessionSurfaceProps,
  "client" | "workspaceId" | "sessionId" | "opencodeBaseUrl" | "openrindDesktopToken"
>;
...
export type SessionPageProps = {
   ...
   *  Used by the Openrind Shell terminal flow to inject xterm.js without
  ...
  openrindDesktopServerToken?: string | null;
  developerMode: boolean;
  headerStatus: string;
   ...
   *  Used by the Openrind Shell flow to inject a "Switch to Terminal" button
...
export function SessionPage(props: SessionPageProps) {
  ...
  const reactSessionBaseUrl = useMemo(() => {
    ...
    const baseUrl = props.openrindDesktopServerClient?.baseUrl?.trim() ?? "";
    if (!workspaceId || !baseUrl) return "";
    const mounted = buildOpenrindDesktopWorkspaceBaseUrl(baseUrl, workspaceId) ?? baseUrl;
    return `${mounted.replace(/\/+$/, "")}/opencode`;
  }, [props.openrindDesktopServerClient?.baseUrl, props.runtimeWorkspaceId]);

  const reactSessionToken = props.openrindDesktopServerClient?.token?.trim() || props.openrindDesktopServerToken?.trim() || "";
  const canRenderReactSurface = Boolean(
    props.selectedSessionId &&
      props.runtimeWorkspaceId &&
      props.openrindDesktopServerClient &&
      reactSessionBaseUrl &&
      reactSessionToken &&
      props.surface,
  );
    ...
    <div className="flex h-full min-h-0 flex-col bg-[radial-gradient(circle_at_top,rgba(74,111,255,0.12),transparent_42%),var(--app-bg,#0b1020)] text-dls-text">
      <div className="flex min-h-0 flex-1 gap-4 p-3 md:p-4">
        ...
        <main className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[24px] border border-dls-border bg-dls-surface shadow-[var(--dls-shell-shadow)]">
          {/* The chat header is hidden while a surface override (the Openrind Shell
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden">
            <div className="relative min-w-0 flex-1 overflow-hidden bg-dls-surface">
              ...
              {props.sessionSurfaceOverride ? (
                ...
                <SessionSurface
                  client={props.openrindDesktopServerClient!}
                  workspaceId={props.runtimeWorkspaceId!}
                  sessionId={props.selectedSessionId!}
                  opencodeBaseUrl={reactSessionBaseUrl}
                  openrindDesktopToken={reactSessionToken}
                  {...props.surface!}
                />
              ) : null}
