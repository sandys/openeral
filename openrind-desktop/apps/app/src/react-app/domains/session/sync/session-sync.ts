...
function syncKey(input: SyncOptions) {
  return `${input.workspaceId}:${input.baseUrl}:${input.openrindDesktopToken}`;
}
...
function startSync(input: SyncOptions) {
  const client = createClient(input.baseUrl, undefined, { token: input.openrindDesktopToken, mode: "openrind-desktop" });
...
export function seedSessionState(workspaceId: string, snapshot: OpenrindDesktopSessionSnapshot) {
  const queryClient = getReactQueryClient();
