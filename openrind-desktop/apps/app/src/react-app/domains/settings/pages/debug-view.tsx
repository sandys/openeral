...
type RuntimeSummary = {
  ...
  openrindDesktopServerVersionLabel: string;
};
...
export type DebugViewProps = {
  ...
  openrindDesktopServerRestarting: boolean;
  opencodeServiceStatus: ServiceStatus;
  openrindDesktopServiceStatus: ServiceStatus;
  opencodeLogStatus: string | null;
  openrindDesktopLogStatus: string | null;
  onCopyOpencodeLogs: () => void | Promise<void>;
  onExportOpencodeLogs: () => void | Promise<void>;
  onCopyOpenrindDesktopLogs: () => void | Promise<void>;
  onExportOpenrindDesktopLogs: () => void | Promise<void>;
  serviceRestartError: string | null;
  onRestartOpencode: () => void | Promise<void>;
  onRestartOpenrindDesktopServer: () => void | Promise<void>;
  engineCard: RuntimeServiceCard;
  opencodeConnectCard: OpenCodeConnectDebugCard;
  openrindDesktopCard: RuntimeServiceCard;
  ...
  openrindDesktopAuditStatus: StatusPill;
  openrindDesktopAuditError: string | null;
  opencodeConnectStatus: OpencodeConnectStatus | null;
  opencodeDevModeEnabled: boolean;
  nukeConfigBusy: boolean;
  nukeConfigStatus: string | null;
  onNukeOpenrindDesktopAndOpencodeConfig: () => void | Promise<void>;
};

function formatActor(entry: OpenrindDesktopAuditEntry) {
    ...
    <section className="space-y-6">
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
          ...
          <div>
            {t("settings.debug_openrind_desktop_server_version", undefined, {
              version: props.runtimeSummary.openrindDesktopServerVersionLabel,
            })}
          </div>
        </div>
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <ServiceCard
            ...
            description={t("settings.openrind_desktop_config_sidecar_desc")}
            pill={props.openrindDesktopCard}
            lines={props.openrindDesktopCard.lines}
            stdout={props.openrindDesktopCard.stdout ?? null}
            stderr={props.openrindDesktopCard.stderr ?? null}
            error={props.openrindDesktopCard.error ?? null}
            restarting={props.openrindDesktopServerRestarting}
            restartLabel={t("settings.restart_openrind_desktop_server")}
            onRestart={props.onRestartOpenrindDesktopServer}
            serviceStatus={props.openrindDesktopServiceStatus}
            logStatus={props.openrindDesktopLogStatus}
            onCopyLogs={props.onCopyOpenrindDesktopLogs}
            onExportLogs={props.onExportOpenrindDesktopLogs}
            isDesktop={isDesktop}
          />
      ...
      <div className={cardClass}>
        <div className={sectionHeaderClass}>
          <div className={sectionTitleClass}>{t("settings.openrind_desktop_diagnostics_title")}</div>
          <div className={sectionDescClass}>
            <span className="font-mono text-[11px] text-dls-secondary">
              {props.openrindDesktopServerDiagnostics?.version ?? "—"}
            </span>
          </div>
        </div>

        {props.openrindDesktopServerDiagnostics ? (
          <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
            <div>{t("settings.diag_started", undefined, { time: formatUptime(props.openrindDesktopServerDiagnostics.uptimeMs) })}</div>
            <div>
              {t("settings.diag_read_only", undefined, {
                value: props.openrindDesktopServerDiagnostics.readOnly ? "true" : "false",
              })}
            </div>
            <div>
              {t("settings.diag_approval", undefined, {
                mode: props.openrindDesktopServerDiagnostics.approval.mode,
                ms: String(props.openrindDesktopServerDiagnostics.approval.timeoutMs),
              })}
            </div>
            <div>{t("settings.diag_workspaces", undefined, { count: String(props.openrindDesktopServerDiagnostics.workspaceCount) })}</div>
            <div>
              {t("settings.diag_selected_workspace", undefined, {
                id: props.openrindDesktopServerDiagnostics.selectedWorkspaceId ?? "—",
              })}
            </div>
            <div>
              {t("settings.diag_runtime_workspace", undefined, {
                id: props.openrindDesktopServerDiagnostics.activeWorkspaceId ?? "—",
              })}
            </div>
            <div>
              {t("settings.diag_config_path", undefined, {
                path: props.openrindDesktopServerDiagnostics.server.configPath ?? t("settings.diag_default"),
              })}
            </div>
            <div>
              {t("settings.diag_token_source", undefined, {
                source: props.openrindDesktopServerDiagnostics.tokenSource.client,
              })}
            </div>
            <div>
              {t("settings.diag_host_token_source", undefined, {
                source: props.openrindDesktopServerDiagnostics.tokenSource.host,
              })}
            </div>
          </div>
        ) : (
        ...
        <div className={subCardClass}>
          ...
          {props.openrindDesktopServerCapabilities ? (
            <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
              <div>{t("settings.cap_skills", undefined, { value: formatCapability(props.openrindDesktopServerCapabilities.skills) })}</div>
              <div>{t("settings.cap_plugins", undefined, { value: formatCapability(props.openrindDesktopServerCapabilities.plugins) })}</div>
              <div>{t("settings.cap_mcp", undefined, { value: formatCapability(props.openrindDesktopServerCapabilities.mcp) })}</div>
              <div>{t("settings.cap_commands", undefined, { value: formatCapability(props.openrindDesktopServerCapabilities.commands) })}</div>
              <div>{t("settings.cap_config", undefined, { value: formatCapability(props.openrindDesktopServerCapabilities.config) })}</div>
              <div>
                {t("settings.cap_browser_tools", undefined, {
                  value: (() => {
                    const browser = props.openrindDesktopServerCapabilities.toolProviders?.browser;
              ...
              <div>
                {t("settings.cap_file_tools", undefined, {
                  value: (() => {
                    const files = props.openrindDesktopServerCapabilities.toolProviders?.files;
              ...
              <div>
                {t("settings.cap_sandbox", undefined, {
                  value: props.openrindDesktopServerCapabilities.sandbox
                    ? `${props.openrindDesktopServerCapabilities.sandbox.backend} (${props.openrindDesktopServerCapabilities.sandbox.enabled ? t("settings.on") : t("settings.off")})`
      ...
      <div className={cardClass}>
        ...
        <div className={subCardClass}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className={`rounded-full border px-2 py-1 text-[11px] font-medium ${props.openrindDesktopAuditStatus.className}`}>
              {props.openrindDesktopAuditStatus.label}
            </div>
          </div>
          {props.openrindDesktopAuditError ? <StatusBanner tone="error" message={props.openrindDesktopAuditError} /> : null}
          {props.openrindDesktopAuditEntries.length > 0 ? (
            <div className="divide-y divide-dls-border/60">
              {props.openrindDesktopAuditEntries.map((entry) => (
      ...
      {props.electronMigrationAvailable ? (
        <div className={cardClass}>
          ...
          <div className="rounded-xl border border-green-7/25 bg-green-3/10 px-3 py-2 text-[12px] leading-relaxed text-green-11">
            ...
            <code className="font-mono">Openrind Desktop.app.migrate-bak</code>.
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              title="Requires a trusted artifact URL. macOS keeps Openrind Desktop.app.migrate-bak for rollback."
            >
      ...
      {isDesktop ? (
        <div className="space-y-3 rounded-2xl border border-red-7/30 bg-red-3/10 p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm font-semibold tracking-[-0.1px] text-dls-text">
                {t("settings.reset_openrind_desktop_title")}
              </div>
              <div className="text-[12px] text-dls-secondary">
                {props.opencodeDevModeEnabled
                  ? t("settings.reset_openrind_desktop_desc_dev")
                  : t("settings.reset_openrind_desktop_desc_prod")}
              </div>
            </div>
            <div
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={() => void props.onNukeOpenrindDesktopAndOpencodeConfig()}
