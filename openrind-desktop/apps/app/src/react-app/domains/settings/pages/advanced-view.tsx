...
export function AdvancedView(props: AdvancedViewProps) {
  const [openrindDesktopReconnectStatus, setOpenrindDesktopReconnectStatus] = useState<string | null>(null);
  const [openrindDesktopReconnectError, setOpenrindDesktopReconnectError] = useState<string | null>(null);
  const [openrindDesktopRestartBusy, setOpenrindDesktopRestartBusy] = useState(false);
  const [openrindDesktopRestartStatus, setOpenrindDesktopRestartStatus] = useState<string | null>(null);
  const [openrindDesktopRestartError, setOpenrindDesktopRestartError] = useState<string | null>(null);
  ...
  const handleReconnectOpenrindDesktopServer = async () => {
    if (props.busy || props.openrindDesktopReconnectBusy || !props.openrindDesktopServerUrl.trim()) return;
    setOpenrindDesktopReconnectStatus(null);
    setOpenrindDesktopReconnectError(null);
    try {
      const ok = await props.reconnectOpenrindDesktopServer();
      if (!ok) {
        setOpenrindDesktopReconnectError(t("settings.reconnect_failed"));
        return;
      }
      setOpenrindDesktopReconnectStatus(t("settings.reconnected"));
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      setOpenrindDesktopReconnectError(message || t("settings.reconnect_server_failed"));
    }
  };

  const handleRestartLocalServer = async () => {
    if (props.busy || openrindDesktopRestartBusy) return;
    setOpenrindDesktopRestartStatus(null);
    setOpenrindDesktopRestartError(null);
    setOpenrindDesktopRestartBusy(true);
    try {
      const ok = await props.restartLocalServer();
      if (!ok) {
        setOpenrindDesktopRestartError(t("settings.restart_failed"));
        return;
      }
      setOpenrindDesktopRestartStatus(t("settings.restarted"));
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      setOpenrindDesktopRestartError(message || t("settings.restart_server_failed"));
    } finally {
      setOpenrindDesktopRestartBusy(false);
    }
  };
      ...
      <div className={`${settingsPanelClass} space-y-3`}>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          <button
            type="button"
            className="inline-flex items-center gap-1.5 rounded-md border border-dls-border bg-dls-surface px-3 py-1.5 text-xs font-medium text-dls-secondary shadow-sm transition-colors duration-150 hover:bg-dls-hover hover:text-dls-text focus:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(var(--dls-accent-rgb),0.25)] disabled:cursor-not-allowed disabled:opacity-60"
            onClick={() => void handleReconnectOpenrindDesktopServer()}
          ...
          {isLocalEngineRunning ? (
            <button
              ...
              disabled={props.busy || openrindDesktopRestartBusy}
            >
              <RefreshCcw size={14} className={`text-dls-secondary ${openrindDesktopRestartBusy ? "animate-spin" : ""}`} />
              {openrindDesktopRestartBusy ? t("settings.restarting") : t("settings.restart_openrind_desktop_server")}
            </button>
          ) : null}
          ...
          {!isLocalEngineRunning && props.openrindDesktopServerStatus === "connected" ? (
            <button
              type="button"
              className="inline-flex items-center gap-1.5 rounded-md border border-dls-border bg-dls-surface px-3 py-1.5 text-xs font-medium text-dls-secondary shadow-sm transition-colors duration-150 hover:bg-dls-hover hover:text-dls-text focus:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(var(--dls-accent-rgb),0.25)] disabled:cursor-not-allowed disabled:opacity-60"
              onClick={props.stopHost}
        ...
        {openrindDesktopReconnectStatus ? <div className="text-xs text-gray-10">{openrindDesktopReconnectStatus}</div> : null}
        {openrindDesktopReconnectError ? <div className="text-xs text-red-11">{openrindDesktopReconnectError}</div> : null}
        {openrindDesktopRestartStatus ? <div className="text-xs text-gray-10">{openrindDesktopRestartStatus}</div> : null}
        {openrindDesktopRestartError ? <div className="text-xs text-red-11">{openrindDesktopRestartError}</div> : null}
      </div>
