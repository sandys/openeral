...
export function AuthorizedFoldersPanel(props: AuthorizedFoldersPanelProps) {
  ...
  const openrindDesktopServerReady = props.openrindDesktopServerStatus === "connected";
  const openrindDesktopServerWorkspaceReady = Boolean(props.runtimeWorkspaceId);
  const canReadConfig =
    openrindDesktopServerReady &&
    openrindDesktopServerWorkspaceReady &&
    (props.openrindDesktopServerCapabilities?.config?.read ?? false);
  const canWriteConfig =
    openrindDesktopServerReady &&
    openrindDesktopServerWorkspaceReady &&
    (props.openrindDesktopServerCapabilities?.config?.write ?? false);

  const authorizedFoldersHint = useMemo(() => {
    if (!openrindDesktopServerReady) return t("context_panel.server_disconnected");
    if (!openrindDesktopServerWorkspaceReady) return t("context_panel.no_server_workspace");
    ...
    return null;
  }, [canReadConfig, canWriteConfig, openrindDesktopServerReady, openrindDesktopServerWorkspaceReady]);
  ...
  useEffect(() => {
    const openrindDesktopClient = props.openrindDesktopServerClient;
    const openrindDesktopWorkspaceId = props.runtimeWorkspaceId;

    if (!openrindDesktopClient || !openrindDesktopWorkspaceId || !canReadConfig) {
  ...
  }, [canReadConfig, props.openrindDesktopServerClient, props.runtimeWorkspaceId]);

  const persistAuthorizedFolders = useCallback(async (nextFolders: string[]) => {
    const openrindDesktopClient = props.openrindDesktopServerClient;
    const openrindDesktopWorkspaceId = props.runtimeWorkspaceId;
    if (!openrindDesktopClient || !openrindDesktopWorkspaceId || !canWriteConfig) {
    ...
    try {
      const currentConfig = await openrindDesktopClient.getConfig(openrindDesktopWorkspaceId);
      ...
      await openrindDesktopClient.patchConfig(openrindDesktopWorkspaceId, {
        opencode: {
          permission: {
