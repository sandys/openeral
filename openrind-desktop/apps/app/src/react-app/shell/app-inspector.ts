 ...
 *
 * Consumers read via `window.__openrindDesktop` from a devtools console or from
...
declare global {
  ...
  var __openrindDesktop: InspectorAPI | undefined;
}
...
function installIfNeeded() {
  ...
  Object.defineProperty(window, "__openrindDesktop", {
    value: api,
    configurable: true,
    writable: false,
  });
...
export function recordInspectorEvent(name: string, data?: unknown) {
  installIfNeeded();
  window.__openrindDesktop?.record(name, data);
}
