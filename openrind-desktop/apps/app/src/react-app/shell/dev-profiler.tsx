 ...
 * localStorage.openrind-desktop.debug.profilerOverlay = "1" / "0".
 *
 * In prod builds the wrapper is a pass-through (no Profiler overhead) and
 * the overlay renders null.
 *
 * Findings also land on window.__openrindDesktop.slice("profiler") so external
...
//
// Explicit opt-ins:
//   - VITE_OPENRIND_DESKTOP_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("openrind-desktop.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    ...
    const flag = env.VITE_OPENRIND_DESKTOP_PROFILER;
    if (flag === "1" || flag === "true" || flag === true) return true;
  } catch {
    // ignore
  }
  try {
    if (window.localStorage.getItem("openrind-desktop.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
// window.__openrindDesktop.slice("profiler") — even for operators who aren't
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("openrind-desktop.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("openrind-desktop.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
