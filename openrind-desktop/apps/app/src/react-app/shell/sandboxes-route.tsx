...
async function invoke<T>(command: string, ...args: unknown[]): Promise<T> {
  ...
  if (!bridge?.invokeDesktop) {
    throw new Error("Desktop bridge unavailable — Openrind Shell sandboxes need the Openrind Desktop desktop app.");
  }
...
// Mirror of deriveOpenrindShellSandboxName() in the main process so the user sees the
...
export function SandboxesRoute() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [list, status] = await Promise.all([
        // No openrindListSessions fallback: that handler returns PTY session
        // records (a different shape) and rendered as blank rows. A list
        // failure now surfaces as listError with the Refresh retry.
        invoke<SandboxRow[]>("openrindListSandboxes"),
        invoke<CredentialStatus>("openrindCredentialStatus").catch(() => null),
      ]);
  ...
  const openSandbox = (name: string, p: SandboxProfile) => {
    ...
    navigate("/session", {
      state: { openrindSandbox: { name, profile: p } },
    });
  };
  ...
  const confirmDelete = async () => {
    ...
    setBusyName(name);
    try {
      await invoke("openrindDeleteSandbox", name);
      await refresh();
    } catch (err) {
      setListError(err instanceof Error ? err.message : String(err));
    } finally {
      ...
      <main className="flex h-full w-full flex-col overflow-hidden rounded-[24px] border border-dls-border bg-dls-surface shadow-[var(--dls-shell-shadow)]">
        ...
        <header className="sticky top-0 z-10 flex h-12 shrink-0 items-center justify-between border-b border-dls-border bg-dls-surface px-4 md:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <h1 className="truncate text-[15px] font-semibold text-dls-text">Openrind Shell Sandboxes</h1>
