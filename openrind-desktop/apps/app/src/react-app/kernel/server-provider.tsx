...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("openrind-desktop.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("openrind-desktop.server.active");
...
function readOpenrindDesktopToken(): string {
  if (typeof window === "undefined") return "";
  try {
    return (window.localStorage.getItem("openrind-desktop.server.token") ?? "").trim();
  } catch {
    return "";
  }
}
...
async function checkHealth(url: string): Promise<boolean> {
  if (!url) return false;
  const token = readOpenrindDesktopToken();
  const headers =
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // Hosted web deployments served by Openrind Desktop must reuse the OpenCode proxy
    ...
    const forceProxy =
      ...
      (import.meta.env.PROD ||
        (typeof import.meta.env?.VITE_OPENRIND_DESKTOP_URL === "string" &&
          import.meta.env.VITE_OPENRIND_DESKTOP_URL.trim().length > 0));
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("openrind-desktop.server.list", JSON.stringify(list));
      window.localStorage.setItem("openrind-desktop.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
  ...
  useEffect(() => {
    ...
    if (isDesktopRuntime() && !active.includes("/opencode")) {
      // Desktop React routes now talk to Openrind Desktop server workspace-mounted
