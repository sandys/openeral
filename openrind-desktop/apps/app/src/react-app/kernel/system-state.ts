...
import type {
  ReloadReason,
  ReloadTrigger,
  ResetOpenrindDesktopMode,
} from "../../app/types";
import { relaunchDesktopApp, resetOpenrindDesktopState } from "../../app/lib/desktop";
import {
...
export type ResetState = {
  ...
  resetModalMode: ResetOpenrindDesktopMode;
...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetOpenrindDesktopMode) => void;
...
function clearOpenrindDesktopLocalStorage(mode: ResetOpenrindDesktopMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (key.includes("openrind-desktop")) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("openrind_desktop_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetOpenrindDesktopMode>("onboarding");
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetOpenrindDesktopState(resetModalMode);
      }
      clearOpenrindDesktopLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
