...
import type { OpenrindDesktopServerClient } from "../lib/openrind-desktop-server";
...
export function parseBundleDeepLink(rawUrl: string): BundleRequest | null {
  ...
  try {
    ...
    // openrind-desktop://import-bundle?ow_bundle=http://169.254.169.254/... would be
...
export async function fetchBundle(
  bundleUrl: string,
  serverClient?: OpenrindDesktopServerClient | null,
  ...
  if (!isConfiguredBundlePublisherUrl(targetUrl.toString())) {
    throw new Error(
      "Bundle URL is not from the configured Openrind Desktop publisher.",
    );
  }
