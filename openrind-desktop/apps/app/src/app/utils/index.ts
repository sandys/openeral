...
export function isElectronRuntime() {
  return typeof window !== "undefined" && (window as Window).__OPENRIND_DESKTOP_ELECTRON__ != null;
}
...
const STARTUP_PREF_KEY = "openrind-desktop.startupPref";
const LEGACY_PREF_KEY = "openrind-desktop.modePref";
const LEGACY_PREF_KEY_ALT = "openrind_desktop_mode_pref";
...
export function addOpencodeCacheHint(message: string) {
  ...
  if (cacheSignals.some((signal) => lower.includes(signal)) && lower.includes("enoent")) {
    return `${message}\n\nThe local engine cache looks corrupted. Restart Openrind Desktop to rebuild it.`;
  }
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.openrindDesktopHostUrl ?? ""}`.toLowerCase();
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(openrind-desktop|opencode)/i, // macOS app bundle identifiers
  /\.openrind-desktop\.dev\//i, // Openrind Desktop dev paths
