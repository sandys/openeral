const ENV_FEEDBACK_URL = String(import.meta.env.VITE_OPENRIND_DESKTOP_FEEDBACK_URL ?? "").trim();
const ENV_APP_VERSION = String(import.meta.env.VITE_OPENRIND_DESKTOP_APP_VERSION ?? "").trim();

export const DEFAULT_FEEDBACK_URL =
  ENV_FEEDBACK_URL || "https://openrind-desktoplabs.com/feedback";

type FeedbackUrlOptions = {
  entrypoint: string;
  deployment?: string | null;
  appVersion?: string | null;
  openrindDesktopServerVersion?: string | null;
...
export function buildFeedbackUrl(options: FeedbackUrlOptions): string {
  ...
  url.searchParams.set("source", "openrind-desktop-app");
  ...
  const entries = {
    ...
    openrindDesktopServerVersion: options.openrindDesktopServerVersion?.trim() ?? "",
