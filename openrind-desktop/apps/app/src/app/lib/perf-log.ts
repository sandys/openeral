...
type PerfRoot = typeof globalThis & {
  __openrindDesktopPerfSeq?: number;
  __openrindDesktopPerfLogs?: PerfLogRecord[];
  __openrindDesktopPerfConsoleAt?: Record<string, number>;
  __openrindDesktopPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__openrindDesktopPerfSeq ?? 0) + 1;
  root.__openrindDesktopPerfSeq = id;
  ...
  const logs = root.__openrindDesktopPerfLogs ?? [];
  logs.push(entry);
  if (logs.length > PERF_LOG_LIMIT) {
    logs.splice(0, logs.length - PERF_LOG_LIMIT);
  }
  root.__openrindDesktopPerfLogs = logs;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__openrindDesktopPerfConsoleAt ?? (root.__openrindDesktopPerfConsoleAt = {});
    const suppressedByKey =
      root.__openrindDesktopPerfConsoleSuppressed ?? (root.__openrindDesktopPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__openrindDesktopPerfLogs = [];
  root.__openrindDesktopPerfSeq = 0;
};
