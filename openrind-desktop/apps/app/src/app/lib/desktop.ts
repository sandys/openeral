...
function isElectronDesktopRuntime() {
  return typeof window !== "undefined" && window.__OPENRIND_DESKTOP_ELECTRON__ != null;
}
...
async function invokeElectronHelper<T>(
  ...
  const invokeDesktop = window.__OPENRIND_DESKTOP_ELECTRON__?.invokeDesktop;
  if (!invokeDesktop) {
...
function resolveElectronBridge(): DesktopBridge {
  const exposed = window.__OPENRIND_DESKTOP_ELECTRON__?.bridge ?? {};
  const invokeDesktop = window.__OPENRIND_DESKTOP_ELECTRON__?.invokeDesktop;
...
function resolveDesktopBridge(): DesktopBridge {
  if (
    ...
    (window.__OPENRIND_DESKTOP_ELECTRON__?.bridge ||
...
function isLoopbackUrl(input: RequestInfo | URL): boolean {
  ...
  try {
    ...
    // Aligns with isLoopbackOpenrindDesktopServerUrl. Getting this wrong routes a
...
export async function openDesktopUrl(url: string): Promise<void> {
  if (isElectronDesktopRuntime()) {
    await window.__OPENRIND_DESKTOP_ELECTRON__?.shell?.openExternal?.(url);
    return;
  }
...
export async function relaunchDesktopApp(): Promise<void> {
  if (isElectronDesktopRuntime()) {
    await window.__OPENRIND_DESKTOP_ELECTRON__?.shell?.relaunch?.();
    return;
  }
...
export async function subscribeDesktopDeepLinks(
  ...
  if (isElectronDesktopRuntime()) {
    ...
    const initialUrls = window.__OPENRIND_DESKTOP_ELECTRON__?.meta?.initialDeepLinks;
