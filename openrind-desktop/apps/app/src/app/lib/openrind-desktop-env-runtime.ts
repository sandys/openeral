const PENDING_CHANGES_KEY = "openrind-desktop.settings.environment.pendingChanges";
...
export function buildOpenrindDesktopEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "openrind-desktop"}::${runtime || "runtime"}`;
}

export function readOpenrindDesktopEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeOpenrindDesktopEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
