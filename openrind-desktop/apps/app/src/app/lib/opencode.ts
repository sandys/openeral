...
import { createOpenrindDesktopServerClient, OpenrindDesktopServerError } from "./openrind-desktop-server";
...
function resolveOpenrindDesktopWorkspaceMount(baseUrl: string): { baseUrl: string; workspaceId: string } | null {
  try {
...
async function wrapOpenrindDesktopRead<T>(
  url: string,
  read: () => Promise<T>,
  ...
  try {
    ...
    return createSyntheticResult(url, "GET", {
      ok: false,
      error,
      status: error instanceof OpenrindDesktopServerError ? error.status : 500,
    });
  }
}

function shouldFallbackToLegacySessionRead(error: unknown): boolean {
  if (!(error instanceof OpenrindDesktopServerError)) return false;
  return error.status === 404 || error.status === 405 || error.status === 501;
}

async function wrapOpenrindDesktopReadWithFallback<T>(
  url: string,
  read: () => Promise<T>,
...
const resolveAuthHeader = (auth?: OpencodeAuth) => {
  if (auth?.mode === "openrind-desktop" && auth.token) {
 ...
 * CORS is already wide open on the openrind-desktop/opencode stack, so there's no
...
export function createClient(baseUrl: string, directory?: string, auth?: OpencodeAuth) {
  ...
  const openrindDesktopMount = auth?.mode === "openrind-desktop" ? resolveOpenrindDesktopWorkspaceMount(baseUrl) : null;
  const openrindDesktopSessionClient =
    openrindDesktopMount && auth?.token
      ? createOpenrindDesktopServerClient({ baseUrl: openrindDesktopMount.baseUrl, token: auth.token })
      : null;
  // TODO(2026-04-12): remove the old-server compatibility path here once all
  // Openrind Desktop servers expose the workspace-scoped session read APIs.
  ...
  sessionOverrides.list = (parameters?: SessionListParameters, options?: { throwOnError?: boolean }) => {
    if (!openrindDesktopMount || !openrindDesktopSessionClient) {
    ...
    const url = `${openrindDesktopMount.baseUrl}/workspace/${encodeURIComponent(openrindDesktopMount.workspaceId)}/sessions${query.size ? `?${query.toString()}` : ""}`;
    return wrapOpenrindDesktopReadWithFallback(
      url,
      async () => (await openrindDesktopSessionClient.listSessions(openrindDesktopMount.workspaceId, parameters)).items,
  ...
  sessionOverrides.get = (parameters: SessionLookupParameters, options?: { throwOnError?: boolean }) => {
    ...
    const url = `${openrindDesktopMount.baseUrl}/workspace/${encodeURIComponent(openrindDesktopMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}`;
    return wrapOpenrindDesktopReadWithFallback(
      url,
      async () => (await openrindDesktopSessionClient.getSession(openrindDesktopMount.workspaceId, parameters.sessionID)).item,
  ...
  sessionOverrides.messages = (parameters: SessionMessagesParameters, options?: { throwOnError?: boolean }) => {
    ...
    const query = new URLSearchParams();
    if (typeof parameters.limit === "number") query.set("limit", String(parameters.limit));
    const url = `${openrindDesktopMount.baseUrl}/workspace/${encodeURIComponent(openrindDesktopMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}/messages${query.size ? `?${query.toString()}` : ""}`;
    return wrapOpenrindDesktopReadWithFallback(
      url,
      async () =>
        (await openrindDesktopSessionClient.getSessionMessages(openrindDesktopMount.workspaceId, parameters.sessionID, {
  ...
  sessionOverrides.todo = (parameters: SessionLookupParameters, options?: { throwOnError?: boolean }) => {
    ...
    const url = `${openrindDesktopMount.baseUrl}/workspace/${encodeURIComponent(openrindDesktopMount.workspaceId)}/sessions/${encodeURIComponent(parameters.sessionID)}/snapshot`;
    return wrapOpenrindDesktopReadWithFallback(
      url,
      async () => (await openrindDesktopSessionClient.getSessionSnapshot(openrindDesktopMount.workspaceId, parameters.sessionID)).item.todos,
