...
try {
  tmpdir = await mkdtemp(path.join(os.tmpdir(), "openrind-desktop-browser-entry-"));
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "Openrind Desktop browser-entry test" });
