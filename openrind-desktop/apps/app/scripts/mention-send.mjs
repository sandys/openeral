...
try {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "Openrind Desktop mention-send" });
  ...
  await step("prompt.invalidFilePart", async () => {
    // Mirrors the bug in Openrind Desktop: sending a file mention with only {path}.
    try {
