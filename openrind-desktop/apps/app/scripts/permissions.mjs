...
try {
  ...
  const session = await client.session.create({
    title: "Openrind Desktop permission test",
    permission: [
      {
        permission: "bash",
        pattern: "*",
  ...
  const externalPath = "/tmp/openrind-desktop-permission-test.txt";
  await writeFile(externalPath, "openrind-desktop permission test\n", "utf8");
