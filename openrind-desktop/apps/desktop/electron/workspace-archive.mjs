...
function defaultOpenrindDesktopConfig(targetDir, preset = "starter") {
  return {
    version: 1,
    workspace: {
...
export async function importWorkspaceConfig({ archivePath, targetDir, name }) {
  ...
  const openrindDesktopPath = path.join(opencodeDir, "openrind-desktop.json");
  let preset = "starter";
  ...
  if (await pathExists(openrindDesktopPath)) {
    ...
    try {
      ...
      const config = defaultOpenrindDesktopConfig(targetDir, preset);
      await writeFile(openrindDesktopPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
    }
  } else {
    const config = defaultOpenrindDesktopConfig(targetDir, preset);
