...
// `openshell sandbox connect <name>` so Claude Code gets a working
// stdin/stdout/resize channel.
//
// IMPORTANT: use ordinary child-process pipes here. On Windows, adding a local
// ConPTY (Windows Pseudoconsole) makes it keep its own
 ...
 * string contract expected by the renderer.
 *
...
async function defaultSpawnImpl(opts) {
  ...
  const shellCmd = `exec ${FUSE_CLI} --gateway-endpoint ${FUSE_GATEWAY_ENDPOINT} sandbox connect ${quotedName}`;
  // extraEnv contains non-secret terminal hints only. Claude credentials are
  // attached to the sandbox by the gateway-managed provider.
 ...
 * @param {Record<string, string>} [opts.extraEnv]  Non-secret terminal hints
 *   forwarded into WSL via WSLENV. Provider credentials never use this path.
 ...
 * PostgreSQL-backed /sandbox/work.
 */
