...
const useCorepackPnpm = process.platform === "win32" && process.env.OPENRIND_DESKTOP_USE_COREPACK_PNPM === "1";
...
const hostOverride = process.env.OPENRIND_DESKTOP_DEV_HOST?.trim() || null;
const port = readPort();
    ...
    watchTimer = setInterval(async () => {
      ...
      try {
        if (!(await looksLikeVite(watchedViteUrl))) {
          console.error(
            `[openrind-desktop] Reused UI dev server at ${watchedViteUrl} is no longer reachable; stopping desktop dev.`
          );
          stop(1);
        }
      } finally {
...
const fetchOpenrindDesktopDevServerId = async (baseUrl) => {
  try {
    const res = await fetchWithTimeout(`${baseUrl}/__openrind_desktop_dev_server_id`, { timeoutMs: 1200 });
...
const runPrepareSidecars = () => {
  ...
  if (process.env.OPENRIND_DESKTOP_SIDECAR_FORCE_BUILD !== "0") {
    args.push("--force");
  }
...
const ensureLinuxDesktopDependencies = () => {
  ...
  console.error(
    "[openrind-desktop] Missing Linux desktop system dependencies required by Tauri:\n" +
      `  ${missing.join(", ")}\n\n` +
...
const runUiDevServer = () => {
  const uiArgs =
    process.platform === "win32"
      ? [...pnpmArgs, "--filter", "@openrind/app", "dev:windows"]
      : [...pnpmArgs, "-w", "dev:ui"];
...
const main = async () => {
  ...
  for (const candidate of baseUrls) {
    if (await looksLikeVite(candidate)) {
      ...
      detectedViteAppRoot = await fetchOpenrindDesktopDevServerId(candidate);
      break;
    }
  }
  ...
  if (detectedViteUrl) {
    if (detectedViteAppRoot && detectedViteAppRoot !== expectedAppRoot) {
      console.error(
        `[openrind-desktop] Found a Vite dev server at ${detectedViteUrl}, but it serves a different checkout:\n` +
    ...
    if (!detectedViteAppRoot) {
      console.error(
        `[openrind-desktop] Found a Vite dev server at ${detectedViteUrl}, but could not verify which checkout it belongs to.\n` +
          `Stop the other dev server or run with a different PORT (for example: PORT=5174 pnpm dev).`
      );
      process.exit(1);
    }
    console.log(`[openrind-desktop] UI dev server already running at ${detectedViteUrl} (reusing).`);
  ...
  if (portInUse) {
    console.error(
      `[openrind-desktop] Port ${port} is in use, but it does not look like a Vite dev server.\n` +
        `Set PORT to a free port (e.g. PORT=5174) or stop the process using port ${port}.`
    );
    process.exit(1);
  }

  console.log(`[openrind-desktop] Starting UI dev server on port ${port}...`);
