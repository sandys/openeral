...
const RELOAD_EVENT: &str = "openrind-desktop://reload-required";
...
fn reason_for_path(path: &Path) -> Option<&'static str> {
    ...
    // Ignore OpenrindDesktop metadata files — they don't affect the OpenCode engine.
    if lower.ends_with("/openrind-desktop.json") {
        return None;
    }
