...
const RESERVED_PREFIXES: &[&str] = &["OPENRIND_DESKTOP_", "OPENCODE_"];
...
fn resolve_user_env_file_path() -> Option<PathBuf> {
    if let Ok(override_path) = std::env::var("OPENRIND_DESKTOP_ENV_STORE") {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return root.map(|base| base.join("openrind-desktop").join("env.json"));
    }

    #[cfg(not(target_os = "windows"))]
    {
        dirs::home_dir().map(|home| home.join(".config").join("openrind-desktop").join("env.json"))
    }
}
...
/// stripped so a tampered file cannot shadow OpenrindDesktop / OpenCode internals.
...
mod tests {
    ...
    fn unique_env_path() -> PathBuf {
        ...
        let dir = std::env::temp_dir()
            .join(format!("openrind-desktop-env-file-{epoch}-{nonce}"));
    ...
    impl EnvStoreGuard {
        fn set(path: PathBuf) -> Self {
            let original = std::env::var("OPENRIND_DESKTOP_ENV_STORE").ok();
            std::env::set_var("OPENRIND_DESKTOP_ENV_STORE", &path);
            Self { path, original }
        }
    }
    ...
    impl Drop for EnvStoreGuard {
        fn drop(&mut self) {
            match &self.original {
                Some(value) => std::env::set_var("OPENRIND_DESKTOP_ENV_STORE", value),
                None => std::env::remove_var("OPENRIND_DESKTOP_ENV_STORE"),
            }
    ...
    fn strips_reserved_and_malformed_keys() {
        ...
        fs::write(
            &path,
            r#"{"schemaVersion":1,"updatedAt":0,"variables":[
                {"key":"OPENRIND_DESKTOP_TOKEN","value":"stolen","updatedAt":0},
