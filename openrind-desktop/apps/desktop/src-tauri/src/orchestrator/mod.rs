...
pub fn resolve_orchestrator_data_dir() -> String {
    let env_dir = env::var("OPENRIND_DESKTOP_DATA_DIR")
        .ok()
    ...
    if let Some(home) = home_dir() {
        return home
            .join(".openrind-desktop")
            .join("openrind-desktop-orchestrator")
            .to_string_lossy()
            .to_string();
    }

    ".openrind-desktop/openrind-desktop-orchestrator".to_string()
}

fn orchestrator_state_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("openrind-desktop-orchestrator-state.json")
}

fn orchestrator_auth_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("openrind-desktop-orchestrator-auth.json")
}
...
mod tests {
    ...
    fn request_shutdown_returns_false_without_state() {
        let dir = std::env::temp_dir().join(format!(
            "openrind-desktop-orchestrator-shutdown-missing-{}",
            Uuid::new_v4()
        ));
    ...
    fn request_shutdown_posts_to_daemon_endpoint() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "openrind-desktop-orchestrator-shutdown-state-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).expect("create state dir");
        let state_path = dir.join("openrind-desktop-orchestrator-state.json");
        fs::write(
            &state_path,
            format!(
