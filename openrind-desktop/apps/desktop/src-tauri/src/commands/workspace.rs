...
use crate::types::{
    ExecResult, RemoteType, WorkspaceInfo, WorkspaceList, WorkspaceOpenrindDesktopConfig, WorkspaceType,
};
...
use crate::workspace::state::{
    ...
    save_workspace_state, stable_workspace_id, stable_workspace_id_for_openrind_desktop,
...
pub fn workspace_create(
    ...
    state.workspaces.push(WorkspaceInfo {
        ...
        directory: None,
        display_name: None,
        openrind_desktop_host_url: None,
        openrind_desktop_token: None,
        openrind_desktop_client_token: None,
        openrind_desktop_host_token: None,
        openrind_desktop_workspace_id: None,
        openrind_desktop_workspace_name: None,
...
pub fn workspace_create_remote(
    ...
    openrind_desktop_host_url: Option<String>,
    openrind_desktop_token: Option<String>,
    openrind_desktop_client_token: Option<String>,
    openrind_desktop_host_token: Option<String>,
    openrind_desktop_workspace_id: Option<String>,
    openrind_desktop_workspace_name: Option<String>,
    sandbox_backend: Option<String>,
    sandbox_profile: Option<String>,
    ...
    let openrind_desktop_host_url = openrind_desktop_host_url
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let openrind_desktop_token = openrind_desktop_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let openrind_desktop_client_token = openrind_desktop_client_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    let openrind_desktop_host_token = openrind_desktop_host_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    if remote_type == RemoteType::OpenrindDesktop {
        let host_url = openrind_desktop_host_url.clone().unwrap_or_default();
        if host_url.is_empty() {
            return Err("openrindDesktopHostUrl is required for OpenrindDesktop remote".to_string());
        }
        if !host_url.starts_with("http://") && !host_url.starts_with("https://") {
            return Err("openrindDesktopHostUrl must start with http:// or https://".to_string());
        }
    }

    let id = if remote_type == RemoteType::OpenrindDesktop {
        stable_workspace_id_for_openrind_desktop(
            openrind_desktop_host_url.as_deref().unwrap_or(""),
            openrind_desktop_workspace_id.as_deref(),
        )
    } else {
        stable_workspace_id_for_remote(&base_url, directory.as_deref())
    };
    let name = openrind_desktop_workspace_name
        .clone()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| display_name.clone())
        .unwrap_or_else(|| {
            if remote_type == RemoteType::OpenrindDesktop {
                openrind_desktop_host_url
                    .clone()
    ...
    state.workspaces.push(WorkspaceInfo {
        ...
        openrind_desktop_client_token,
        openrind_desktop_host_token,
        openrind_desktop_workspace_id,
        openrind_desktop_workspace_name,
        sandbox_backend: sandbox_backend
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty()),
        sandbox_profile: sandbox_profile
...
pub fn workspace_update_remote(
    ...
    if let Some(next_host_url) = openrind_desktop_host_url
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        ...
        entry.openrind_desktop_host_url = Some(next_host_url);
    }

    if openrind_desktop_token.is_some() {
        entry.openrind_desktop_token = openrind_desktop_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if openrind_desktop_client_token.is_some() {
        entry.openrind_desktop_client_token = openrind_desktop_client_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if openrind_desktop_host_token.is_some() {
        entry.openrind_desktop_host_token = openrind_desktop_host_token
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if openrind_desktop_workspace_id.is_some() {
        entry.openrind_desktop_workspace_id = openrind_desktop_workspace_id
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
    }

    if let Some(next_name) = openrind_desktop_workspace_name
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    {
        entry.openrind_desktop_workspace_name = Some(next_name.clone());
...
pub fn workspace_add_authorized_root(
    ...
    let openrind_desktop_path = PathBuf::from(&workspace_path)
        .join(".opencode")
        .join("openrind-desktop.json");

    if let Some(parent) = openrind_desktop_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
    }

    let mut config: WorkspaceOpenrindDesktopConfig = if openrind_desktop_path.exists() {
        let raw = fs::read_to_string(&openrind_desktop_path)
            .map_err(|e| format!("Failed to read {}: {e}", openrind_desktop_path.display()))?;
        serde_json::from_str(&raw).unwrap_or_default()
    } else {
        let mut cfg = WorkspaceOpenrindDesktopConfig::default();
...
#[tauri::command]
pub fn workspace_openrind_desktop_read(
    _app: tauri::AppHandle,
    workspace_path: String,
) -> Result<WorkspaceOpenrindDesktopConfig, String> {
    let workspace_path = workspace_path.trim().to_string();
    if workspace_path.is_empty() {
        return Err("workspacePath is required".to_string());
    }
    ...
    serde_json::from_str::<WorkspaceOpenrindDesktopConfig>(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", openrind_desktop_path.display()))
}

#[tauri::command]
pub fn workspace_openrind_desktop_write(
    _app: tauri::AppHandle,
    workspace_path: String,
    ...
    Ok(ExecResult {
        ok: true,
        status: 0,
        stdout: format!("Wrote {}", openrind_desktop_path.display()),
        stderr: String::new(),
    })
}
...
pub fn workspace_import_config(
    ...
    let openrind_desktop_path = target_path.join(".opencode").join("openrind-desktop.json");
    ...
    if openrind_desktop_path.exists() {
        let raw = fs::read_to_string(&openrind_desktop_path)
            .map_err(|e| format!("Failed to read {}: {e}", openrind_desktop_path.display()))?;
        if let Ok(mut config) = serde_json::from_str::<WorkspaceOpenrindDesktopConfig>(&raw) {
        ...
        let config = WorkspaceOpenrindDesktopConfig::new(&target_dir, &preset, now_ms());
        if let Some(parent) = openrind_desktop_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
        }
