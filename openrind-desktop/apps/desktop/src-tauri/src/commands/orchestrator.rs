...
const SANDBOX_PROGRESS_EVENT: &str = "openrind-desktop://sandbox-create-progress";

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct OrchestratorDetachedHost {
    pub openrind_desktop_url: String,
    pub token: String,
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct OpenrindDesktopDockerCleanupResult {
    pub candidates: Vec<String>,
...
fn resolve_docker_candidates() -> Vec<PathBuf> {
    ...
    for key in ["OPENRIND_DESKTOP_DOCKER_BIN", "OPENWRK_DOCKER_BIN", "DOCKER_BIN"] {
...
fn run_docker_command_detailed(
    ...
    let hint = "Set OPENRIND_DESKTOP_DOCKER_BIN (or OPENWRK_DOCKER_BIN) to your docker binary, e.g. /opt/homebrew/bin/docker";
    Err(format!(
...
fn is_sensitive_progress_key(key: &str) -> bool {
    ...
    matches!(
        ...
        "token"
            ...
            | "opencodeusername"
            | "authorization"
    ) || ((key.starts_with("OPENRIND_DESKTOP_") || key.starts_with("OPENCODE_") || key.starts_with("DEN_"))
...
fn derive_orchestrator_container_name(run_id: &str) -> String {
    // Must match openrind-desktop-orchestrator's docker naming scheme:
    // `openrind-desktop-orchestrator-${runId.replace(/[^a-zA-Z0-9_.-]+/g, "-").slice(0, 24)}`
    ...
    format!("openrind-desktop-orchestrator-{sanitized}")
}

fn is_openrind_desktop_managed_container(name: &str) -> bool {
    name.starts_with("openrind-desktop-orchestrator-")
        || name.starts_with("openrind-desktop-dev-")
        || name.starts_with("openwrk-")
}

fn list_openrind_desktop_managed_containers() -> Result<Vec<String>, String> {
    ...
    let mut names: Vec<String> = stdout
        ...
        .filter(|name| !name.is_empty() && is_openrind_desktop_managed_container(name))
        .collect();
    names.sort();
    names.dedup();
    Ok(names)
...
fn format_sandbox_start_timeout_error(
    elapsed_ms: u64,
    openrind_desktop_url: &str,
    last_error: Option<&str>,
    container_state: Option<&str>,
    container_probe_error: Option<&str>,
) -> String {
    let mut details = vec![
        format!("stage=openrind-desktop.healthcheck"),
        format!("elapsed_ms={elapsed_ms}"),
        format!("url={openrind_desktop_url}"),
    ...
    format!(
        "Timed out waiting for OpenrindDesktop server ({})",
        details.join(", ")
    )
}

fn issue_owner_token(openrind_desktop_url: &str, host_token: &str) -> Result<String, String> {
    let response = ureq::post(&format!("{}/tokens", openrind_desktop_url.trim_end_matches('/')))
        .set("X-OpenrindDesktop-Host-Token", host_token)
        .set("Content-Type", "application/json")
        .send_string(r#"{"scope":"owner","label":"OpenrindDesktop detached owner token"}"#)
...
pub fn orchestrator_start_detached(
    ...
    openrind_desktop_token: Option<String>,
    openrind_desktop_host_token: Option<String>,
    ...
    let token = openrind_desktop_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| Uuid::new_v4().to_string());
    let host_token = openrind_desktop_host_token
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| Uuid::new_v4().to_string());
    let openrind_desktop_url = format!("http://127.0.0.1:{port}");
    ...
    emit_sandbox_progress(
        ...
        json!({
            ...
            "openrindDesktopUrl": openrind_desktop_url,
            "port": port,
    ...
    if wants_docker_sandbox {
        ...
        emit_sandbox_progress(
            ...
            json!({
                ...
                "hasOpenrindDesktopDockerBinOverride": env::var("OPENRIND_DESKTOP_DOCKER_BIN").ok().is_some(),
    ...
    let (command, command_label) = match app.shell().sidecar("openrind-desktop-orchestrator") {
        Ok(command) => (command, "sidecar:openrind-desktop-orchestrator".to_string()),
        Err(_) => (app.shell().command("openrind-desktop"), "path:openrind-desktop".to_string()),
    };
    ...
    // We pass explicit tokens and a free port so the UI can connect deterministically.
    {
        let mut args: Vec<String> = vec![
            ...
            "auto".to_string(),
            "--detach".to_string(),
            "--openrind-desktop-port".to_string(),
            port.to_string(),
            "--run-id".to_string(),
        ...
        emit_sandbox_progress(
            ...
            json!({
                ...
                "openrindDesktopUrl": openrind_desktop_url,
                "argCount": args.len(),
                "hasDockerOverrides": env::var("OPENRIND_DESKTOP_DOCKER_BIN").ok().is_some()
        ...
        if let Err(err) = command
            .env("OPENRIND_DESKTOP_TOKEN", token.clone())
            .env("OPENRIND_DESKTOP_HOST_TOKEN", host_token.clone())
            .spawn()
        {
            emit_sandbox_progress(
                &app,
            ...
            return Err(format!("Failed to start openrind-desktop orchestrator: {err}"));
        }
        eprintln!(
            "[sandbox-create][at={}][runId={}][stage=spawn] launched openrind-desktop sidecar for detached sandbox host",
            now_ms(),
            sandbox_run_id
        );
    }
    ...
    emit_sandbox_progress(
        ...
        &sandbox_run_id,
        "spawned",
        "Sandbox process launched. Waiting for OpenrindDesktop server...",
        json!({
            "openrindDesktopUrl": openrind_desktop_url,
        }),
    );
    ...
    while start.elapsed() < Duration::from_millis(health_timeout_ms) {
        ...
        match ureq::get(&format!("{}/health", openrind_desktop_url.trim_end_matches('/'))).call() {
            Ok(response) if response.status() >= 200 && response.status() < 300 => {
                emit_sandbox_progress(
                    &app,
                    &sandbox_run_id,
                    "openrind-desktop.healthy",
                    "OpenrindDesktop server is ready.",
                    json!({
                        "openrindDesktopUrl": openrind_desktop_url,
                        "elapsedMs": elapsed_ms,
        ...
        if last_tick.elapsed() > Duration::from_millis(850) {
            ...
            emit_sandbox_progress(
                &app,
                &sandbox_run_id,
                "openrind-desktop.waiting",
                "Waiting for OpenrindDesktop server...",
                json!({
                    "openrindDesktopUrl": openrind_desktop_url,
                    "elapsedMs": elapsed_ms,
    ...
    let owner_token = issue_owner_token(&openrind_desktop_url, &host_token).ok();
...
pub fn sandbox_stop(container_name: String) -> Result<ExecResult, String> {
    ...
    if !name.starts_with("openrind-desktop-orchestrator-") {
        return Err(
            "Refusing to stop container: expected name starting with 'openrind-desktop-orchestrator-'"
                .to_string(),
        );
    }
    if !name
...
#[tauri::command]
pub fn sandbox_cleanup_openrind_desktop_containers() -> Result<OpenrindDesktopDockerCleanupResult, String> {
    let candidates = list_openrind_desktop_managed_containers()?;
    if candidates.is_empty() {
        return Ok(OpenrindDesktopDockerCleanupResult {
            candidates,
            removed: Vec::new(),
            errors: Vec::new(),
        });
    ...
    Ok(OpenrindDesktopDockerCleanupResult {
        candidates,
        removed,
        errors,
    })
...
pub fn sandbox_debug_probe(app: AppHandle) -> SandboxDebugProbeResult {
    ...
    let workspace_dir = env::temp_dir().join(format!("openrind-desktop-sandbox-probe-{}", Uuid::new_v4()));
...
mod tests {
    ...
    fn docker_command_falls_back_after_timeout() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("openrind-desktop-docker-timeout-test-{}", Uuid::new_v4()));
...
"#,
        ...
        let _docker = EnvGuard::set("OPENRIND_DESKTOP_DOCKER_BIN", slow.to_string_lossy().to_string());
    ...
    fn local_command_timeout_returns_when_descendant_keeps_pipe_open() {
        let tmp =
            std::env::temp_dir().join(format!("openrind-desktop-timeout-pipe-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&tmp).expect("create tmp dir");
    ...
    fn sandbox_start_timeout_error_includes_stage_diagnostics() {
        ...
        assert!(message.contains("stage=openrind-desktop.healthcheck"));
    ...
    fn sandbox_doctor_uses_override_docker_bin() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("openrind-desktop-docker-doctor-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&tmp).expect("create tmp dir");
...
"#,
        ...
        let _path = EnvGuard::set("PATH", tmp.to_string_lossy().to_string());
        let _docker = EnvGuard::set("OPENRIND_DESKTOP_DOCKER_BIN", fast.to_string_lossy().to_string());
        let _docker_alt = EnvGuard::unset("OPENWRK_DOCKER_BIN");
        let _docker_bin = EnvGuard::unset("DOCKER_BIN");
