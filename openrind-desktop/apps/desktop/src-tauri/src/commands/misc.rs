...
use crate::openrind_desktop_server::manager::OpenrindDesktopServerManager;
...
use crate::types::{ExecResult, WorkspaceOpenrindDesktopConfig};
...
pub struct AppBuildInfo {
    ...
    pub openrind_desktop_dev_mode: bool,
    pub os: &'static str,
    pub arch: &'static str,
}
...
fn current_openrind_desktop_state_paths(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    let mut paths = vec![
        app.path()
            .app_cache_dir()
...
fn read_workspace_openrind_desktop_config(
    workspace_path: &Path,
) -> Result<WorkspaceOpenrindDesktopConfig, String> {
    let openrind_desktop_path = workspace_path.join(".opencode").join("openrind-desktop.json");
...
fn load_authorized_roots(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    ...
    for workspace in state.workspaces {
        ...
        let mut config = read_workspace_openrind_desktop_config(&workspace_path)?;
...
#[tauri::command]
pub fn reset_openrind_desktop_state(
    app: tauri::AppHandle,
    mode: String,
    engine_manager: State<EngineManager>,
    orchestrator_manager: State<OrchestratorManager>,
    ...
    stop_host_services(&engine_manager, &orchestrator_manager, &openrind_desktop_manager);
...
pub fn app_build_info(app: AppHandle) -> AppBuildInfo {
    ...
    let git_sha = option_env!("OPENRIND_DESKTOP_GIT_SHA").map(|value| value.to_string());
    let build_epoch = option_env!("OPENRIND_DESKTOP_BUILD_EPOCH").map(|value| value.to_string());
    AppBuildInfo {
        version,
        git_sha,
        build_epoch,
        openrind_desktop_dev_mode: env_truthy("OPENRIND_DESKTOP_DEV_MODE"),
...
#[tauri::command]
pub fn nuke_openrind_desktop_and_opencode_config_and_exit(
    app: AppHandle,
    engine_manager: State<EngineManager>,
    orchestrator_manager: State<OrchestratorManager>,
    openrind_desktop_manager: State<OpenrindDesktopServerManager>,
    ...
    let dev_mode = env_truthy("OPENRIND_DESKTOP_DEV_MODE");
    let mut paths = current_openrind_desktop_state_paths(&app)?;
    if dev_mode {
        // In dev mode, the current app + orchestrator directories are already isolated
        // by the dev app identity and OPENRIND_DESKTOP_DATA_DIR, so only clear those dev paths.
    } else {
