...
pub struct MigrateToElectronRequest {
    ...
    /// Optional override for where the new OpenrindDesktop bundle should land on
...
fn current_app_bundle_path() -> Result<PathBuf, String> {
    // Tauri's running .app is <something>/Contents/MacOS/OpenrindDesktop. Walk up
...
fn write_macos_migration_script(
    ...
    let script_path = cache.join("openrind-desktop-migrate.sh");
...
URL="{url}"
TAURI_PID="{tauri_pid}"
WORK=$(mktemp -d /tmp/openrind-desktop-migrate-XXXXXX)
ZIP="$WORK/OpenrindDesktop-electron.zip"
...
NEW_APP=$(find "$WORK" -maxdepth 2 -name 'OpenrindDesktop.app' -type d | head -n 1)
if [ -z "$NEW_APP" ]; then
  echo "[migration] no OpenrindDesktop.app in zip" >&2
  exit 1
fi
