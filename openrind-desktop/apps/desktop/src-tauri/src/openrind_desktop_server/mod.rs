...
use crate::types::OpenrindDesktopServerInfo;
...
use manager::OpenrindDesktopServerManager;
use spawn::{resolve_openrind_desktop_port, spawn_openrind_desktop_server};

fn generate_token() -> String {
    Uuid::new_v4().to_string()
}

const OPENRIND_DESKTOP_SERVER_TOKEN_STORE_VERSION: u32 = 1;
const OPENRIND_DESKTOP_SERVER_STATE_VERSION: u32 = 3;
const LEGACY_FIXED_OPENRIND_DESKTOP_PORT: u16 = 8787;

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct PersistedOpenrindDesktopServerTokens {
...
#[serde(rename_all = "camelCase")]
struct PersistedOpenrindDesktopServerTokenStore {
    version: u32,
    #[serde(default)]
    workspaces: HashMap<String, PersistedOpenrindDesktopServerTokens>,
}

#[derive(Debug, Default, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct PersistedOpenrindDesktopServerState {
    version: u32,
...
fn openrind_desktop_server_token_store_path(app: &AppHandle) -> Result<PathBuf, String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    Ok(data_dir.join("openrind-desktop-server-tokens.json"))
}

fn openrind_desktop_server_state_path(app: &AppHandle) -> Result<PathBuf, String> {
    let data_dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    Ok(data_dir.join("openrind-desktop-server-state.json"))
}
...
fn load_openrind_desktop_server_token_store(
    path: &Path,
) -> Result<PersistedOpenrindDesktopServerTokenStore, String> {
    if !path.exists() {
        return Ok(PersistedOpenrindDesktopServerTokenStore {
            version: OPENRIND_DESKTOP_SERVER_TOKEN_STORE_VERSION,
    ...
    let mut store: PersistedOpenrindDesktopServerTokenStore = serde_json::from_str(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
    if store.version < OPENRIND_DESKTOP_SERVER_TOKEN_STORE_VERSION {
        store.version = OPENRIND_DESKTOP_SERVER_TOKEN_STORE_VERSION;
    }
    Ok(store)
}

fn save_openrind_desktop_server_token_store(
    path: &Path,
    store: &PersistedOpenrindDesktopServerTokenStore,
) -> Result<(), String> {
...
fn load_openrind_desktop_server_state(path: &Path) -> Result<PersistedOpenrindDesktopServerState, String> {
    if !path.exists() {
        return Ok(PersistedOpenrindDesktopServerState {
            version: OPENRIND_DESKTOP_SERVER_STATE_VERSION,
    ...
    let payload: Value = serde_json::from_str(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
    let mut state = parse_openrind_desktop_server_state_value(&payload)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
    if state.version < OPENRIND_DESKTOP_SERVER_STATE_VERSION {
        if state.version < 2 && state.preferred_port == Some(LEGACY_FIXED_OPENRIND_DESKTOP_PORT) {
            state.preferred_port = None;
        }
        if state.version < 3 && state.preferred_port == Some(LEGACY_FIXED_OPENRIND_DESKTOP_PORT) {
            state.preferred_port = None;
        }
        state.version = OPENRIND_DESKTOP_SERVER_STATE_VERSION;
    }
    Ok(state)
}

fn parse_openrind_desktop_server_state_value(
    payload: &Value,
) -> Result<PersistedOpenrindDesktopServerState, String> {
    let version = payload
        .get("version")
        .and_then(|value| value.as_u64())
        .and_then(|value| u32::try_from(value).ok())
        .unwrap_or(OPENRIND_DESKTOP_SERVER_STATE_VERSION);
    ...
    Ok(PersistedOpenrindDesktopServerState {
        version,
        workspace_ports,
...
fn save_openrind_desktop_server_state(
    path: &Path,
    state: &PersistedOpenrindDesktopServerState,
) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
...
#[cfg(test)]
fn read_preferred_openrind_desktop_port_at_path(
    path: &Path,
    workspace_key: &str,
) -> Result<Option<u16>, String> {
    let state = load_openrind_desktop_server_state(path)?;
    let normalized = normalize_workspace_key(workspace_key);
    if !normalized.is_empty() {
...
fn reserved_openrind_desktop_ports_at_path(
    path: &Path,
...
fn reserved_openrind_desktop_ports(
    app: &AppHandle,
    exclude_workspace_key: &str,
) -> Result<HashSet<u16>, String> {
    let path = openrind_desktop_server_state_path(app)?;
    reserved_openrind_desktop_ports_at_path(&path, exclude_workspace_key)
}

fn persist_preferred_openrind_desktop_port_at_path(
    path: &Path,
    workspace_key: &str,
    port: u16,
) -> Result<(), String> {
    let mut state = load_openrind_desktop_server_state(path)?;
    state.version = OPENRIND_DESKTOP_SERVER_STATE_VERSION;
    let normalized = normalize_workspace_key(workspace_key);
    if normalized.is_empty() {
    ...
    save_openrind_desktop_server_state(path, &state)
}

fn persist_preferred_openrind_desktop_port(
    app: &AppHandle,
    workspace_key: &str,
    port: u16,
) -> Result<(), String> {
    let path = openrind_desktop_server_state_path(app)?;
    persist_preferred_openrind_desktop_port_at_path(&path, workspace_key, port)
}

fn load_or_create_workspace_tokens(
    app: &AppHandle,
    workspace_key: &str,
) -> Result<PersistedOpenrindDesktopServerTokens, String> {
    let path = openrind_desktop_server_token_store_path(app)?;
...
fn load_or_create_workspace_tokens_at_path(
    ...
    let mut store = load_openrind_desktop_server_token_store(path)?;
    let normalized = normalize_workspace_key(workspace_key);
    if let Some(tokens) = store.workspaces.get(&normalized) {
        return Ok(tokens.clone());
    }

    let tokens = PersistedOpenrindDesktopServerTokens {
    ...
    save_openrind_desktop_server_token_store(path, &store)?;
    Ok(tokens)
}
...
fn wait_for_openrind_desktop_health(base_url: &str, timeout: Duration) -> Result<(), String> {
    let deadline = Instant::now() + timeout;
    let health_url = format!("{}/health", base_url.trim_end_matches('/'));
    let mut last_error = "OpenrindDesktop server did not become healthy".to_string();
    ...
    while Instant::now() < deadline {
        match ureq::get(&health_url).call() {
            ...
            Ok(response) => {
                last_error = format!(
                    "OpenrindDesktop server health check returned {}",
                    response.status()
                )
            }
...
fn issue_owner_token(base_url: &str, host_token: &str) -> Result<String, String> {
    let response = ureq::post(&format!("{}/tokens", base_url.trim_end_matches('/')))
        .set("X-OpenrindDesktop-Host-Token", host_token)
        .set("Content-Type", "application/json")
        .send_string(r#"{"scope":"owner","label":"OpenrindDesktop desktop owner token"}"#)
    ...
    payload
        ...
        .ok_or_else(|| "OpenrindDesktop server did not return an owner token".to_string())
}
...
pub fn start_openrind_desktop_server(
    app: &AppHandle,
    manager: &OpenrindDesktopServerManager,
...
) -> Result<OpenrindDesktopServerInfo, String> {
    let mut state = manager
        .inner
        .lock()
        .map_err(|_| "openrind-desktop server mutex poisoned".to_string())?;
    OpenrindDesktopServerManager::stop_locked(&mut state);
    ...
    let active_workspace = workspace_paths
        ...
        .unwrap_or("");
    let reserved_ports = reserved_openrind_desktop_ports(app, active_workspace)?;
    ...
    let port = resolve_openrind_desktop_port(&host, None, &reserved_ports)?;
    ...
    let (mut rx, child) = spawn_openrind_desktop_server(
        app,
        &host,
        port,
        workspace_paths,
    ...
    if let Err(error) = wait_for_openrind_desktop_health(&base_url, Duration::from_secs(10)) {
        OpenrindDesktopServerManager::stop_locked(&mut state);
        return Err(error);
    }
    // Owner tokens can be reset independently inside openrind-desktop-server. Mint a
    ...
    let _ = persist_preferred_openrind_desktop_port(app, active_workspace, port);
                ...
                CommandEvent::Terminated(payload) => {
                    if let Ok(mut state) = state_handle.try_lock() {
                        ...
                        if let Some(code) = payload.code {
                            let next = format!("OpenrindDesktop server exited (code {code}).");
                            state.last_stderr = Some(truncate_output(&next, 8000));
                        }
                    }
                }
    ...
    Ok(OpenrindDesktopServerManager::snapshot_locked(&mut state))
}
...
mod tests {
    ...
    fn unique_temp_path(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("openrind-desktop-server-{name}-{nonce}.json"))
    }
  ...
  "preferredPort": 49124
        ...
        let preferred = read_preferred_openrind_desktop_port_at_path(&path, "/tmp/workspace")
            .expect("read camel preferred port");
        let reserved = reserved_openrind_desktop_ports_at_path(&path, "/tmp/other")
  ...
  "preferredPort": null
        ...
        let reserved = reserved_openrind_desktop_ports_at_path(&path, "/tmp/workspace")
    ...
    fn migrates_legacy_fixed_port_to_no_preference() {
        ...
        let state = load_openrind_desktop_server_state(&path).expect("load migrated state");
        assert_eq!(state.version, OPENRIND_DESKTOP_SERVER_STATE_VERSION);
    ...
    fn reuses_workspace_tokens_across_canonical_path_aliases() {
        ...
        let workspace = PathBuf::from(format!(
            "/tmp/openrind-desktop-workspace-token-alias-{}-{}",
            std::process::id(),
            now_ms()
        ));
    ...
    fn reuses_workspace_port_across_canonical_path_aliases() {
        ...
        let workspace = PathBuf::from(format!(
            "/tmp/openrind-desktop-workspace-port-alias-{}-{}",
            std::process::id(),
            now_ms()
        ));
        fs::create_dir_all(&workspace).expect("create temp workspace");
        ...
        persist_preferred_openrind_desktop_port_at_path(&path, &workspace.to_string_lossy(), 49_123)
            .expect("persist preferred port using raw path");

        let preferred = read_preferred_openrind_desktop_port_at_path(&path, &canonical.to_string_lossy())
            .expect("read preferred port using canonical path");
        let reserved = reserved_openrind_desktop_ports_at_path(&path, &canonical.to_string_lossy())
