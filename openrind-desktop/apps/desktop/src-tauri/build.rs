...
fn main() {
    ...
    ensure_openrind_desktop_server_sidecar();
...
fn emit_build_info() {
    let sha = env::var("OPENRIND_DESKTOP_GIT_SHA")
        .ok()
        .and_then(|value| {
    ...
    if let Some(value) = sha {
        println!("cargo:rustc-env=OPENRIND_DESKTOP_GIT_SHA={}", value);
    }
    ...
    if let Some(value) = build_epoch {
        println!("cargo:rustc-env=OPENRIND_DESKTOP_BUILD_EPOCH={}", value);
    }
}

fn emit_desktop_bootstrap_seed() {
    for key in [
        "OPENRIND_DESKTOP_DESKTOP_DEN_BASE_URL",
        "OPENRIND_DESKTOP_DESKTOP_DEN_API_BASE_URL",
        "OPENRIND_DESKTOP_DESKTOP_DEN_REQUIRE_SIGNIN",
        "VITE_DEN_BASE_URL",
    ...
    if let Some(value) = env::var("OPENRIND_DESKTOP_DESKTOP_DEN_BASE_URL")
        .ok()
        .or_else(|| env::var("VITE_DEN_BASE_URL").ok())
        .map(|entry| entry.trim().to_string())
        .filter(|entry| !entry.is_empty())
    {
        println!("cargo:rustc-env=OPENRIND_DESKTOP_DESKTOP_DEN_BASE_URL={value}");
    }

    if let Some(value) = env::var("OPENRIND_DESKTOP_DESKTOP_DEN_API_BASE_URL")
        .ok()
        .or_else(|| env::var("VITE_DEN_API_BASE_URL").ok())
        .map(|entry| entry.trim().to_string())
        .filter(|entry| !entry.is_empty())
    {
        println!("cargo:rustc-env=OPENRIND_DESKTOP_DESKTOP_DEN_API_BASE_URL={value}");
    }

    if let Some(value) = env::var("OPENRIND_DESKTOP_DESKTOP_DEN_REQUIRE_SIGNIN")
        .ok()
        .or_else(|| env::var("VITE_DEN_REQUIRE_SIGNIN").ok())
        .map(|entry| entry.trim().to_string())
        .filter(|entry| !entry.is_empty())
    {
        println!("cargo:rustc-env=OPENRIND_DESKTOP_DESKTOP_DEN_REQUIRE_SIGNIN={value}");
    }
}
...
fn ensure_orchestrator_sidecar() {
    ...
    let canonical_name = if target.contains("windows") {
        "openrind-desktop-orchestrator.exe"
    } else {
        "openrind-desktop-orchestrator"
    };
    let mut target_name = format!("openrind-desktop-orchestrator-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let source_path = env::var("OPENRIND_DESKTOP_ORCHESTRATOR_BIN_PATH")
        .ok()
        .map(PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| {
            find_in_path(if target.contains("windows") {
                "openrind-desktop.exe"
            } else {
                "openrind-desktop"
            })
        });
    ...
    let Some(source_path) = source_path else {
        println!(
            "cargo:warning=orchestrator sidecar missing at {} (set OPENRIND_DESKTOP_ORCHESTRATOR_BIN_PATH or install openrind-desktop-orchestrator)",
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
        create_debug_stub(&target_dest_path, &sidecar_dir, &profile, &target);
...
fn ensure_openrind_desktop_server_sidecar() {
    let target = env::var("CARGO_CFG_TARGET_TRIPLE")
        .or_else(|_| env::var("TARGET"))
        .or_else(|_| env::var("TAURI_ENV_TARGET_TRIPLE"))
        .unwrap_or_default();
    ...
    let canonical_name = if target.contains("windows") {
        "openrind-desktop-server.exe"
    } else {
        "openrind-desktop-server"
    };

    let mut target_name = format!("openrind-desktop-server-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let source_path = env::var("OPENRIND_DESKTOP_SERVER_BIN_PATH")
        .ok()
        .map(PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| {
    ...
    let Some(source_path) = source_path else {
        println!(
      "cargo:warning=OpenrindDesktop server sidecar missing at {} (set OPENRIND_DESKTOP_SERVER_BIN_PATH or install openrind-desktop-server)",
      dest_path.display()
    );
    ...
    if copied {
        ...
        println!(
            "cargo:warning=Failed to copy OpenrindDesktop server sidecar from {} to {}",
            source_path.display(),
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
