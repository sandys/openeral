...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `openrind-desktop CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "openrind-desktop-file-session-"));
...
try {
  await waitFor(`${openrindDesktopUrl}/health`);

  const workspaces = await fetchJson(`${openrindDesktopUrl}/workspaces`, {
  ...
  const wrote = await runCli([
    ...
    "notes/remote.md",
    "--content",
    "updated by openrind-desktop cli\n",
    "--if-match",
    firstRevision,
    "--json",
  ]);
  ...
  assert.equal(diskContent, "updated by openrind-desktop cli\n");
  ...
  console.log(JSON.stringify({ ok: true, openrindDesktopUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
