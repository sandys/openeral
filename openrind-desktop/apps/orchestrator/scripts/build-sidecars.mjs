...
if (!orchestratorVersion) {
  throw new Error(
    "openrind-desktop-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "openrind-desktop-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "openrind-desktop-server", "build:bin:all"], repoRoot);
...
const entries = {
  "openrind-desktop-server": { version: serverVersion, targets: {} },
  "opencode-router": { version: routerVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `openrind-desktop-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing openrind-desktop-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `openrind-desktop-server-${target.id}${ext}`);
  ...
  entries["openrind-desktop-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "openrind-desktop-orchestrator-sidecars.json"),
