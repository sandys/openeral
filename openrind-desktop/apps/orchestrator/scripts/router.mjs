...
async function runCli(args, dataDir) {
  const child = spawn("node", [cliPath, ...args], {
    env: {
      ...process.env,
      OPENRIND_DESKTOP_DATA_DIR: dataDir,
    },
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `openrind-desktop failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "openrind-desktop-orchestrator-router-"));
