...
if (!version) {
  throw new Error("openrind-desktop-orchestrator version missing in apps/orchestrator/package.json")
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `openrind-desktop-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing openrind-desktop binary at ${src}. Run: pnpm --filter openrind-desktop-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `openrind-desktop${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for openrind-desktop-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      openrindDesktop: `./bin/openrind-desktop${ext}`,
      "openrind-desktop-orchestrator": `./bin/openrind-desktop${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "openrind-desktop-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "openrind-desktop")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "openrind-desktop"))
chmodSync(join(meta, "bin", "openrind-desktop"), 0o755)
...
writeJson(join(meta, "package.json"), {
  name: "openrind-desktop-orchestrator",
  version,
  description: "Openrind Desktop host orchestrator for opencode + Openrind Desktop server + opencode-router",
  license: "MIT",
  bin: {
    openrindDesktop: "./bin/openrind-desktop",
    "openrind-desktop-orchestrator": "./bin/openrind-desktop",
  },
  scripts: {
...
published.push({ name: "openrind-desktop-orchestrator", dir: meta })
