---
name: openrind-desktop-docker-chrome-mcp
description: |
  Start the Openrind Desktop dev stack via Docker and verify real user flows via Chrome MCP.
...
Run from the Openrind Desktop repo root:
```bash
...
- Openrind Desktop server URL (http://localhost:<OPENRIND_DESKTOP_PORT>)
- Token file path (`tmp/.dev-env-<id>`) containing `OPENRIND_DESKTOP_TOKEN` + `OPENRIND_DESKTOP_HOST_TOKEN`
...
```bash
docker ps --format '{{.Names}}' | rg '^openrind-desktop-dev-'
```
