# Openrind Desktop Design System

This document turns the visual direction in `DESIGN-LANGUAGE.md` into an implementation system that can unify:

- `apps/app` (Openrind Desktop app)
- `ee/apps/den-web` (Openrind Desktop Cloud / Den web surfaces)
- `ee/apps/landing` (marketing + product storytelling)

The goal is not to create three similar styles. The goal is one Openrind Desktop design system with a few environment-specific expressions.
...
Openrind Desktop should use a three-layer design system:
...
Openrind Desktop has three main UI contexts:
...
Openrind Desktop should use a consistent spacing scale instead of one-off values.
...
Openrind Desktop tokens are the product API.
...
Restyle one Openrind Desktop app screen fully using the system to prove the direction.
...
- replace the Openrind Desktop mood described in `DESIGN-LANGUAGE.md`
