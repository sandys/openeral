...
Openrind Desktop Cloud needs an MCP server for `ee/apps/den-api` so users and agents can interact with the Den control-plane API through MCP clients.
...
- A small Openrind Desktop-owned OpenAPI-to-MCP adapter that builds tools from the current Den OpenAPI document and dispatches tool calls through the existing Hono app.
...
-> starts OAuth authorization with resource=https://api.openrind-desktoplabs.com/mcp
...
```text
https://api.openrind-desktoplabs.com/mcp
```
...
- OAuth resource/audience is `https://api.openrind-desktoplabs.com/mcp`.
...
```json
{
  "https://openrind-desktoplabs.com/token_use": "mcp",
  "https://openrind-desktoplabs.com/org_id": "org_...",
  "https://openrind-desktoplabs.com/resource": "https://api.openrind-desktoplabs.com/mcp"
}
```
