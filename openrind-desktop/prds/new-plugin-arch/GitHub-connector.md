...
- ingest matching files into Openrind Desktop;
- keep Openrind Desktop in sync when GitHub sends webhook events.
...
- Openrind Desktop stores a connector account, connector instance, target repo, and path mappings;
- GitHub webhooks notify Openrind Desktop that the selected branch changed;
- Openrind Desktop then reconciles against the current branch head state;
- Openrind Desktop does not treat individual changed files in the webhook payload as the final source of truth.
...
- authorize Openrind Desktop to see which installations/repos they can use;
...
### Openrind Desktop connector records
...
Inside Openrind Desktop, an admin creates a GitHub connector instance.
...
After setup, Openrind Desktop should run an initial full reconciliation against the selected branch head.
...
Openrind Desktop must verify the GitHub App webhook secret on every incoming webhook delivery.
...
Openrind Desktop should fetch repo state at the selected branch head commit.
...
- GitHub App installation gives Openrind Desktop repo access and webhooks;
...
- Openrind Desktop reads the current branch-head repo state and ingests from that state;
- Openrind Desktop compares desired current files against existing bindings to create, update, delete, and tombstone objects;
