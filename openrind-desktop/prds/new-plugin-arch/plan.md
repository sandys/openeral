...
- Cloud: created or edited directly in Openrind Desktop Cloud.
...
- cloud/import objects are first-party editable in Openrind Desktop;
- connector-managed objects may still be editable in Openrind Desktop by users with permission, but connector sync remains authoritative and may overwrite content on the next sync;
- Openrind Desktop must keep enough source metadata to re-ingest, reconcile, audit, and display provenance.
...
- Openrind Desktop stores the repo binding;
...
At minimum, Openrind Desktop should know:
