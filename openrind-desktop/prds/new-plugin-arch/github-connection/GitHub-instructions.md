...
4. GitHub redirects back to Openrind Desktop.
5. Openrind Desktop shows the repositories visible to that installation.
...
- App name: choose any unique name, for example `Openrind Desktop Den Local`
...
```bash
pnpm --filter @openrind-ee/den-api dev
pnpm --filter @openrind-ee/den-web dev
```
