...
4. let the user choose which discovered plugins should map into Openrind Desktop;
5. translate the selected discovery result into Openrind Desktop connector records and future ingestion work.
...
- how the result maps into Openrind Desktop internal structures.
...
It is not enough to understand the shape of the repo and convert that shape into useful Openrind Desktop mappings.

The discovery phase fills that gap.

Instead of immediately asking the user to author raw path mappings, Openrind Desktop should first inspect the repo and propose a structured interpretation of what it found.
...
3. Openrind Desktop creates the connector instance and target.
4. Openrind Desktop routes the user into a dedicated `Setup` / `Discovery` page for that connector instance.
5. Openrind Desktop reads the repository tree and shows progress steps in the UI.
6. Openrind Desktop classifies the repo shape.
7. Openrind Desktop shows discovered plugins, preselected by default.
8. User confirms or deselects discovered plugins.
9. Openrind Desktop creates the initial connector mappings and plugin records from that discovery result.
10. Openrind Desktop is ready for initial ingestion/sync.
...
That means Openrind Desktop discovery should treat marketplace repos as a first-class shape, but be explicit about what is in-scope for a connected single repo.
...
This keeps the behavior honest and still lets users understand what Openrind Desktop detected.
...
- a short explanation that Openrind Desktop is figuring out how to map this repo.
...
- what Openrind Desktop looked for;
...
- one Openrind Desktop `plugin` row;
...
## Mapping discovered plugins to Openrind Desktop internal data
...
- create Openrind Desktop plugin `Sales`
- create Openrind Desktop plugin `finance`
- create mappings:
...
- create one Openrind Desktop plugin from the marketplace/plugin metadata;
...
5. Openrind Desktop creates plugin rows + connector mappings
6. Openrind Desktop enqueues initial full sync
...
- hooks-to-Openrind Desktop runtime semantics beyond discovery;
...
4. Should root-level `plugin.json` remain a metadata hint only, or do we want to formalize it as an Openrind Desktop-specific compatibility rule?
