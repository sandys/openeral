...
- Reusable client package: `packages/openrind-desktop-server-sdk`
...
- `packages/openrind-desktop-server-sdk/src/streams/**`
...
- `packages/openrind-desktop-server-sdk`: generated Server V2 client plus small handwritten SSE helpers for Server V2
...
### `packages/openrind-desktop-server-sdk/package.json`
...
```json
{
  "scripts": {
    ...
    "sdk:generate": "pnpm --filter openrind-desktop-server-v2 openapi:generate && pnpm --filter @openrind/server-sdk generate"
  }
}
```
...
- `packages/openrind-desktop-server-sdk/scripts/watch.mjs`
...
```bash
pnpm --filter openrind-desktop-server-v2 openapi:generate && pnpm --filter @openrind/server-sdk generate && git diff --exit-code
```
...
- SSE helpers: handwritten in `packages/openrind-desktop-server-sdk/src/streams/**`
