...
```bash
OPENRIND_DESKTOP_UI_USE_SERVER_V2=1 pnpm dev:server-v2
```
...
-> SDK watch regenerates packages/openrind-desktop-server-sdk/generated/**
...
To keep iteration fast, `packages/openrind-desktop-server-sdk` should ideally work like this in development:
...
In the ideal product model, most user-facing app traffic should still target the local Openrind Desktop server as the canonical adapter and registry. Direct alternate `serverId` targeting is mainly for migration, testing, and explicit server-management flows.
...
- expose small handwritten SSE helpers from `packages/openrind-desktop-server-sdk`
