...
This document captures the desired end-state runtime model for Openrind Desktop Server V2.
...
```text
DesktopApp
-> starts local Openrind Desktop server
...
-> talks to remote Openrind Desktop servers for remote workspaces
```

## Server And Workspace Registry Model

The local Openrind Desktop server should maintain the canonical registry of servers and workspaces.
...
- launch the local Openrind Desktop server
...
The local Openrind Desktop server becomes the primary owner of:
...
- remote Openrind Desktop workspace connections
...
This database is the source of truth for Openrind Desktop-managed metadata and relationships.
...
- `skills`
  - skill metadata and Openrind Desktop-managed ownership
- `plugins`
...
The sqlite DB is for Openrind Desktop-owned metadata.
...
- belongs to the local Openrind Desktop server
- has a stable Openrind Desktop workspace ID
- maps to exactly one OpenCode project
- has its own Openrind Desktop-managed config directory
...
- belongs to a remote Openrind Desktop server
- maps to exactly one workspace on that remote Openrind Desktop server
...
- Openrind Desktop owns the workspace record
- Openrind Desktop workspace ID remains separate from the OpenCode project ID
- OpenCode owns the underlying session state for that project
- Openrind Desktop server is responsible for translating between workspace IDs and OpenCode project/session identifiers
...
Each remote workspace should map to a single workspace on a remote Openrind Desktop server.

That means:

- the local UI does not treat a remote workspace as a direct OpenCode connection
- the local server talks to the remote Openrind Desktop server
- the remote Openrind Desktop server is responsible for its own OpenCode integration

This preserves one product model:

- UI <-> Openrind Desktop server

instead of:

- UI <-> OpenCode directly for some things
- UI <-> Openrind Desktop server for others
...
- one local Openrind Desktop server
- one exposed workspace access surface per workspace
- remote consumers connect to the Openrind Desktop server and are scoped to the workspace they were granted
...
2. Desktop app starts the local Openrind Desktop server.
...
6. The server resolves the workspace to its backend:
   ...
   - remote Openrind Desktop server workspace for remote workspaces
...
`opencode-router` should be started by the new Openrind Desktop server, not by the desktop app.
...
1. Desktop app launches the local Openrind Desktop server.
...
- one router process per local Openrind Desktop server
...
That makes `opencode-router` just another runtime dependency of the Openrind Desktop server, not a separate app-owned or orchestrator-owned control surface.
...
The Openrind Desktop server should query OpenCode via the SDK to get those.

Openrind Desktop should not duplicate the source of truth for those fields in its own sqlite DB unless it needs a cache or index for performance.

## Workspace Config Ownership Model

The config of a workspace should be managed by Openrind Desktop.
...
- plugins
- skills
- any Openrind Desktop-owned workspace settings
...
This lets Openrind Desktop own the config model while still projecting it into the OpenCode format OpenCode needs.
...
- Openrind Desktop-managed config directory
- OpenCode-managed runtime state

When a user adds a new local workspace:

1. Openrind Desktop prompts for the directory the user wants to work with.
2. That chosen directory becomes the workspace data directory.
3. Openrind Desktop creates a separate workspace config directory under an Openrind Desktop-controlled path.
4. OpenCode is pointed at the Openrind Desktop-controlled config directory.
5. The user’s chosen data directory is added as an authorized path in config.

This gives a cleaner split between:

- files owned by the user
- files controlled by Openrind Desktop
...
- parsing those items into Openrind Desktop-owned models
...
For anything related to a session, the client should always talk to the Openrind Desktop server with the workspace ID.

Example shape:

```text
client action
-> Openrind Desktop server request with workspaceId (+ sessionId if needed)
-> server resolves workspace backend
-> local OpenCode project/session OR remote Openrind Desktop workspace
-> response comes back through Openrind Desktop server
```
...
- the server forwards to the remote Openrind Desktop server
...
- Openrind Desktop workspace ID
- backend type
- OpenCode project ID or remote Openrind Desktop workspace ID

## OpenCode Primitive Exposure

To make this work, the Openrind Desktop server will need to expose the OpenCode primitives it depends on.
...
The Openrind Desktop server becomes the adapter and policy layer over those upstream primitives.

The public API exposed to clients should not use those raw session-first routes directly.

Instead, the public API should be normalized around workspace-first Openrind Desktop routes, for example:
...
- projection of Openrind Desktop config into OpenCode workspace config
...
- remote workspace support should normalize around "one workspace on one remote Openrind Desktop server"
...
9. Local workspaces keep a stable Openrind Desktop workspace ID that is separate from the OpenCode project ID, and the server stores the mapping.
10. Session and message primitives should be exposed through workspace-first Openrind Desktop-shaped endpoints, not raw OpenCode endpoint shapes.
...
4. What exact workspace-first Openrind Desktop route naming should wrap the required OpenCode session/message primitives?
