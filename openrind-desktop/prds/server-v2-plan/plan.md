...
For planning purposes in this doc set, `openrind-desktop-server-v2` is the working name for that new package, binary, and runtime bundle. We can rename it later without changing the architecture direction.
...
```text
desktop app or CLI
-> starts or connects to one Openrind Desktop server process
-> Openrind Desktop server owns workspace/runtime/product behavior
-> Openrind Desktop server supervises the local runtime pieces it needs
```
...
- local bootstrap logic that only exists to support the Openrind Desktop runtime
...
The new server should expose Openrind Desktop-shaped routes directly.

Recommendation:

- use workspace-first Openrind Desktop routes as the real public API shape
...
packages/openrind-desktop-server-sdk/
...
- `packages/openrind-desktop-server-sdk` watches `openapi/openapi.json` and regenerates the reusable generated client package
- `packages/openrind-desktop-server-sdk` regenerates the reusable server-agnostic client package
- the app watches its own `createSdk({ serverId })` adapter alongside normal app code
- the app depends on `openrind-desktop-server-sdk` through the workspace so type updates are visible immediately
...
- import or normalize workspace records from current desktop state such as `openrind-desktop-workspaces.json`
...
- hosted Openrind Desktop Cloud server
...
In the ideal model, the local Openrind Desktop server owns the durable mapping between:

- Openrind Desktop workspace ID
- remote Openrind Desktop workspace ID
- OpenCode project ID
- backend server identity

The app should usually operate on stable Openrind Desktop workspace IDs returned by the local server, not on remote backend IDs directly.
...
In the ideal steady state, normal product traffic should target the local Openrind Desktop server as the canonical adapter and registry, while direct alternate `serverId` targeting is reserved for explicit server-management or migration/testing scenarios.
...
- the Openrind Desktop server should still be the only streaming surface the app talks to
