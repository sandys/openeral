...
- Called from and when: called when the `openrind-desktop-server` binary or `bun src/cli.ts` is launched.
...
- What it does: creates starter `.opencode` state, commands, skills, agent, `opencode.json`, and `openrind-desktop.json`.
- Called from and when: called when creating a local workspace.
- What it calls: Openrind Desktop/OpenCode starter file generation.
...
### `ensureWorkspaceOpenrindDesktopConfig`

- What it is: Openrind Desktop config seeder.
- What it does: seeds `openrind-desktop.json` with authorized roots, blueprint sessions, and workspace metadata.
- Called from and when: called during `ensureWorkspaceFiles`.
- What it calls: Openrind Desktop-specific workspace behavior and starter session metadata.

### workspace config routes

- What they are: config read/patch/raw text endpoints.
- What they do: read or patch workspace `opencode` and `openrind-desktop` config, including raw OpenCode config editor flows.
...
- What they do: manage uploadable inbox files and downloadable artifact files under `.opencode/openrind-desktop`.
...
- What they do: publish and fetch named bundle payloads via a trusted Openrind Desktop publisher.
...
- What they call: audit persistence under Openrind Desktop data directories.

### blueprint session helpers

- What they are: blueprint template normalization/materialization helpers.
- What they do: parse starter-session templates from `openrind-desktop.json`, track what was already materialized, create OpenCode sessions, and seed starter messages.
- Called from and when: called by blueprint/session materialization routes and workspace bootstrap flows.
- What they call: upstream OpenCode session creation, direct OpenCode DB seeding, and `openrind-desktop.json` updates.
