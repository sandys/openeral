...
The local Openrind Desktop server should become the durable owner of product state that is not inherently owned by OpenCode.

That means:

- OpenCode owns session/project runtime state
- Openrind Desktop server owns workspace registry, config registry, remote connections, and product metadata
- the desktop app should stop being the long-term owner of those records

## Database Scope

The sqlite DB is for Openrind Desktop-owned metadata and relationships.
...
Those should still come live from OpenCode or remote Openrind Desktop servers.
...
- Remote and cloud servers should both be modeled as remote Openrind Desktop servers at the product level; `hosting_kind` captures whether a remote server is cloud-hosted or not.

### `workspaces`

Purpose:

- store the canonical Openrind Desktop workspace registry
...
- local workspaces keep a stable Openrind Desktop workspace ID that is separate from the OpenCode project ID
- remote workspaces map to exactly one workspace on a remote Openrind Desktop server
- control/help workspaces are real workspaces with `is_hidden = true`
- the local Openrind Desktop server is still the canonical routing layer that the app talks to, even when a workspace belongs to a remote server
...
- `workspace_id`
- `backend_kind` (`local_opencode`, `remote_openrind-desktop`)
- `last_sync_at`
...
- store MCP definitions and related auth/config metadata as Openrind Desktop-managed records
...
- `config_json`
- `auth_json`
- `source` (`openrind_desktop_managed`, `imported`, `discovered`)
- `created_at`
- `updated_at`
...
- store Openrind Desktop-managed skill metadata
...
- `source` (`openrind_desktop_managed`, `imported`, `discovered`, `cloud_synced`)
...
These tables are what let Openrind Desktop own config items once and apply them to one or many workspaces.
...
- local Openrind Desktop workspace ID <-> OpenCode project ID mappings
...
- local server queries remote Openrind Desktop servers for remote workspaces
...
- Openrind Desktop creates and owns the config directory
- OpenCode is pointed at the Openrind Desktop-owned config directory
- the user data directory is added as an authorized path

This gives a clean separation between:

- user-owned data
- Openrind Desktop-owned config
- OpenCode-owned runtime state

## API Shape

All session and workspace behavior should be exposed through workspace-first Openrind Desktop routes.

That means:

- the client sends `workspaceId`
- the server resolves the backend mapping
- the server talks to OpenCode or a remote Openrind Desktop server
- the client never needs raw OpenCode endpoint shapes

Recommended principle:

- normalize everything into Openrind Desktop-shaped endpoints
- do not expose raw OpenCode endpoint design directly as the public product API

## Initial Primitive Surface

The initial OpenCode-derived session surface should still include the primitives already listed in `prds/server-v2-plan/ideal-flow.md`, but wrapped in Openrind Desktop route design.
...
- mapping rules from Openrind Desktop routes to OpenCode SDK calls
