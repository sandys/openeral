...
The orchestrator is the host-side runtime manager for Openrind Desktop.
...
into a running Openrind Desktop worker stack.

Today it is responsible for things like:

- starting and supervising `opencode`
- starting and supervising `openrind-desktop-server`
...
3. It starts and supervises OpenCode, Openrind Desktop server, and optional router.
...
- Called from and when: process entrypoint for the `openrind-desktop` CLI.
...
- Called from and when: called by `main()` for `openrind-desktop start` and `openrind-desktop serve`; also used indirectly when desktop detached mode shells out to `openrind-desktop start --detach`.
- Ends up calling: OpenCode startup, Openrind Desktop server startup, optional OpenCodeRouter startup, sandbox/container logic, health loops, owner token issuance, and runtime control server setup.
...
- `startOpencode()` -> `Split`
- `startOpenrindDesktopServer()` -> `Split`
...
### `startOpenrindDesktopServer()`

- What it does: launches `openrind-desktop-server` with the bootstrap information needed for host mode today.
- Called from and when: called from `runStart()` and restart paths.
- Ends up calling: the main Openrind Desktop API/control layer process; over time this bootstrap contract should shrink toward the minimum needed to bring the server up.
...
- Ends up calling: rolling restart of OpenCode, Openrind Desktop server, and OpenCodeRouter.
...
Reasoning: in the current orchestrator these are host/bootstrap concerns, but the Server V2 target changes the default distribution model. When the canonical runtime is `openrind-desktop-server-v2` bundling and extracting its own sidecars, the equivalent resolution logic should move into the server for that path. A thinner host shell may still need fallback or external-runtime resolution in some modes, so this boundary is better treated as `Split` than `Stay`.

### `resolveOpenrindDesktopServerBin()`

- What it does: decides which `openrind-desktop-server` binary to run.
...
- Called from and when: called by `openrind-desktop daemon run`; desktop also spawns it for desktop host mode.
...
- Called from and when: called by `openrind-desktop workspace ...`.
- Ends up calling: daemon HTTP API.

### `runInstanceCommand()`

- What it does: thin CLI wrapper for instance disposal commands.
- Called from and when: called by `openrind-desktop instance dispose ...`.
- Ends up calling: daemon HTTP API.
...
### `issueOpenrindDesktopOwnerToken()`
...
- What they do: read and write `openrind-desktop-orchestrator-state.json` and related state snapshots.
...
- What it does: writes the in-container boot script that starts OpenCode, optional router, then Openrind Desktop server.
...
- Called from and when: called by `openrind-desktop start --check` and related flags.
- Ends up calling: broad end-to-end runtime verification.

### `runStatus()`

- What it does: reports current Openrind Desktop/OpenCode URLs and related status in human-readable form.
- Called from and when: called by `openrind-desktop status`.
...
Reasoning: these are not orchestrator responsibilities. They are convenience clients over `openrind-desktop-server` APIs.

### `runFiles()`

- What it does: exposes CLI wrappers for file session creation, read/write, catalog, events, mkdir, delete, and rename.
- Called from and when: called by `openrind-desktop files ...`.
- Ends up calling: Openrind Desktop server file-session APIs.

### `runApprovals()`

- What it does: exposes CLI wrappers for approvals listing and replies.
- Called from and when: called by `openrind-desktop approvals ...`.
- Ends up calling: Openrind Desktop server approvals APIs.
...
```text
desktop app or CLI
-> one Openrind Desktop server process
...
```text
desktop app
-> launches or connects to Openrind Desktop server
