...
- Server V2 should be enabled explicitly with `OPENRIND_DESKTOP_UI_USE_SERVER_V2=1` (or the Vite-exposed equivalent in the frontend build).
...
```text
OPENRIND_DESKTOP_UI_USE_SERVER_V2=1
```
...
export function checkServerFlag() {
  return import.meta.env.VITE_OPENRIND_DESKTOP_UI_USE_SERVER_V2 === "1"
}
```
...
- direct fetches to current Openrind Desktop server endpoints
