...
- one `opencode-router` child per local Openrind Desktop server
...
packages/openrind-desktop-server-sdk/generated/**
packages/openrind-desktop-server-sdk/src/index.ts
...
- `packages/openrind-desktop-server-sdk`: watch `openapi/openapi.json`, regenerate the reusable generated client package
- `apps/app`: watch the app-side `createSdk({ serverId })` adapter alongside normal app code
- `packages/openrind-desktop-server-sdk`: optional watch build if the package publishes built output
...
The local Openrind Desktop server should maintain the durable registry of servers and workspaces. The app should render or cache what the server returns.
...
- local desktop-hosted Openrind Desktop server
- remote worker-backed Openrind Desktop server
...
- `id` is the stable Openrind Desktop workspace identifier the UI uses
...
In the ideal steady state, normal app traffic should still flow through the local Openrind Desktop server using stable Openrind Desktop workspace IDs, with remote Openrind Desktop workspace IDs and OpenCode project IDs remaining server-owned mappings.
...
The app should consume OpenCode-related streaming only through the Openrind Desktop server.
...
- expose small handwritten streaming helpers from `packages/openrind-desktop-server-sdk`
...
- whether the working name `openrind-desktop-server-v2` survives to ship time or is renamed before release
