...
4. The desktop layer starts or reconnects to the local Openrind Desktop server host path.
...
- Ends up calling: child-process kill paths for the engine, orchestrator, Openrind Desktop server, and router.
...
- What it does: renames the macOS process to `Openrind Desktop - Dev` in dev mode.
...
Reasoning: the desktop app should keep the ability to launch the Openrind Desktop server, but OpenCode runtime ownership and coordination should move behind the server boundary over time.
...
- Ends up calling: local project-directory setup, `opencode.json` seeding, port selection, credential generation, engine spawn, Openrind Desktop server startup, and OpenCodeRouter startup.
...
- Ends up calling: child-process shutdown/restart across engine, router, orchestrator, and hosted Openrind Desktop server.
...
- What it does: launches the Openrind Desktop orchestrator daemon sidecar.
...
- Ends up calling: `openrind-desktop-orchestrator start --detach`, token generation, health polling, optional owner-token issuance, and progress event emission.

### `sandbox_doctor()`, `sandbox_stop()`, `sandbox_cleanup_openrind_desktop_containers()`, `sandbox_debug_probe()`
...
## Hosted Openrind Desktop Server Lifecycle

Disposition guidance:

- `start_openrind_desktop_server()` -> `Split`
- `spawn_openrind_desktop_server()` -> `Stay`
- `resolve_openrind_desktop_port()` -> `Stay`
- token and state helpers in `openrind_desktop_server/mod.rs` -> `Split`
- `build_urls()` -> `Stay`
- `openrind_desktop_server_info()` / `openrind_desktop_server_restart()` -> `Split`
- `OpenrindDesktopServerManager` -> `Split`

Reasoning: the desktop shell should keep launch/supervision of the local Openrind Desktop server process, but token semantics, runtime info, and restart/control behavior should keep shrinking into the server over time.

### `start_openrind_desktop_server()`

- What it does: starts the local desktop-hosted Openrind Desktop server and tracks its tokens, URLs, and health.
- Called from and when: called by engine startup and explicit Openrind Desktop server restart flows.
...
### `spawn_openrind_desktop_server()`

- What it does: launches the `openrind-desktop-server` sidecar.
- Called from and when: called by `start_openrind_desktop_server()`.
- Ends up calling: child-process spawn, cwd selection, and env var injection including OpenCode base URL, OpenCode creds, and Openrind Desktop tokens.

### `resolve_openrind_desktop_port()`

- What it does: picks an Openrind Desktop server port with reuse and conflict avoidance.
- Called from and when: called on host-mode server start.
- Ends up calling: local TCP port selection logic.

### token and state helpers in `openrind_desktop_server/mod.rs`
...
### `openrind_desktop_server_info()` / `openrind_desktop_server_restart()`

- What they do: expose Openrind Desktop server runtime info and restart it with the current engine credentials/workspaces.
...
- authorized-root and `openrind-desktop.json` read/write helpers -> `Move`
...
- Ends up calling: `openrind-desktop-workspaces.json` reads/writes and path canonicalization; in the ideal model this becomes transitional reconnect/cache state rather than the canonical registry.

### `workspace_create()`

- What it does: creates a new local workspace and seeds its initial Openrind Desktop/OpenCode files.
...
### `workspace_add_authorized_root()`, `workspace_openrind_desktop_read()`, `workspace_openrind_desktop_write()`

- What they do: manage `.opencode/openrind-desktop.json` data for authorized roots and related settings.
- Called from and when: called from settings/permissions UI.
- Ends up calling: workspace-local config file reads and writes; in the ideal model this is server-side materialization into the Openrind Desktop-managed workspace config directory.
...
- What they do: register `openrind-desktop://` and `openrind-desktop-dev://`, receive deep links, and forward them into the UI.
...
- `reset_openrind_desktop_state()` -> `Split`
- `nuke_openrind_desktop_and_opencode_config_and_exit()` -> `Split`
...
### `reset_openrind_desktop_state()`

- What it does: stops managed services and clears Openrind Desktop desktop state.
...
### `nuke_openrind_desktop_and_opencode_config_and_exit()`

- What it does: performs a broader local reset across Openrind Desktop and OpenCode state, then exits.
