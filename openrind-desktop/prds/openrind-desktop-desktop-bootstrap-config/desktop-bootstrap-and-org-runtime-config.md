# Openrind Desktop Desktop Bootstrap And Org Runtime Config

## Goal

Support three desktop distribution modes while still allowing all of them to update from the default app build:

1. Default build pointing at the default Openrind Desktop server with no forced sign-in.
2. Custom build pointing at the default Openrind Desktop server with forced sign-in.
...
- `_repos/openrind-desktop/_worktrees/Desktop-config-den-endpoint/apps/app/src/app/lib/den.ts`
...
```bash
OPENRIND_DESKTOP_DESKTOP_DEN_BASE_URL="https://client.example.com" \
OPENRIND_DESKTOP_DESKTOP_DEN_API_BASE_URL="https://client.example.com/api/den" \
OPENRIND_DESKTOP_DESKTOP_DEN_REQUIRE_SIGNIN="1" \
pnpm --filter @openrind/desktop build
```

### Accepted boolean values

`OPENRIND_DESKTOP_DESKTOP_DEN_REQUIRE_SIGNIN` and `VITE_DEN_REQUIRE_SIGNIN` are treated as enabled when set to one of:
...
For the current external bootstrap-file approach, the desktop shell should look for `desktop-bootstrap.json` in the shared Openrind Desktop config directory, unless an explicit override path is provided with `OPENRIND_DESKTOP_DESKTOP_BOOTSTRAP_PATH`.

Expected default locations:

- All platforms: `~/.config/openrind-desktop/desktop-bootstrap.json`
...
- `server.baseUrl`: default Openrind Desktop server
- `requireSignin`: `false`
