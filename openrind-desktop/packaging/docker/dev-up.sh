...
#
# Usage (from _repos/openrind-desktop repo root):
#   packaging/docker/dev-up.sh
#
# Defaults to isolated OpenCode dev state inside the container.
# Escape hatch: set OPENRIND_DESKTOP_DOCKER_DEV_MOUNT_HOST_OPENCODE=1 to import host
# OpenCode config/auth into the isolated dev state for this stack.
#
# Outputs:
# - Web UI URL
# - OpenrindDesktop server URL
# - Share service URL
# - Token file path
...
MOUNT_HOST_OPENCODE="${OPENRIND_DESKTOP_DOCKER_DEV_MOUNT_HOST_OPENCODE:-0}"

resolve_opencode_config_dir() {
  local override="${OPENRIND_DESKTOP_OPENCODE_CONFIG_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: OPENRIND_DESKTOP_OPENCODE_CONFIG_DIR is not a directory: $override" >&2
  fi
...
resolve_opencode_data_dir() {
  local override="${OPENRIND_DESKTOP_OPENCODE_DATA_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: OPENRIND_DESKTOP_OPENCODE_DATA_DIR is not a directory: $override" >&2
  fi
...
detect_public_host() {
  if [ -n "${OPENRIND_DESKTOP_PUBLIC_HOST:-}" ]; then
    printf '%s\n' "$OPENRIND_DESKTOP_PUBLIC_HOST"
    return
  fi
...
PROJECT="openrind-desktop-dev-$DEV_ID"
...
OPENRIND_DESKTOP_PORT="$(pick_port)"
WEB_PORT="$(pick_port)"
if [ "$WEB_PORT" = "$OPENRIND_DESKTOP_PORT" ]; then
  WEB_PORT="$(pick_port)"
fi
SHARE_PORT="${SHARE_PORT:-$(pick_port)}"
if [ "$SHARE_PORT" = "$OPENRIND_DESKTOP_PORT" ] || [ "$SHARE_PORT" = "$WEB_PORT" ]; then
...
echo "- OPENRIND_DESKTOP_PORT=$OPENRIND_DESKTOP_PORT" >&2
echo "- WEB_PORT=$WEB_PORT" >&2
echo "- SHARE_PORT=$SHARE_PORT" >&2
echo "- OPENRIND_DESKTOP_DEV_MODE=1" >&2
if [ "$MOUNT_HOST_OPENCODE" = "1" ]; then
...
start_stack() {
  local config_dir="$1"
  local data_dir="$2"
  OPENRIND_DESKTOP_DEV_ID="$DEV_ID" OPENRIND_DESKTOP_PORT="$OPENRIND_DESKTOP_PORT" WEB_PORT="$WEB_PORT" SHARE_PORT="$SHARE_PORT" \
    OPENRIND_DESKTOP_DEV_MODE="1" \
    OPENRIND_DESKTOP_PUBLIC_HOST="$PUBLIC_HOST" \
    OPENRIND_DESKTOP_HOST_OPENCODE_CONFIG_DIR="$config_dir" \
    OPENRIND_DESKTOP_HOST_OPENCODE_DATA_DIR="$data_dir" \
...
echo "- OPENRIND_DESKTOP_HOST_OPENCODE_CONFIG_DIR=$ACTIVE_OPENCODE_CONFIG_DIR" >&2
echo "- OPENRIND_DESKTOP_HOST_OPENCODE_DATA_DIR=$ACTIVE_OPENCODE_DATA_DIR" >&2
...
echo "" >&2
echo "OpenrindDesktop web UI:     http://localhost:$WEB_PORT" >&2
echo "OpenrindDesktop web UI (LAN/public): http://$PUBLIC_HOST:$WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "OpenrindDesktop web UI (LAN IP):     http://$LAN_IPV4:$WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "OpenrindDesktop web UI (Tailscale):  http://$TAILSCALE_DNS_NAME:$WEB_PORT" >&2
fi
echo "OpenrindDesktop server:     http://localhost:$OPENRIND_DESKTOP_PORT" >&2
echo "OpenrindDesktop server (LAN/public): http://$PUBLIC_HOST:$OPENRIND_DESKTOP_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "OpenrindDesktop server (LAN IP):     http://$LAN_IPV4:$OPENRIND_DESKTOP_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "OpenrindDesktop server (Tailscale):  http://$TAILSCALE_DNS_NAME:$OPENRIND_DESKTOP_PORT" >&2
fi
