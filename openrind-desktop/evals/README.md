# Openrind Desktop UI evals

Human-readable scenarios that an LLM (or a person) can replay against a live
Openrind Desktop instance to verify end-to-end behavior of the UI.
...
(OpenCode + Openrind Desktop server + React UI) so regressions in wiring — not just
types — get caught.
...
4. Stop the stack with the exact `docker compose -p openrind-desktop-dev-... down`
