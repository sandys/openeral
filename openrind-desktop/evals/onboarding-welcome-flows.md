...
1. Reset onboarding state so the welcome screen appears:
   - Open DevTools console and run:
     ```js
     const raw = localStorage.getItem("openrind-desktop.preferences");
     const prefs = raw ? JSON.parse(raw) : {};
     prefs.hasCompletedOnboarding = false;
     localStorage.setItem("openrind-desktop.preferences", JSON.stringify(prefs));
     location.reload();
     ```
...
**Why**: When a user opens Openrind Desktop for the first time with zero
...
4. Expect: full-screen page renders with:
   - "Welcome to Openrind Desktop" heading.
...
Pass criteria:
- URL is `/welcome`.
- Heading "Welcome to Openrind Desktop" is visible.
...
3. Expect: the local panel shows:
   ...
   - Explanation text: "This folder becomes your workspace. Openrind Desktop
     will be able to:"
...
**Why**: Users connecting to a remote Openrind Desktop server from the welcome
flow should also have onboarding marked complete.

Steps:
1. From `/welcome`, click "Get started".
2. Click "Connect custom remote".
3. Enter a valid Openrind Desktop server URL.
