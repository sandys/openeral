...
- Openrind Desktop server proxy endpoints for `/w/:workspaceId/opencode/session/**`

## Preflight

Before running any eval:

1. Install dependencies in the Openrind Desktop repo:
   ```bash
   pnpm install --frozen-lockfile
   ```
2. Start the Electron dev app:
   ```bash
   nohup pnpm dev:electron > .openrind-desktop-run/electron.log 2>&1 &
   ```
   Wait ~15s for Vite + Electron to boot. The log will show
   `[openrind-desktop] Electron CDP exposed at http://127.0.0.1:9823` when ready.
3. Chrome MCP automatically connects to the Electron renderer via CDP on
   port 9823. Verify by taking a snapshot:
   ```
   chrome-devtools_take_snapshot
   ```
   You should see the full Openrind Desktop UI tree (sidebar, composer, footer).
4. Confirm the footer shows **"Openrind Desktop Ready"**.
