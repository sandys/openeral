...
Openrind Desktop helps individual create, consume, and maintain their agentic workflows.

Openrind Desktop helps companies share their agentic workflows and provision their entire team.
...
- In Openrind Desktop Cloud (internal name is Den).

Where are workflow hosted:
- Local Machine
- Remote via a Openrind Desktop Host (CLI or desktop)
- Remote on Openrind Desktop Cloud (via Den sandbox workers)

## Current Openrind Desktop Cloud flow
...
- After a worker is ready, the user connects from the Openrind Desktop app with `Add a worker` -> `Connect remote`, or opens the generated deep link directly.
...
How to decide if Openrind Desktop should do something:
