...
/// 2. An environment variable named by `env_var` (e.g. `"OPENRIND_SHELL_DATABASE_URL"`).
/// 3. The first connection entry in `~/.openrind-shell/config.yml`.
///
...
pub fn resolve_connection_string(cli_arg: Option<&str>, env_var: &str) -> Result<String, FsError> {
    ...
    // 3. Config file at ~/.openrind-shell/config.yml
    if let Some(home) = home_dir() {
        let config_path = home.join(".openrind-shell").join("config.yml");
    ...
    Err(FsError::InvalidArgument(
        "No connection string provided. Pass one via CLI, set the environment variable, \
         or configure ~/.openrind-shell/config.yml"
            .to_string(),
    ))
}
