...
//!
//! When the system calls `mount -t fuse.openrind-shell <source> <mountpoint>`,
//! mount.fuse3 invokes openrind-shell in one of two ways:
//!
//! Without drop_privileges:
//!   openrind-shell <source> <mountpoint> -o <options>
//!   (openrind-shell opens /dev/fuse and mounts itself)
//!
//! With drop_privileges:
//!   openrind-shell <source> /dev/fd/N -o <options>
//!   (mount.fuse3 pre-opens /dev/fuse, openrind-shell uses the fd)
...
///
/// mount.fuse3 calls: `openrind-shell <source> <mountpoint> -o <opts>`
...
/// Format:
///   - Database: `<connstr>`
///   - Database (env): `env` (reads OPENRIND_SHELL_DATABASE_URL from environment)
...
enum MountMode {
    ...
    PreOpenedFd(OwnedFd),
    /// Regular mountpoint path — use mount2() (same as direct `openrind-shell mount`)
    MountPoint(PathBuf),
}

/// Entry point for mount.fuse3 invocation.
///
/// Without drop_privileges: openrind-shell <source> <mountpoint> [-o options]
/// With drop_privileges:    openrind-shell <source> /dev/fd/N [-o options]
pub async fn execute() -> Result<(), FsError> {
    let args: Vec<String> = std::env::args().collect();

    if args.len() < 3 {
        return Err(FsError::InvalidArgument(
            "mount.fuse3 invocation requires: openrind-shell <source> <mountpoint> [-o options]"
                .to_string(),
        ));
    }
...
async fn execute_database(
    ...
    let conn_str = resolve_connection_string(cli_arg, "OPENRIND_SHELL_DATABASE_URL")?;
    ...
    match mount_mode {
        ...
        MountMode::MountPoint(mount_point) => {
            ...
            let mut mount_options = vec![
                fuser::MountOption::FSName("openrind-shell".to_string()),
                fuser::MountOption::Subtype("openrind-shell".to_string()),
