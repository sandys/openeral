...
async fn execute_create(args: CreateArgs) -> Result<(), FsError> {
    let conn_str = resolve_connection_string(args.connection.as_deref(), "OPENRIND_SHELL_DATABASE_URL")?;
...
async fn execute_mount(args: MountWorkspaceArgs) -> Result<(), FsError> {
    ...
    fuse_config.mount_options = vec![
        fuser::MountOption::FSName("openrind-shell-workspace".to_string()),
...
async fn execute_list() -> Result<(), FsError> {
    let conn_str = resolve_connection_string(None, "OPENRIND_SHELL_DATABASE_URL")?;
    let pool = create_pool(&conn_str, 30)?;
