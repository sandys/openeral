...
pub async fn create_workspace(
    ...
    client
        .execute(
            "INSERT INTO _openrind.workspace_config (id, display_name, config) VALUES ($1, $2, $3)",
...
pub async fn get_workspace(
    ...
    let row = client
        .query_opt(
            "SELECT id, display_name, config, created_at, updated_at FROM _openrind.workspace_config WHERE id = $1",
            &[&id],
        )
        .await?
...
pub async fn list_workspaces(
    ...
    let rows = client
        .query(
            "SELECT id, display_name, config, created_at, updated_at FROM _openrind.workspace_config ORDER BY id",
            &[],
        )
        .await?;
...
pub async fn delete_workspace(pool: &deadpool_postgres::Pool, id: &str) -> Result<(), FsError> {
    ...
    let count = client
        .execute(
            "DELETE FROM _openrind.workspace_config WHERE id = $1",
            &[&id],
        )
        .await?;
...
pub async fn create_file(
    ...
    let exists = client
        .query_opt(
            "SELECT 1 FROM _openrind.workspace_files WHERE workspace_id = $1 AND path = $2",
...
pub async fn update_file_attrs(
    ...
    if let Some(m) = mode {
        client
            .execute(
                "UPDATE _openrind.workspace_files SET mode = $3, ctime_ns = $4 \
                 WHERE workspace_id = $1 AND path = $2",
    ...
    if let Some(mt) = mtime_ns {
        client
            .execute(
                "UPDATE _openrind.workspace_files SET mtime_ns = $3 \
                 WHERE workspace_id = $1 AND path = $2",
    ...
    if let Some(at) = atime_ns {
        client
            .execute(
                "UPDATE _openrind.workspace_files SET atime_ns = $3 \
                 WHERE workspace_id = $1 AND path = $2",
...
pub async fn delete_file(
    ...
    let count = client
        .execute(
            "DELETE FROM _openrind.workspace_files WHERE workspace_id = $1 AND path = $2 AND is_dir = false",
            &[&workspace_id, &path],
        )
        .await?;
...
pub async fn delete_directory(
    ...
    let has_children = client
        .query_opt(
            "SELECT 1 FROM _openrind.workspace_files \
    ...
    let count = client
        .execute(
            "DELETE FROM _openrind.workspace_files WHERE workspace_id = $1 AND path = $2 AND is_dir = true",
            &[&workspace_id, &path],
        )
        .await?;
...
pub async fn rename_file(
    ...
    client
        .execute(
            "DELETE FROM _openrind.workspace_files WHERE workspace_id = $1 AND path = $2",
