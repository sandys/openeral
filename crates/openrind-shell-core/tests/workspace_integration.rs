...
use openrind_shell_core::db::queries::workspace as ws_queries;
use openrind_shell_core::db::types::{WorkspaceFile, WorkspaceLayout};
...
async fn test_create_file_exists_error() {
    ...
    assert!(matches!(err, openrind_shell_core::error::FsError::FileExists));
...
async fn test_delete_file() {
    ...
    let err = ws_queries::get_file(&pool, &ws_id, "/todelete.txt")
        .await
        .unwrap_err();
    assert!(matches!(err, openrind_shell_core::error::FsError::NotFound));
...
async fn test_delete_nonempty_dir() {
    ...
    assert!(matches!(
        err,
        openrind_shell_core::error::FsError::DirectoryNotEmpty
    ));
...
async fn test_seed_from_directory_uses_default_workspace_owner() {
    ...
    let layout = WorkspaceLayout::default();
    let temp_dir = std::env::temp_dir().join(format!("openrind-shell-seed-{}", ws_id));
