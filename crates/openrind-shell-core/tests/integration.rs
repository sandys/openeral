use openrind_shell_core::db::migrate;
use openrind_shell_core::db::pool::create_pool;
use openrind_shell_core::db::queries::{indexes, introspection, rows, stats};
use openrind_shell_core::fs::cache::MetadataCache;
use openrind_shell_core::fs::inode::{InodeTable, NodeIdentity};
...
async fn test_metadata_cache() {
    ...
    let schemas = vec![openrind_shell_core::db::types::SchemaInfo {
        name: "public".to_string(),
    }];
...
async fn test_format_json() {
    ...
    let json = openrind_shell_core::format::json::format_row(&row).unwrap();
...
async fn test_format_json_rows() {
    ...
    let json = openrind_shell_core::format::json::format_rows(&rows).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&json).unwrap();
...
async fn test_format_csv() {
    ...
    let csv = openrind_shell_core::format::csv::format_row(&row).unwrap();
...
async fn test_format_yaml() {
    ...
    let yaml = openrind_shell_core::format::yaml::format_row(&row).unwrap();
...
async fn test_quote_ident() {
    assert_eq!(
        openrind_shell_core::db::queries::quote_ident("simple"),
        "\"simple\""
    );
    assert_eq!(
        openrind_shell_core::db::queries::quote_ident("has\"quote"),
        "\"has\"\"quote\""
    );
    assert_eq!(
        openrind_shell_core::db::queries::quote_ident("with space"),
        "\"with space\""
    );
}
...
async fn test_pk_display_parsing() {
    use openrind_shell_core::fs::nodes::column::parse_pk_display;
...
async fn test_pk_display_percent_encoding_roundtrip() {
    use openrind_shell_core::db::queries::rows::encode_pk_value;
    use openrind_shell_core::fs::nodes::column::parse_pk_display;
...
#[tokio::test]
async fn test_migrations_create_openrind_schema() {
    ...
    client
        .batch_execute(
            "
        DROP SCHEMA IF EXISTS _openrind CASCADE;
    ...
    // Verify _openrind schema exists
    let client = pool.get().await.unwrap();
    let rows = client
        .query(
            "SELECT schema_name FROM information_schema.schemata WHERE schema_name = '_openrind'",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT table_name FROM information_schema.tables \
             WHERE table_schema = '_openrind' AND table_name = 'mount_log'",
            &[],
        )
        .await
        .unwrap();
    ...
    let rows = client
        .query(
            "SELECT table_name FROM information_schema.tables \
             WHERE table_schema = '_openrind' AND table_name = 'cache_hints'",
            &[],
        )
        .await
        .unwrap();
...
async fn test_log_mount_session() {
    ...
    let client = pool.get().await.unwrap();
    let rows = client
        .query("SELECT mount_point, schemas_filter, page_size, openrind_shell_version FROM _openrind.mount_log", &[])
        .await
        .unwrap();
    assert_eq!(rows.len(), 1);
