...
/**
 * openrind-shell-bash — drop-in bash replacement that routes commands through
 * openrind-shell-js (just-bash + PostgreSQL virtual filesystem).
 *
 * Two modes:
 *
 *   --daemon     Start a persistent daemon on a Unix socket. The daemon holds
 *                the openrind-shell shell instance (PostgreSQL connection, just-bash
...
const SOCKET_PATH = '/tmp/openrind-shell-bash.sock';
...
async function startDaemon() {
  ...
  const isOpenclaw = (process.env.OPENRIND_SHELL_AGENT || '').toLowerCase().includes('openclaw');
  let syncWatch = null;
  if (!isOpenclaw) {
    try {
  ...
  async function shutdown() {
    ...
    try {
      ...
      process.stderr.write(`${formatError(err, 'openrind-shell-bash: shutdown syncFromFs failed')}\n`);
    }
    server.close();
