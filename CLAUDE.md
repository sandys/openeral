...
Above all the per-step budgets sits one hard ceiling, `OPENCLAW_LAUNCH_BUDGET` (default 420s): the launcher hands over the terminal within that window no matter what, dropping to local mode if it runs out. Worst cases of individual steps compose, so the guarantee has to be stated once rather than emerge from the parts.
...
- `ESC[3J` from the agent is always dropped. Erasing the user's scrollback is never needed for the agent's own rendering to be correct.
...
- Kill switch: `OPENRIND_SHELL_PTY_KEEP_SCROLLBACK=0` streams the agent's bytes through verbatim.
