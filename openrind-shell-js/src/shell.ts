 ...
 *
 * Usage:
 *   const shell = await createOpenrindShell({ connectionString, workspaceId })
...
export interface OpenrindShellOptions {
 ...
 */
export async function createOpenrindShell(opts: OpenrindShellOptions): Promise<Bash> {
  ...
  await pool.query(
    `INSERT INTO _openrind.workspace_config (id, display_name, config)
     VALUES ($1, $2, '{}'::jsonb)
