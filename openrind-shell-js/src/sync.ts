...
export const DEFAULT_EXCLUDE_DIRS = new Set(['node_modules', '.git', '.openrind-shell']);
...
export async function syncToFs(
  ...
  const { rows: allRows } = await pool.query(
    `SELECT path, is_dir, content, mode FROM _openrind.workspace_files
...
export async function syncFromFs(
  ...
  const existing = new Map<string, { mtimeNs: bigint; size: bigint; isDir: boolean }>();
  {
    const { rows } = await pool.query(
      `SELECT path, mtime_ns::text AS mtime_ns, size::text AS size, is_dir
       FROM _openrind.workspace_files WHERE workspace_id = $1`,
      [workspaceId],
    );
    for (const row of rows) {
  ...
  if (syncOpts.prune) {
    ...
    if (toDelete.length > 0) {
      // Single batched DELETE FROM _openrind.workspace_files — one round trip
      // for the whole prune set instead of one per stale row.
      await pool.query(
        `DELETE FROM _openrind.workspace_files WHERE workspace_id = $1 AND path = ANY($2::text[])`,
