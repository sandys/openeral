...
describe('pg helper script', () => {
  const tmpDir = '/tmp/openrind-shell-cli-test-' + Date.now();
...
describe('openrind-shell skill shape', () => {
  ...
  it('launches from the published GHCR image, not a local build', () => {
    ...
    expect(skill).not.toMatch(/openrind-shell-sandbox:dev/);
  });

  it('uses openshell-only commands (no npx, no pnpm)', () => {
    expect(skill).not.toMatch(/\bnpx openrind-shell\b/);
  ...
  it('creates generic providers explicitly with KEY=VALUE (not by env lookup)', () => {
    ...
    // generic provider we still wire is `openrind-gateway`.
    expect(skill).toMatch(/openshell provider create --name openrind-gateway[\s\S]*OPENRIND_GATEWAY_API_KEY=/);
...
describe('OpenrindGateway presign URL normalization', () => {
  it('strips the presigned endpoint so Claude appends /v1/messages only once', () => {
    const baseUrl = openrindGatewayProxyBaseUrl(
  ...
  it('accepts an already-normalized proxy base URL', () => {
    expect(
      openrindGatewayProxyBaseUrl('https://proxy.openrind.com/openrind-gateway-proxy/t/example-presign-id'),
    ).toBe('https://proxy.openrind.com/openrind-gateway-proxy/t/example-presign-id');
  });

  it('rejects unexpected OpenrindGateway proxy URL shapes', () => {
    expect(() => openrindGatewayProxyBaseUrl('https://proxy.openrind.com/openrind-gateway-proxy/t/example-presign-id/other')).toThrow(
      /Unexpected OpenrindGateway presign URL shape/,
    );
    expect(() => openrindGatewayProxyBaseUrl('https://api.anthropic.com/v1/messages')).toThrow(
      /Unexpected OpenrindGateway presign URL shape/,
    );
  });
});
...
describe('findRepoRoot', () => {
  it('finds the repo root containing sandboxes/openrind-shell/Dockerfile', () => {
    const root = findRepoRoot();
    expect(root).not.toBeNull();
    // The discovered root must contain the Dockerfile landmark
    const { existsSync } = require('node:fs');
    expect(existsSync(join(root!, 'sandboxes', 'openrind-shell', 'Dockerfile'))).toBe(true);
  });
