 ...
 *
 * `npx openrind-shell` launches an OpenShell sandbox from the openrind-shell image.
 * Inside the sandbox, setup.sh runs migrations, seeds the workspace,
 * starts the openrind-shell-bash daemon, then execs `claude`.
 *
 * Usage:
 *   npx openrind-shell                      # interactive Claude Code (published image)
 *   npx openrind-shell --dev                # interactive Claude Code (local dev image)
 *   npx openrind-shell -d                   # same as --dev (short flag)
 *   npx openrind-shell -- -p 'hello'        # non-interactive
 *   npx openrind-shell --workspace myid     # custom workspace ID
 *   npx openrind-shell optimize stats       # show optimization stats
 *
 * Auth (presign-first):
 *   If ~/.config/openrind-shell/presign.json exists → no env vars required.
 *   If not → set ANTHROPIC_API_KEY + OPENRIND_GATEWAY_API_KEY to create one on first run.
 *   Run `npx openrind-shell presign renew` to replace the stored presign.
 *
 * Required env:
 *   DATABASE_URL            PostgreSQL connection string (Supabase, Neon, etc.)
 *                           Store once with: npx openrind-shell db-url postgresql://...
 *
 * Optional env:
 *   OPENRIND_SHELL_WORKSPACE_ID   Workspace ID (default: openrind-shell-claude, normalized to lowercase)
 *   OPENRIND_SHELL_SANDBOX_IMAGE  Override sandbox image (default: ghcr.io/openrind/openrind-shell/sandbox:just-bash)
 *   OPENRIND_SHELL_DEV_IMAGE      Override dev sandbox image (default: openrind-shell-sandbox:dev, used with --dev)
 *
 * Features:
...
// ---------------------------------------------------------------------------
// Presign persistence — store one permanent presign in ~/.config/openrind-shell/
// ---------------------------------------------------------------------------

interface StoredPresign {
  /** Full URL as returned by OpenrindGateway (e.g. .../t/{JWT}/v1/messages) */
  url: string;
  createdAt: string;
  /** OpenrindGateway API key stored so skill downloads work without the env var */
...
export function openrindGatewayProxyBaseUrl(presignUrl: string): string {
  ...
  if (!/^https:\/\/proxy\.openrind\.com\/openrind-gateway-proxy\/t\/[^/]+$/.test(normalized)) {
    throw new Error('Unexpected OpenrindGateway presign URL shape');
  }
  return normalized;
}

/**
 * Create a new OpenrindGateway presign: never expires, unlimited uses, $10 cost cap.
 * Returns the full presign URL as returned by OpenrindGateway.
 */
async function createPresignUrl(anthropicKey: string, openrindGatewayKey: string): Promise<string> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 second timeout

  try {
    const response = await fetch('https://app.openrind.com/v1/presign', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${openrindGatewayKey}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenrindGateway presign failed (${response.status}): ${text}`);
    }

    const data = await response.json() as { url: string };
    if (!data?.url) throw new Error('OpenrindGateway presign returned no URL');
    return data.url;
  } catch (err) {
    clearTimeout(timeoutId);
    if (err instanceof Error && err.name === 'AbortError') {
      throw new Error('OpenrindGateway presign request timed out after 30 seconds');
    }
    throw err;
  }
}
...
export function parseCliArgs(args: string[]): ParsedArgs {
  ...
  if (args[0] === 'memory') {
    if (args[1] === 'refresh') {
      let workspaceId = process.env.OPENRIND_SHELL_WORKSPACE_ID || 'openrind-shell-claude';
      let projectRoot = '';
      let query = '';
      let dryRun = false;
      let backup = true;
...
function printHelp(): void {
  console.log(`Usage:
  openrind-shell [options] [-- claude-args]    Launch Claude Code (published image)
  openrind-shell --dev [options] [-- args]     Launch Claude Code (local dev image)
  openrind-shell presign                        Show the current OpenrindGateway presign
  openrind-shell presign renew                  Create and store a new OpenrindGateway presign
  openrind-shell stats [options]                Show API usage statistics
  openrind-shell analyze [options]              Analyze session history and suggest optimizations
  openrind-shell apply [options]                Apply optimization suggestions to project files
  openrind-shell memory refresh [options]       Refresh memory system
...
Auth (presign-first model):
  If ~/.config/openrind-shell/presign.json exists, no env vars are required.
  If the presign file is absent, both of these are required on first run:
    ANTHROPIC_API_KEY        Your Anthropic API key (sk-ant-...)
    OPENRIND_GATEWAY_API_KEY       Your OpenrindGateway API key
  The presign is created once and stored permanently — subsequent runs need no keys.
  Run \`npx openrind-shell presign renew\` to replace the stored presign at any time.
...
Optional env:
  OPENRIND_SHELL_WORKSPACE_ID    Default workspace ID (will be normalized to lowercase)
  OPENRIND_SHELL_SANDBOX_IMAGE   Override prod sandbox image (default: ghcr.io/openrind/openrind-shell/sandbox:just-bash)
  OPENRIND_SHELL_DEV_IMAGE       Override dev sandbox image (default: openrind-shell-sandbox:dev, used with --dev/-d)
  OPENRIND_SHELL_AUTO_FIX_TLS    Set to 1 to suppress the TLS regeneration confirmation delay
...
Notes:
  - Presign stored in ~/.config/openrind-shell/presign.json (mode 600, expires_in=-1, max_uses=-1, cost_limit=$10)
...
// ---------------------------------------------------------------------------
// OpenrindGateway org skills
// ---------------------------------------------------------------------------

/**
 * Fetch the org skills bundle from OpenrindGateway and return only the validated entries.
 *
...
export async function fetchOrgSkills(apiKey: string): Promise<Array<{ slug: string; content: string }>> {
  ...
  let res: Response;
  try {
    res = await fetch('https://app.openrind.com/v2/skills/bundle', {
...
/**
 * Download org skills from OpenrindGateway and write them into `homeDir/.claude/skills/`.
 *
...
async function fixBrokenTls(): Promise<boolean> {
  ...
  // Skip confirmation if OPENRIND_SHELL_AUTO_FIX_TLS=1 is set (for automation/CI)
  if (process.env.OPENRIND_SHELL_AUTO_FIX_TLS !== '1') {
    process.stderr.write(
      ...
      '\x1b[33mTo proceed automatically in the future, set: OPENRIND_SHELL_AUTO_FIX_TLS=1\x1b[0m\n' +
  ...
  const tmpDir = '/tmp/openrind-shell-tls';
  try {
  ...
  process.stderr.write('\x1b[2mopenrind-shell: waiting for pod to restart with new certs...\x1b[0m\n');
  ...
  process.stderr.write('\x1b[2mopenrind-shell: verifying TLS connection...\x1b[0m\n');
 ...
 * "openrind-shell-sandbox:dev" becomes "docker.io/library/openrind-shell-sandbox:dev".
 ...
 *
 * Looks for `sandboxes/openrind-shell/Dockerfile` as a landmark file so the
...
export function findRepoRoot(maxLevels = 6): string | null {
  ...
  for (let i = 0; i < maxLevels; i++) {
    if (existsSync(join(dir, 'sandboxes', 'openrind-shell', 'Dockerfile'))) {
      return dir;
    }
...
async function ensureSandboxImage(sandboxImage: string): Promise<void> {
  ...
  function imageExistsInCluster(): boolean {
    ...
    // ctr ls --quiet lists fully-qualified refs like: docker.io/library/openrind-shell-sandbox:dev
    // Match exact expanded ref only — substring matching causes false positives
    // (e.g. openrind-shell-sandbox:dev-openrind-shell-flat also contains "openrind-shell-sandbox" and "dev").
    return refs.split('\n').some(line => {
      const t = line.trim();
  ...
  process.stderr.write('\x1b[2mopenrind-shell: checking sandbox image availability...\x1b[0m\n');
  ...
  // Local-only images (e.g. openrind-shell-sandbox:dev built with `docker build`)
  ...
  if (!imageExistsLocally) {
    const devImageName = process.env.OPENRIND_SHELL_DEV_IMAGE ?? 'openrind-shell-sandbox:dev';
    ...
    if (isDevImage) {
      ...
      const dockerfile = repoRoot ? join(repoRoot, 'sandboxes', 'openrind-shell', 'Dockerfile') : null;
      ...
      if (!dockerfile || !existsSync(dockerfile)) {
        ...
        process.stderr.write(
          ...
          `  ${location}\n` +
          'Run from inside the openrind-shell repo, or build the image manually:\n' +
          `  docker build -f sandboxes/openrind-shell/Dockerfile -t ${sandboxImage} <repo-root>\n`
        );
        process.exit(1);
      }
      ...
      const dockerCfgDir = join(homedir(), '.config', 'openrind-shell', 'docker');
  ...
  const fsTar    = '/tmp/openrind-shell-sandbox-fs.tar';
  const tmpTar   = '/tmp/openrind-shell-sandbox-image.tar';
  const containerTar = '/tmp/openrind-shell-sandbox-image.tar';
  const flatTag  = `${sandboxImage}-openrind-shell-flat`;
  ...
  const flatContainer = `openrind-shell-flatten-${Date.now()}`;
...
async function cleanupExistingSandbox(workspaceId: string): Promise<void> {
  ...
  process.stderr.write(`\x1b[2mopenrind-shell: cleaning up existing sandbox '${workspaceId}'...\x1b[0m\n`);
...
/**
 * `npx openrind-shell presign` — show the currently stored OpenrindGateway presign.
 */
async function cmdPresignShow(): Promise<void> {
  ...
  if (!stored) {
    ...
    console.log('  npx openrind-shell presign renew');
    return;
  }

  const { decodePresignUrl } = await import('./optimize/openrind-gateway-api.js');
  const decoded = decodePresignUrl(stored.url);

  console.log('OpenrindGateway presign (currently in use):');
  ...
  if (stored.openrindGatewayApiKey) {
    const k = stored.openrindGatewayApiKey;
    const masked = k.length > 8 ? `${k.slice(0, 4)}...${k.slice(-4)}` : '****';
    console.log(`  OpenrindGateway API Key: ${masked}  (stored — used for automatic skill downloads)`);
  } else {
    console.log(`  OpenrindGateway API Key: not stored — run \`npx openrind-shell presign renew\` to save it`);
  }
  console.log('');
...
/**
 * `npx openrind-shell presign renew` — create a new OpenrindGateway presign and persist it.
 *
 * Prompts for ANTHROPIC_API_KEY and OPENRIND_GATEWAY_API_KEY if not set in env.
 * Writes the new presign to:
 *   - ~/.config/openrind-shell/presign.json  (openrind-shell's own cache)
...
async function cmdPresignRenew(): Promise<void> {
  ...
  let openrindGatewayKey = (process.env.OPENRIND_GATEWAY_API_KEY ?? '').replace('@openrind_gateway_api_key', '').trim();

  if (!anthropicKey || !openrindGatewayKey) {
    ...
    try {
      ...
      if (!openrindGatewayKey) {
        openrindGatewayKey = (await question('OpenrindGateway API key: ')).trim();
      }
    } finally {
      rl.close();
    }
  ...
  try {
    const fullUrl = await createPresignUrl(anthropicKey, openrindGatewayKey);
    const baseUrl = openrindGatewayProxyBaseUrl(fullUrl);

    // 1. Store in openrind-shell's own config (include the API key so skill downloads
    //    work automatically on future runs without the env var being set)
    saveStoredPresign(fullUrl, openrindGatewayKey);
    ...
    console.log('Run "npx openrind-shell presign" to view it, or "npx openrind-shell presign renew" to replace it.');
  } catch (err) {
 ...
 *   2. Create a sandbox from the openrind-shell image and attach to it.
 *      setup.sh runs migrations, seeds the workspace, starts the
 *      openrind-shell-bash daemon, then execs `claude`.
 */
async function launchViaSandbox(workspaceId: string, claudeArgs: string[], devMode = false, agent: AgentKind = 'claude'): Promise<void> {
  const sandboxImage = devMode
    ? (process.env.OPENRIND_SHELL_DEV_IMAGE ?? 'openrind-shell-sandbox:dev')
    : (process.env.OPENRIND_SHELL_SANDBOX_IMAGE ?? 'ghcr.io/openrind/openrind-shell/sandbox:just-bash');
  ...
  if (checkResult.error) {
    ...
    if (isNotFound) {
      process.stderr.write(
        ...
        'openrind-shell runs Claude Code inside an OpenShell sandbox — install OpenShell first:\n' +
  ...
  if (networkCheck.status !== 0) {
    process.stderr.write('\x1b[2mopenrind-shell: creating OpenShell network...\x1b[0m\n');
  ...
  if (!containerExists) {
    ...
    process.stderr.write('\x1b[2mopenrind-shell: starting OpenShell gateway (this may take 5-8 minutes on first run)...\x1b[0m\n');
    ...
    process.stderr.write('\x1b[2mopenrind-shell: waiting for gateway to initialize (this may take 5-8 minutes on first run)...\x1b[0m\n');
    ...
    process.stderr.write('\x1b[2mopenrind-shell: gateway container is stopped, starting it...\x1b[0m\n');
    ...
    process.stderr.write('\x1b[2mopenrind-shell: waiting for gateway to resume...\x1b[0m\n');
    ...
    process.stderr.write('\x1b[2mopenrind-shell: gateway is already running\x1b[0m\n');
  }
  ...
  //   - Stored presign present → use it; ANTHROPIC_API_KEY and OPENRIND_GATEWAY_API_KEY are not needed.
  //   - No stored presign       → require both keys to create one now, then store it.
  // Run `npx openrind-shell presign renew` to replace the stored presign at any time.
  let openrindGatewayUrl: string | undefined;
  ...
  if (agent === 'openclaw') {
    ...
    keyUploadPath = join(tmpdir(), `openrind-shell-anthropic-key-${process.pid}-${Date.now()}`);
    ...
    if (storedPresign) {
      ...
      openrindGatewayUrl = openrindGatewayProxyBaseUrl(storedPresign.url);
      process.stderr.write('\x1b[32m✓ Using stored OpenrindGateway presign\x1b[0m\n');
      process.stderr.write(`\x1b[2m  Proxy URL: ${openrindGatewayUrl}\x1b[0m\n`);
    } else {
      ...
      const openrindGatewayKey = (process.env.OPENRIND_GATEWAY_API_KEY ?? '').replace('@openrind_gateway_api_key', '').trim();

      if (!anthropicKey || !openrindGatewayKey) {
        process.stderr.write(
          '\x1b[31merror: no stored presign found and required keys are missing.\x1b[0m\n' +
          'Either run `npx openrind-shell presign renew` once (requires both keys), or set:\n' +
      ...
      process.stderr.write('\x1b[2mopenrind-shell: no stored presign — creating permanent presign...\x1b[0m\n');
      try {
        const fullUrl = await createPresignUrl(anthropicKey, openrindGatewayKey);
        saveStoredPresign(fullUrl, openrindGatewayKey);
        openrindGatewayUrl = openrindGatewayProxyBaseUrl(fullUrl);
        process.stderr.write('\x1b[32m✓ OpenrindGateway presign created and stored (expires_in=-1, max_uses=-1, cost_limit=$10)\x1b[0m\n');
      } catch (err) {
        const error = err instanceof Error ? err : new Error(String(err));
        process.stderr.write('\x1b[31merror: failed to create OpenrindGateway presign: ' + error.message + '\x1b[0m\n');
        process.exit(1);
      }
    }

    if (!openrindGatewayUrl && process.env.ANTHROPIC_API_KEY) {
  ...
  if (dbUrl) {
    dbUploadPath = join(tmpdir(), `openrind-shell-db-url-${process.pid}-${Date.now()}`);
  ...
  // OpenClaw routes through setup.sh (-- openrind-shell) which handles everything.
  let skillsBase64 = '';
  if (agent !== 'openclaw') {
    // Resolve the OpenrindGateway API key for org skills download.
    // Priority: env var > key stored alongside presign.
    const storedPresignForSkills = loadStoredPresign();
    const envSkillsKey = (process.env.OPENRIND_GATEWAY_API_KEY ?? '').replace('@openrind_gateway_api_key', '').trim();
    const storedSkillsKey = storedPresignForSkills?.openrindGatewayApiKey?.trim() ?? '';
    const openrindGatewayKeyForSkills = envSkillsKey || storedSkillsKey;
    ...
    if (envSkillsKey && storedPresignForSkills && envSkillsKey !== storedSkillsKey) {
      ...
      process.stderr.write('\x1b[2mopenrind-shell: stored OpenrindGateway API key updated from env\x1b[0m\n');
    }

    if (openrindGatewayKeyForSkills) {
      process.stderr.write('\x1b[2mopenrind-shell: downloading org skills...\x1b[0m\n');
      try {
        const skills = await fetchOrgSkills(openrindGatewayKeyForSkills);
...
# OpenrindGateway proxy URL (passed as environment variable from host)
export OPENRIND_GATEWAY_PROXY_URL="${openrindGatewayUrl || ''}"

# OpenrindGateway integration - if OPENRIND_GATEWAY_PROXY_URL is set, use it as ANTHROPIC_BASE_URL
if [ -n "\${OPENRIND_GATEWAY_PROXY_URL:-}" ]; then
  export ANTHROPIC_BASE_URL="\${OPENRIND_GATEWAY_PROXY_URL}"
  echo "setup: using OpenrindGateway proxy at \${ANTHROPIC_BASE_URL}"
fi
...
if [ -z "\${DATABASE_URL:-}" ]; then
  ...
  echo "setup:   Store one with: npx openrind-shell db-url postgresql://user:pass@host/db" >&2
...
${skillsBase64 ? `# Write org skills downloaded from OpenrindGateway (base64-encoded JSON bundle)
node -e "
...
"
` : '# No org skills (OPENRIND_GATEWAY_API_KEY not set or bundle download skipped)'}

# Write OpenrindGateway proxy config directly into Claude Code settings.json so
...
console.log('setup: OpenrindGateway proxy written to ~/.claude/settings.json');
"
fi
...
echo "setup: starting openrind-shell-bash daemon..."
...
# openrindGatewayUrl was normalized by openrindGatewayProxyBaseUrl() on the host.
#
...
# key to settings.json; settings only stores the proxy base URL. OPENRIND_GATEWAY_API_KEY
...
`;
  ...
  if (agent === 'openclaw') {
    // OpenClaw: delegate entirely to setup.sh (the `openrind-shell` shim in the image).
    // setup.sh reads OPENRIND_SHELL_AGENT=openclaw from the openclaw provider and handles
    // migrations, seeding, daemon, and openclaw launch.
    sandboxArgs.push('--', 'openrind-shell', ...claudeArgs);
  } else {
  ...
  if (dbUrl) {
    process.stderr.write('\x1b[2mopenrind-shell: verifying database connection...\x1b[0m\n');
  ...
  process.stderr.write('\x1b[2mopenrind-shell: registering providers...\x1b[0m\n');

  if (agent === 'openclaw') {
    // openclaw provider injects OPENRIND_SHELL_AGENT=openclaw so setup.sh launches OpenClaw.
    const creds = ['--credential', 'OPENRIND_SHELL_AGENT=openclaw'];
  ...
  process.stderr.write(
    `\x1b[2mopenrind-shell: launching ${agentLabel} in OpenShell sandbox (${workspaceId})...\x1b[0m\n` +
    `\x1b[2m  (if startup stalls for >3 min, press Ctrl+C and retry with OPENRIND_SHELL_AUTO_FIX_TLS=1)\x1b[0m\n\n`,
  );
  ...
  const startupWarnTimer = setTimeout(() => {
    process.stderr.write(
      ...
      '  OPENRIND_SHELL_AUTO_FIX_TLS=1 npx openrind-shell\n' +
      'Diagnostics:\n' +
      '  docker logs openshell-cluster-openshell\n' +
      '  docker exec openshell-cluster-openshell kubectl get pods -n openshell\n\n'
    );
...
export async function main() {
  ...
  // Separate openrind-shell's own args from claude passthrough args (after --)
  ...
  // `npx openrind-shell --dev presign` works — the flag can appear anywhere before --)
  ...
  if (ownArgs[0] === 'stats' || ownArgs[0] === 'analyze' || ownArgs[0] === 'apply') {
    ...
    const stored = loadStoredPresign();
    if (stored) env['OPENRIND_SHELL_PRESIGN_URL'] = stored.url;
  ...
  process.stderr.write(`\x1b[2mopenrind-shell: agent      ${agent}\x1b[0m\n`);
  if (devMode) {
    const devImage = process.env.OPENRIND_SHELL_DEV_IMAGE ?? 'openrind-shell-sandbox:dev';
    process.stderr.write(`\x1b[2mopenrind-shell: mode       dev (${devImage})\x1b[0m\n`);
  }
