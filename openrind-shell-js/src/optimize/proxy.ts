 ...
 * real Anthropic API (or a OpenrindGateway proxy URL if configured).
 *
...
export interface ProxyConfig {
  ...
  /** Upstream base URL, e.g. 'https://api.anthropic.com' or a OpenrindGateway URL. */
...
export async function startProxy(config: Partial<ProxyConfig> & { port?: number; optimizerEnabled?: boolean } = {}): Promise<OptimizerProxy> {
  const proxy = new OptimizerProxy({
    ...
    workspaceId: config.workspaceId ?? process.env.OPENRIND_SHELL_WORKSPACE_ID ?? 'default',
