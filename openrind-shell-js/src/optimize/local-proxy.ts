/**
 * Lightweight local proxy that optimizes requests before sending to OpenrindGateway
 * Runs automatically when npx openrind-shell starts
 */
...
export interface ProxyOptions {
  targetUrl: string; // OpenrindGateway URL or Anthropic URL
...
async function handleProxyRequest(
  ...
  if (options.optimizationEnabled) {
    ...
    if (process.env.OPENRIND_SHELL_DEBUG) {
      console.log(`[Optimizer] ${originalModel} → ${optimalModel}, saved ${tokensSaved} tokens`);
    }
  }

  // Forward to target (OpenrindGateway or Anthropic)
