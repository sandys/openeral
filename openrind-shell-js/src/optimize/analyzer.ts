...
export interface StrategicReport {
  ...
  proposals: Proposal[];
  /**
   * null  = file-path tracking is not available (OpenrindGateway only stores token counts, not file paths)
...
function detectMemoryDir(projectRoot: string, workspaceId: string): string | null {
  ...
  const candidates = [
    join(process.env.OPENRIND_SHELL_HOME ?? `/tmp/openrind-shell-${workspaceId}`, '.claude', 'projects', encodedProject, 'memory'),
...
async function querySessionStats(
  ...
  try {
    await pool.query(`SELECT 1 FROM _openrind.optimization_metrics LIMIT 1`);
  } catch {
  ...
  const totals = await pool.query<{
    ...
    `SELECT
     ...
     FROM _openrind.optimization_metrics
...
async function findActualHotspots(
  ...
  try {
    ...
    // OpenrindGateway events never populate file_path — only the old proxy-based optimizer did.
    ...
    const hasFilePathData = await pool.query<{ has_data: string }>(
      `SELECT EXISTS (
         SELECT 1 FROM _openrind.optimization_metrics
         WHERE workspace_id = $1
           AND timestamp > NOW() - INTERVAL '1 day' * $2
    ...
    if (hasFilePathData.rows[0]?.has_data !== 'true') {
      // No file-path tracking data — OpenrindGateway only logs token counts per API call,
...
function buildProposals(
  ...
  if (!surface.hasModelRouting && stats.hasData) {
    ...
    if (expensivePct > 20 || wastedCalls > 0) {
      ...
      proposals.push({
        ...
        howToApply: 'Run: npx openrind-shell optimize apply --proposal model-routing',
        canAutoApply: true,
      });
    }
  }
  ...
  if (!surface.hasContextFile && !surface.hasContextFileInstruction && stats.hasData) {
    ...
    if (stats.avgTokensPerSession > 2000) {
      ...
      proposals.push({
        ...
        estimatedSavingsTokensPerSession: savings,
        howToApply: 'Run: npx openrind-shell optimize apply --proposal context-file',
        canAutoApply: true,
      });
    }
  }
  ...
  if (hasReadme && !surface.hasReadmeUpdateInstruction && stats.hasData) {
    ...
    if (avgTokens > 3000) {
      ...
      proposals.push({
        ...
        estimatedSavingsTokensPerSession: savings,
        howToApply: 'Run: npx openrind-shell optimize apply --proposal readme-updates',
        canAutoApply: true,
      });
    }
  }
  ...
  if (!surface.hasLazyLoadInstruction && stats.hasData && stats.avgTokensPerCall > 2500) {
    ...
    proposals.push({
      ...
      estimatedSavingsTokensPerSession: savings,
      howToApply: 'Run: npx openrind-shell optimize apply --proposal lazy-reading',
      canAutoApply: true,
    });
  }
  ...
  if (surface.memoryFiles.length > 2 || memTokens > 1500) {
    ...
    proposals.push({
      ...
      estimatedSavingsTokensPerSession: savings,
      howToApply: 'Run: npx openrind-shell optimize apply --proposal memory-compact',
      canAutoApply: true,
    });
  }
...
export async function analyzePromptSurface(opts: AnalyzeOptions = {}): Promise<StrategicReport> {
  const workspaceId = opts.workspaceId ?? process.env.OPENRIND_SHELL_WORKSPACE_ID ?? hostname();
...
export function formatPromptSurfaceReport(report: StrategicReport): string {
  ...
  lines.push('# Openrind Token Analysis Report');
  lines.push('');
  ...
  if (s.hasData) {
    ...
    lines.push('');
    lines.push('  No session data yet. Run a Claude Code session via `npx openrind-shell` to collect data.');
  ...
  if (report.hotspots === null) {
    // OpenrindGateway proxy logs token counts per API call but not which files were read.
    ...
    lines.push('  OpenrindGateway records token counts per API call but does not track which');
    ...
    if (s.hasData && s.avgTokensPerCall > 1000) {
      ...
      lines.push('  read in full. Consider adding lazy-reading rules (npx openrind-shell apply --proposal lazy-reading).');
    }
