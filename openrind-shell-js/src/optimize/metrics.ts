...
export async function storeMetrics(pool: DbPool, metrics: OptimizationMetrics): Promise<void> {
  await pool.query(
    `INSERT INTO _openrind.optimization_metrics (
...
export function formatStats(stats: Awaited<ReturnType<typeof getOptimizationStats>>, days = 7): string {
  const lines = [
    `Openrind - Usage Statistics (last ${days} day${days === 1 ? '' : 's'})`,
    '═'.repeat(60),
    '',
    'COST',
  ...
  if (stats.apiCallsMade === 0) {
    lines.push('  No data yet — run sessions via "npx openrind-shell" first.');
    if (!process.env.OPENRIND_GATEWAY_API_KEY) {
      lines.push('  Set OPENRIND_GATEWAY_API_KEY to sync live usage data from OpenrindGateway.');
    }
  } else {
