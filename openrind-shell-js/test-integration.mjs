...
/**
 * Integration test: verifies openrind-shell-js against a live PostgreSQL database.
 *
 * Usage: DATABASE_URL='...' node test-integration.mjs
 */

import { createOpenrindShell } from './dist/shell.js';
...
async function run() {
  ...
  shell = await createOpenrindShell({
    connectionString,
    workspaceId,
    migrate: true,
  });
  ...
  r = await shell.exec('echo "hello openrind-shell" > /home/agent/test.txt');
  assertExitCode('write file exits 0', r, 0);

  r = await shell.exec('cat /home/agent/test.txt');
  assertExitCode('cat file exits 0', r, 0);
  assert('file content matches', r.stdout, 'hello openrind-shell\n');
  ...
  const shell2 = await createOpenrindShell({
    connectionString,
    workspaceId,
    migrate: false,
  });
  ...
  assert('persist: file content matches', r.stdout, 'hello openrind-shell\n');
  ...
  await pool.query('DELETE FROM _openrind.workspace_files WHERE workspace_id = $1', [workspaceId]);
  await pool.query('DELETE FROM _openrind.workspace_config WHERE id = $1', [workspaceId]);
  await pool.end();
