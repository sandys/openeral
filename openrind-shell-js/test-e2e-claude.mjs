...
async function run() {
  const { createOpenrindShell } = await import('./dist/shell.js');
  ...
  const shell1 = await createOpenrindShell({
  ...
  const shell3 = await createOpenrindShell({
    connectionString: DB_URL,
    workspaceId: WORKSPACE,
    migrate: false,
  });
  ...
  const { rows } = await pool.query(
    `SELECT path, is_dir, size FROM _openrind.workspace_files
